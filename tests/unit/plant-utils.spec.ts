  ...
  describe('Image Compression', () => {
    ...
    beforeEach(() => {
      ...
      class MockFileReader {
        readAsDataURL() {
          this.onload({ target: { result: 'data:image/png;base64,source' } } as any);
        }
        onload: any;
      }
      ...
      class MockImage {
        ...
        height = 1000;
        onload: any;
        set onloadFunc(fn: any) {
          this.onload = fn;
          /* trigger immediately for test */ setTimeout(fn, 0);
        }
      }
...
describe('preloadImage', () => {
  ...
  it('should reject on error', async () => {
    ...
    await expect(PlantUtils.preloadImage('bad.jpg')).rejects.toThrow(
      'Failed to load image: bad.jpg'
    );
    vi.restoreAllMocks();
  });
});
...
describe('compressImage Error Handling', () => {
  ...
  it('should reject if Image loading fails', async () => {
    ...
    vi.stubGlobal(
      ...
      class {
        readAsDataURL() {
          setTimeout(() => this.onload({ target: { result: 'data:...' } } as any), 0);
        }
        onload: any;
        onerror: any;
      }
    ...
    vi.stubGlobal(
      ...
      class {
        set src(v: string) {
          setTimeout(() => this.onerror(new Error('Image fail')), 0);
        }
        onload: any;
        onerror: any;
      }
