...
import {
  saveHarvestMetrics as sliceSaveHarvestMetrics,
...
describe('PlantHarvestTab', () => {
  ...
  beforeEach(async () => {
    vi.mocked(sliceSaveHarvestMetrics).mockReset().mockResolvedValue(undefined);
  ...
  it('calls store actions and dispatches harvest-saved on save click', async () => {
    ...
    expect(sliceSaveHarvestMetrics).toHaveBeenCalledWith('p123', expect.any(Object));
    expect(sliceScorePlant).toHaveBeenCalledWith('p123', expect.any(Object));
    expect(savedCalled).toBe(true);
    expect((element as any)._savingHarvest).toBe(false);
  });

  it('handles error during save', async () => {
    vi.mocked(sliceSaveHarvestMetrics).mockRejectedValue(new Error('Failed'));
