...
describe('PlantHarvestTab', () => {
  ...
  it('renders correctly with initial data', () => {
    ...
    // Check scores
    const scoreRows = element.shadowRoot?.querySelectorAll(
      'div[style*="flex-direction:column; gap:6px;"]'
    );
    expect(scoreRows?.length).toBe(5);

    const vigorScore = scoreRows?.[0]
  ...
  it('updates harvest metrics on input', async () => {
    const wetInput = element.shadowRoot?.querySelector(
    ...
    const dryInput = element.shadowRoot?.querySelector(
    ...
    const trimInput = element.shadowRoot?.querySelector(
  ...
  it('updates lab results on input', async () => {
    const thcInput = element.shadowRoot?.querySelector(
      'input[placeholder*="e.g. 24.5"]'
    ) as HTMLInputElement;
    thcInput.value = '25.5';
    thcInput.dispatchEvent(new InputEvent('input'));
    expect((element as any)._harvestMetricsEdit.thc_percentage).toBe(25.5);

    const cbdInput = element.shadowRoot?.querySelector(
