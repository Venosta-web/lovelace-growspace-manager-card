...
describe('growspace-grid-ui', () => {
  ...
  describe('Events', () => {
    it('emits empty-slot-click when empty slot is clicked', async () => {
      ...
      el.addEventListener('empty-slot-click', (e: any) => {
        payload = e.detail;
      });
    ...
    it('emits grid-drop on empty slot drop', async () => {
      ...
      let payload: any = null;
      el.addEventListener('grid-drop', (e: any) => {
        payload = e.detail;
      });
    ...
    it('emits plant-drag-start and grid-drop when plant card container dragged and dropped', async () => {
      ...
      el.addEventListener('plant-drag-start', (e: any) => {
        dragPayload = e.detail;
      });
      el.addEventListener('grid-drop', (e: any) => {
        dropPayload = e.detail;
      });

      const plantContainer = el.shadowRoot?.querySelector('plant-card-container') as any;

      // Simulate events emitted by the child
      plantContainer.dispatchEvent(
        new CustomEvent('plant-drag-start', { detail: { plant: defaultPlant } })
      );
      expect(dragPayload.plant).to.deep.equal(defaultPlant);

      // Now drop it on the same container
      plantContainer.dispatchEvent(
        new CustomEvent('plant-drop', { detail: { originalEvent: new Event('drop') } })
      );
    ...
    it('forwards mobile drop events', async () => {
      const el = await fixture<GrowspaceGridUI>(html` <growspace-grid-ui></growspace-grid-ui> `);

      let payload: any = null;
      el.addEventListener('grid-mobile-drop', (e: any) => {
        payload = e.detail;
      });
    ...
    it('forwards cell click events', async () => {
      ...
      let payload: any = null;
      el.addEventListener('cell-click', (e: any) => {
        payload = e.detail;
      });
