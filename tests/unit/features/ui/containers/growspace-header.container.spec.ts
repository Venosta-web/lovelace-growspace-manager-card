...
const buildMockStore = () => {
  ...
  return {
    ...
    get openAddPlantDialog() {
      return ui_actions.openAddPlantDialog;
    },
    get openConfigDialog() {
      return ui_actions.openConfigDialog;
    },
    get openStrainLibraryDialog() {
      return ui_actions.openStrainLibraryDialog;
    },
    get openIrrigationDialog() {
      return ui_actions.openIrrigationDialog;
    },
    get openGrowMasterDialog() {
      return ui_actions.openGrowMasterDialog;
    },
    get openLogbookDialog() {
      return ui_actions.openLogbookDialog;
    },
    get openSnapshotsDialog() {
      return ui_actions.openSnapshotsDialog;
    },
    get openWateringDialog() {
      return ui_actions.openWateringDialog;
    },
    get openIPMDialog() {
      return ui_actions.openIPMDialog;
    },
    get openNutrientsDialog() {
      return ui_actions.openNutrientsDialog;
    },
    get toggleEnvGraph() {
...
describe('GrowspaceHeaderContainer', () => {
  ...
  it('_handleToggleGraph with object detail toggles the requested reading graph', () => {
    (element as any)._handleToggleGraph(
      new CustomEvent('toggle-graph', { detail: { metric: 'humidity' } })
    );
  ...
  it('_handleToggleGraph with empty metric does not call toggleEnvGraph', () => {
    (element as any)._handleToggleGraph(
      new CustomEvent('toggle-graph', { detail: { metric: '' } })
    );
  ...
  it('add_plant action calls store.actions.ui.openAddPlantDialog', () => {
    ...
    expect(uiSlice.openAddPlantDialog).toHaveBeenCalledOnce();
  });
  ...
  it('ipm action with no selected plants calls openIPMDialog without plantIds', () => {
    ...
    expect(uiSlice.openIPMDialog).toHaveBeenCalledWith(
  ...
  it('action does nothing when store is not set', () => {
    ...
    expect(uiSlice.openAddPlantDialog).not.toHaveBeenCalled();
  });
  ...
  describe('Drag and Drop', () => {
    it('_handleChipDrop calls dragController.handleDrop and invokes linkGraphs callback', () => {
      ...
      (element as any)._dragController.handleDrop.mockImplementation(
        (event: any, target: any, callback: any) => {
          callback('temp', 'hum');
        }
      );
    ...
    it('_handleChipDragStart calls dragController.handleDragStart', () => {
      ...
      expect((element as any)._dragController.handleDragStart).toHaveBeenCalledWith(
        e.detail.event,
        'temp'
      );
    });
    ...
    it('_handleUnlinkGraphs does nothing if store.history is missing', () => {
      ...
      (element as any).store.history = undefined;
      (element as any)._handleUnlinkGraphs(
        new CustomEvent('unlink', { detail: { groupIndex: 0 } })
      );
