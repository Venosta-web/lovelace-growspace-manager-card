...
describe('GrowspaceManagerCard', () => {
  ...
  beforeEach(() => {
    ...
    atomMocks.$cardViewState = computed(
      [
        atomMocks.$viewMode,
        atomMocks.$isLoading,
        atomMocks.$isEditMode,
        atomMocks.$isCompactView,
        atomMocks.$activeDialog,
        atomMocks.$notification,
        atomMocks.$focusedPlantIndex,
        atomMocks.$selectedPlants,
        atomMocks.$gridOverlayMode,
      ],
      (
        viewMode,
        isLoading,
  ...
  describe('Lifecycle & Rendering', () => {
    ...
    it('should sync strain library when controller updates', async () => {
      (element as any)._viewController = {
        value: { grid: {}, ui: {}, strainLibrary: [{ strain: 'new' }] },
      };
  ...
  describe('Rendering Logic', () => {
    ...
    it('should render no data message if no devices', async () => {
      ...
      atomMocks.$gridViewState.set({
        devices: [],
        selectedDevice: null,
        gridLayout: { effectiveRows: 0, grid: [] },
        growspaceOptions: {},
      });
      document.body.appendChild(element);
      await element.updateComplete;
  ...
  describe('Event Handlers', () => {
    ...
    it('should handle errors and report to system log', () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        'Growspace Manager Card caught error:',
        error,
        errorInfo
      );
    ...
    it('should exit transplant mode and restore edit bar when transplant mode button is clicked while transplanting', async () => {
      const { gridInteraction$, completeTransplant } =
        await import('../../src/slices/grid-interaction');
      vi.mocked(gridInteraction$.get).mockReturnValue({
    ...
    it('should not call startTransplant when already transplanting', async () => {
      const { gridInteraction$, startTransplant } =
        await import('../../src/slices/grid-interaction');
      vi.mocked(gridInteraction$.get).mockReturnValue({
        status: 'transplanting',
        sourcePlantId: null,
  ...
  describe('setMutateListener callback', () => {
    it('should show a success toast with undo button when a mutate action fires', () => {
      ...
      vi.mocked(setMutateListener).mockImplementation((fn) => {
        capturedListener = fn;
      });
