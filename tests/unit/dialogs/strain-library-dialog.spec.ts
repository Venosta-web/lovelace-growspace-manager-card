...
describe('StrainLibraryDialog', () => {
  ...
  const openManageMenu = async (): Promise<HTMLElement[]> => {
    ...
    await browseView?.updateComplete;
    return Array.from(
      browseView?.shadowRoot?.querySelectorAll('.manage-menu-item') || []
    ) as HTMLElement[];
  };
  ...
  describe('Delete Flow', () => {
    ...
    it('should dispatch delete-strain and return to browse when editor fires delete-strain', async () => {
      ...
      expect(listener).toHaveBeenCalledWith(
        expect.objectContaining({ detail: { key: mockStrains[0].key } })
      );
      expect((element as any)._view).toBe('browse');
    });
  ...
  describe('Import Dialog UI', () => {
    ...
    it('should close on cancel', async () => {
      const cancelBtn = Array.from(element.shadowRoot?.querySelectorAll('button') || []).find((b) =>
    ...
    it('should render Select File button in import dialog', async () => {
      const selectBtn = Array.from(element.shadowRoot?.querySelectorAll('button') || []).find((b) =>
  ...
  describe('Delete Confirmation UI', () => {
    ...
    it('should confirm delete via UI button', async () => {
      ...
      const delBtn = (Array.from(overlay?.querySelectorAll('button') || []) as HTMLElement[]).find(
        (b) => b.textContent?.trim() === 'Delete' && b.classList.contains('text')
      );
    ...
    it('should cancel delete via UI button', async () => {
      ...
      const overlay = browseView?.shadowRoot?.querySelector('.crop-overlay');
      const cancelBtn = (
        Array.from(overlay?.querySelectorAll('button') || []) as HTMLElement[]
  ...
  describe('Import Dialog Extras', () => {
    it('should close via header button', async () => {
      ...
      const closeBtn = Array.from(importOverlay?.querySelectorAll('button') || []).find(
        (b) => b.classList.contains('text') && !b.textContent?.trim()
      ); // icon button only?
  ...
  describe('Coverage Gap Fillers', () => {
    ...
    it('should toggle manage menu via header button (browse view)', async () => {
      ...
      const buttons = Array.from(
        browseView?.shadowRoot?.querySelectorAll('.header-actions button') || []
      ) as HTMLElement[];
