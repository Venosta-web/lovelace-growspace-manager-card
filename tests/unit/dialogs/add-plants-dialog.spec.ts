...
describe('AddPlantsDialog', () => {
  ...
  it('renders header correctly', () => {
    expect(element.shadowRoot?.querySelector('.dialog-title')?.textContent).toBe(
      'Batch Add Plants'
    );
  ...
  it('updates phenotype and addToLibrary toggle', async () => {
    const phenoInput = element.shadowRoot?.querySelector(
      'md3-text-input[label="Phenotype (Optional)"]'
    );
  ...
  it('covers all branch variants of renderTimelineContent', async () => {
    ...
    await element.updateComplete;
    expect(
      element.shadowRoot?.querySelector('md3-date-input[label="Seedling Start"]')
    ).toBeTruthy();
  ...
  it('renders and updates default date inputs', async () => {
    ...
    const seedlingInput = element.shadowRoot?.querySelector(
      'md3-date-input[label="Seedling Start"]'
    );
  ...
  it('shows validation error when growspace slots are full', async () => {
    ...
    const addBtn = restrictedElement.shadowRoot?.querySelector(
      '.md3-button.primary'
    ) as HTMLElement;
    addBtn.click();
  ...
  it('handles missing device properties correctly (0 slots)', async () => {
    const elementPartial = await fixture(html`
      <add-plants-dialog .open=${true} .growspaceDevice=${{ deviceId: 'g1' }}></add-plants-dialog>
    `);
    ...
    const elementSuccess = await fixture(html`
      <add-plants-dialog
        ...
        .growspaceDevice=${{
          rows: 4,
          plantsPerRow: 4,
          plants: ['p1'], // 16 slots, 1 occupied, 15 free
        }}
      ></add-plants-dialog>
    `);
