...
describe('SeedsGeneticsTab Coverage', () => {
  ...
  describe('Seed Batch Actions', () => {
    it('handles editing a seed batch', async () => {
      ...
      const saveBtn = Array.from(
        element.shadowRoot?.querySelectorAll('.form-actions button') || []
    ...
    it('handles batch deletion confirmation', async () => {
      ...
      const confirmBtn = element.shadowRoot?.querySelector(
        '.icon-btn.danger[title="Confirm delete"]'
      ) as HTMLElement;
      confirmBtn.click();
      await new Promise((resolve) => setTimeout(resolve, 0));
    ...
    it('handles sowing seeds', async () => {
      const sowBtn = Array.from(
        element.shadowRoot?.querySelectorAll('.seed-batch-actions button') || []
      ...
      const plantBtn = Array.from(
        element.shadowRoot?.querySelectorAll('.sow-form button') || []
  ...
  describe('Pollination Actions', () => {
    ...
    it('handles pollination deletion confirmation', async () => {
      const deleteBtn = element.shadowRoot?.querySelector(
        '.pollination-card-actions .icon-btn.danger'
      ) as HTMLElement;
      deleteBtn.click();
      await element.updateComplete;
  ...
  describe('Form Input Handlers', () => {
    it('exercises all add-batch form handlers', async () => {
      ...
      const parent2Select = element.shadowRoot?.querySelectorAll(
        '.form-view select'
