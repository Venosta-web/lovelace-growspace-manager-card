...
describe('SubareaConfigDialog', () => {
  ...
  it('should populate state from subarea on update', async () => {
    expect(field(element, 0).shadowRoot?.querySelector('.chip-name')?.textContent).toBe(
      'sensor.t1'
    );
    expect(field(element, 1).shadowRoot?.querySelector('.chip-name')?.textContent).toBe(
      'sensor.h1'
    );
  });
  ...
  it('should close on cancel click', () => {
    ...
    const cancelBtn = Array.from(element.shadowRoot?.querySelectorAll('button') || []).find(
  ...
  it('should save by calling updateSubarea and dispatch subarea-updated event', async () => {
    ...
    const saveBtn = Array.from(element.shadowRoot?.querySelectorAll('button') || []).find(
  ...
  it('should save a cleared field as an empty list, never an omitted key', async () => {
    ...
    const config = vi.mocked(subareaSlice.updateSubarea).mock.calls[0][2] as Record<
      string,
      unknown
    >;
