...
describe('ConfigDialog - Branch Coverage Expansion', () => {
    ...
    element.devices = [
      {
        ...
        rows: 4,
        plantsPerRow: 4,
        notificationTarget: 'notify.mobile_app_test',
      },
    ] as any;
  ...
  it('should cover Vision Checkup configuration branches', async () => {
    ...
    const visionSaveBtn = element.shadowRoot?.querySelector(
      'button.md3-button.primary'
    ) as HTMLElement;
  ...
  it('should cover Subarea management branches', async () => {
    vi.clearAllMocks();
    vi.mocked(subareaSlice.addSubarea).mockResolvedValue({
      id: 'sa1',
      name: 'Sub 1',
      environment_config: {},
    });
  ...
  it('should cover Humidifier logic branches', async () => {
    ...
    const humidifierHeader = sectionHeaders(root).find((header) =>
      header.label.includes('Humidity')
    );
    ...
    const openHead = Array.from(element.shadowRoot?.querySelectorAll('.acc-head') ?? []).find((h) =>
      h.closest('.acc-card')?.querySelector('.acc-head-title')?.textContent?.includes('Seedling')
  ...
  it('renders substrate temp sensors in the SENSORS tab', async () => {
    ...
    const substrateLabel = entityPickers(await sensorsShadow(element)).find((picker) =>
      picker.label.includes('Substrate')
    );
  ...
  it('renders all sensor rows in the IRRIGATION tab and triggers chip-remove for each', async () => {
    ...
    const chipRemoves = entityPickers(await sensorsShadow(element)).flatMap((picker) =>
      Array.from(picker.shadowRoot.querySelectorAll<HTMLElement>('.chip-remove'))
    );
  ...
  it('renders the tank list and toggles add/edit/delete form in the TANKS tab', async () => {
    ...
    const editTankBtn =
      (
        Array.from(
          element.shadowRoot?.querySelectorAll('button.md3-button.text:not(.danger)') ?? []
        ) as HTMLElement[]
      ).find((b) => !b.style.minWidth || b.style.padding === '6px') ??
      (Array.from(element.shadowRoot?.querySelectorAll('button') ?? []) as HTMLElement[]).find(
        (b) => b.style.padding === '6px' && !b.classList.contains('error')
      );
    ...
    const tankRowBtns = allBtns.filter(
      (b) =>
    ...
    const formInputs = Array.from(
      tanksTab.querySelectorAll('input.md3-input')
    ) as HTMLInputElement[];
    ...
    const deleteTankBtn = Array.from(
      element.shadowRoot?.querySelectorAll('button.md3-button.text.danger') ?? []
  ...
  it('renders add-subarea form: clicking Add Subarea button and Cancel covers inline handlers', async () => {
    ...
    const addSubareaBtn = Array.from(
      (await sensorsShadow(element)).querySelectorAll('button')
    ...
    // Click Cancel
    const cancelBtn = Array.from((await sensorsShadow(element)).querySelectorAll('button')).find(
  ...
  it('renders subarea list: clicking delete enters confirm, No exits, edit button opens dialog', async () => {
    ...
    const deleteBtn = Array.from((await sensorsShadow(element)).querySelectorAll('button')).find(
      (b) => b.title === 'Delete subarea'
    );
    ...
    const noBtn = Array.from((await sensorsShadow(element)).querySelectorAll('button')).find(
    ...
    (element as any)._handleDeleteSubarea('sa1');
    await element.updateComplete;
    const yesBtn = Array.from((await sensorsShadow(element)).querySelectorAll('button')).find(
    ...
    (element as any)._deleteConfirmSubareaId = '';
    await element.updateComplete;
    const editBtn = Array.from((await sensorsShadow(element)).querySelectorAll('button')).find(
      (b) => b.title === 'Edit sensors'
    );
    expect(editBtn).toBeDefined();
    editBtn!.click();
    await element.updateComplete;
...
describe('ConfigDialog - Fan Controller Panel coverage', () => {
  ...
  it('wind_enabled toggle (lines 2014–2015) expands wind settings; period/amplitude handlers fire (lines 2025–2033)', async () => {
    ...
    const windCb = checkboxes.find((cb) =>
      cb.closest('label')?.textContent?.includes('Dynamic Wind')
    );
    ...
    const fanCard = Array.from(root.querySelectorAll('.detail-card')).find(
      (card) =>
        (card.querySelector('config-section-header') as SectionHeaderElement | null)?.label ===
        'Fan Controller'
    );
    expect(fanCard).toBeDefined();
    const marginGrids = Array.from(fanCard!.querySelectorAll('.row-col-grid[style*="margin-top"]'));
    const windInputs = Array.from(
      marginGrids[marginGrids.length - 1].querySelectorAll('md3-number-input')
    );
