...
describe('IrrigationDialog', () => {
  ...
  describe('Additional Interactions', () => {
    ...
    it('should switch tabs back and forth', async () => {
      ...
      const navItem = (label: string) =>
        Array.from(element.shadowRoot?.querySelectorAll('.v1-nav-item') ?? []).find((el) =>
    ...
    it('should dispatch close event with composed: true', async () => {
      ...
      const closeBtn = Array.from(element.shadowRoot?.querySelectorAll('button') || []).find(
  ...
  describe('Drain Add Dialog', () => {
    ...
    it('shows lights-on read-only with a hint on the Steering tab', async () => {
      ...
      expect(child.shadowRoot?.querySelector('md3-text-input[label="Lights On Time"]')).toBeNull();
  ...
  describe('Configuration Tab', () => {
    beforeEach(async () => {
      ...
      element.hass = {
        states: {
          ...
          'input_boolean.valve': {
            entity_id: 'input_boolean.valve',
            attributes: { friendly_name: 'Valve A' },
          },
    ...
    it('should populate entity selects with filtered and sorted entities', () => {
      ...
      const options = Array.from(pumpSelect?.querySelectorAll('option') || [])
        .map((o) => o.value)
        .filter((v) => v);
  ...
  describe('Tanks Tab', () => {
    ...
    const tanksRoot = () => element.shadowRoot!.querySelector('irrigation-tanks-tab')!.shadowRoot!;
    ...
    beforeEach(async () => {
      ...
      (
        Array.from(tabs ?? []).find((t) => t.textContent?.includes('Tanks')) as HTMLElement
      )?.click();
      await element.updateComplete;
      await renderTanks();
    });
    ...
    it('should hide Tanks tab when no tanks configured', async () => {
      ...
      const labels = Array.from(tabs || []).map((t) =>
        (t.querySelector('span:first-of-type') as HTMLElement)?.textContent?.trim()
      );
  ...
  describe('Footer Save Changes', () => {
    ...
    it('should not render per-tab save buttons inside the dialog body', () => {
      ...
      const bodyPrimaryBtns = Array.from(innerSaveBtns ?? []).filter((b) =>
  ...
  describe('Footer Next Display', () => {
    function footerNextText(): string | null | undefined {
      ...
      const nextSpan = Array.from(spans ?? []).find((s) =>
        s.textContent?.trim().startsWith('Next')
      );
  ...
  describe('Uncovered Lines Coverage', () => {
    ...
    describe('Halt on Runoff EC input (lines 2148-2149)', () => {
      beforeEach(async () => {
        ...
        const steeringTab = Array.from(tabs ?? []).find((t) =>
          t.textContent?.includes('Crop Steering')
        );
      ...
      it('should update _haltOnRunoffEcThreshold when number input changes', async () => {
        (element as any)._sm = transition((element as any)._sm, {
          ...
          partial: { haltOnRunoffEcThreshold: 4.0 },
        });
        await element.updateComplete;

        const sr = await steeringChild(element);
        const haltInput = sr.querySelector(
          'md3-number-input[data-field="haltOnRunoffEcValue"]'
        ) as any;
    ...
    describe('Config tab cycle parameter inputs (lines 2218-2247)', () => {
      beforeEach(async () => {
        ...
        const tabs = element.shadowRoot?.querySelectorAll('.v1-nav-item');
        const configTab = Array.from(tabs ?? []).find((t) =>
          t.textContent?.includes('Configuration')
        );
      ...
      it('should update _soilTriggerPercent from the P2 Direct Trigger input', async () => {
        ...
        const sr = await steeringChild(element);
        const soilInput = Array.from(sr.querySelectorAll('md3-number-input')).find(
          (i) => i.getAttribute('label') === 'P2 Direct Trigger (%)'
        ) as any;
      ...
      it('should set _dailyVolumeCapLiters to null when input is cleared', async () => {
        (element as any)._sm = transition((element as any)._sm, {
          type: 'UPDATE_CONFIG_DRAFT',
          partial: { dailyVolumeCapLiters: 10 },
        });
        await element.updateComplete;
      ...
      it('should update _maxCyclesPerDay from the Max Cycles input', async () => {
        ...
        const allNumberInputs = Array.from(
          configChild().shadowRoot?.querySelectorAll('input[type="number"]') ?? []
        ) as HTMLInputElement[];
        const maxCyclesInput = allNumberInputs.find(
          (i) => i.getAttribute('step') === '1' && !i.getAttribute('max')
        );
    ...
    describe('Water analytics with tank history (lines 2420-2424, 2527-2545)', () => {
      ...
      beforeEach(async () => {
        ...
        const tabs = element.shadowRoot?.querySelectorAll('.v1-nav-item');
        const analyticsTab = Array.from(tabs ?? []).find((t) =>
          t.textContent?.includes('Water Analytics')
        );
      ...
      it('should render consumption buckets chart bars for the last 24 hours', async () => {
        ...
        const chartBars = Array.from(allDivs ?? []).filter((d) =>
          d.getAttribute('title')?.includes('—')
        );
    ...
    describe('Stage aggregates sort (lines 2610-2611)', () => {
      it('should sort and render stage aggregates in descending order', async () => {
        ...
        (element as any)._sm = transition((element as any)._sm, {
          ...
          data: { seedling: 3, veg: 15, flower: 25 },
        });
        await element.updateComplete;
