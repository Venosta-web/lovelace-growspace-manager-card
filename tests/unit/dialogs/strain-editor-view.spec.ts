...
function getSM(el: StrainEditorView) {
...
function getSubKind(el: StrainEditorView) {
  return getSM(el).sub.kind;
}
...
function setBreederDraft(
  el: StrainEditorView,
  draft: { name: string; logo: string; originalName: string }
) {
...
describe('StrainEditorView', () => {
  ...
  describe('Editor Interactions', () => {
    ...
    it('keeps secondary editor actions available from the compact action menu', async () => {
      const trigger = editorEl.shadowRoot?.querySelector(
      ...
      const actions = Array.from(
        editorEl.shadowRoot?.querySelectorAll('.mobile-action-item') ?? []
      ).map((button) => button.textContent?.trim());
      expect(actions).toEqual(['Print label', 'Delete strain']);
    });
  ...
  describe('Delete Flow (Editor)', () => {
    it('should dispatch delete-strain from editor delete button', async () => {
      ...
      const delBtn = Array.from(
        editorEl.shadowRoot?.querySelectorAll('.sd-footer button') || []
  ...
  describe('Editor Fields', () => {
    ...
    it('should update breeder', () => {
      const input = Array.from(editorEl.shadowRoot?.querySelectorAll('input') || []).find((i) =>
        i.parentElement?.textContent?.includes('Breeder')
      );
    ...
    it('should update phenotype', () => {
      const input = Array.from(editorEl.shadowRoot?.querySelectorAll('input') || []).find((i) =>
        i.parentElement?.textContent?.includes('Phenotype')
      );
    ...
    it('should update strain name', () => {
      const input = Array.from(editorEl.shadowRoot?.querySelectorAll('input') || []).find((i) =>
        i.parentElement?.textContent?.includes('Strain Name')
      );
    ...
    it('should update flowering days', () => {
      const inputs = Array.from(editorEl.shadowRoot?.querySelectorAll('input') || []).filter(
        (i) => i.placeholder?.includes('Min') || i.placeholder?.includes('Max')
      );
  ...
  describe('Crop Interaction', () => {
    ...
    it('should close crop overlay on Done', async () => {
      const overlay = editorEl.shadowRoot?.querySelector('.crop-overlay');
      const doneBtn = Array.from(overlay?.querySelectorAll('button') || []).find(
  ...
  describe('Coverage Gaps', () => {
    ...
    it('should click UI Save button in Editor view', async () => {
      ...
      const saveBtn = Array.from(
        editorEl.shadowRoot?.querySelectorAll('.sd-footer button.primary') || []
    ...
    it('should log when a captured gallery upload fails', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        ...
        'Compression Failed inside Input'
      );
      consoleSpy.mockRestore();
    });
  });
  ...
  it('should clamp indica input values and handle NaN', async () => {
    ...
    const indicaInput = editorEl.shadowRoot?.querySelector(
      '.hg-input-label input'
    ) as HTMLInputElement;
    expect(indicaInput).toBeTruthy();
  ...
  describe('Additional Coverage Edge Cases', () => {
    it('covers _handlePrintLabel branch', () => {
      ...
      expect(dispatchSpy).toHaveBeenCalledWith(
        expect.objectContaining({ type: 'open-print-label' })
      );
    ...
    it('clicks the back button and triggers _goBack', async () => {
      ...
      const iconButton = editorEl.shadowRoot?.querySelector(
        '.dialog-header .md3-button.tonal'
      ) as HTMLElement;
    ...
    it('opens Seedfinder dialog when clicking the Seedfinder button', async () => {
      ...
      const seedfinderBtn = Array.from(editorEl.shadowRoot?.querySelectorAll('button') || []).find(
        (b) => b.textContent?.includes('Seedfinder')
      );
    ...
    it('toggles lineage edit mode and loads strain tree', async () => {
      ...
      const editTreeBtn = Array.from(
        editorEl.shadowRoot?.querySelectorAll('.sd-btn-text') || []
      ...
      const viewBtn = Array.from(editorEl.shadowRoot?.querySelectorAll('.sd-btn-text') || []).find(
    ...
    it('navigates to ancestor on node-click if match exists, does nothing if no match', async () => {
      ...
      const navigateSpy = vi
        .spyOn(editorEl as any, '_navigateToAncestor')
        .mockImplementation(() => {});

      // Click with match
      treeComponent?.dispatchEvent(
  ...
  describe('Breeder Management Core Actions', () => {
    ...
    it('updates existing breeder if originalName is present', () => {
      const dispatchSpy = vi.spyOn(editorEl, 'dispatchEvent');
      setBreederDraft(editorEl, {
        name: 'Updated Name',
        logo: 'updatedLogo',
        originalName: 'Old Breeder',
      });
      (editorEl as any)._handleSaveBreeder();
      expect(dispatchSpy).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe('Breeder Logo in Strain Editor Form', () => {
    ...
    it('triggers file input click when Upload Logo button is clicked', async () => {
      const uploadBtn = Array.from(editorEl.shadowRoot?.querySelectorAll('button') || []).find(
        (b) => b.textContent?.includes('Change Logo') || b.textContent?.includes('Upload Logo')
      );
    ...
    it('clears breeder logo when delete logo button is clicked', async () => {
      ...
      const deleteBtn = buttons.find((b) =>
        b.getAttribute('style')?.includes('color: var(--error-color')
      );
  ...
  describe('Dialog Rendering and Interactions', () => {
    it('renders and interacts with the Import Dialog', async () => {
      ...
      const radioButtons = Array.from(
        overlay?.querySelectorAll('input[type="radio"]') || []
      ) as HTMLInputElement[];
      expect(radioButtons.length).toBe(2);
      expect(
        getSubKind(editorEl) === 'importing' ? (getSM(editorEl).sub as any).replace : false
      ).toBe(false);
      ...
      const selectBtn = Array.from(overlay?.querySelectorAll('button') || []).find((b) =>
        b.textContent?.includes('Select File')
      );
      expect(selectBtn).toBeTruthy();

      const importFileSpy = vi
        .spyOn(editorEl as any, '_handleImportFile')
        .mockImplementation(() => {});
      (selectBtn as HTMLElement).click();
      expect(importFileSpy).toHaveBeenCalled();

      const closeBtn = Array.from(overlay?.querySelectorAll('button') || []).find(
        (b) => b.textContent?.includes('Cancel') || b.querySelector('svg')
      );
      expect(closeBtn).toBeTruthy();
      (closeBtn as HTMLElement).click();
      expect(getSubKind(editorEl)).not.toBe('importing');
    ...
    it('renders and interacts with Breeder Dialog - Breeder List mode', async () => {
      ...
      expect(getBreederDraft(editorEl)).toEqual({
        name: 'Breeder A',
        logo: 'logoA',
        originalName: 'Breeder A',
      });

      const closeBtn = Array.from(overlay?.querySelectorAll('button') || []).find(
        (b) => b.textContent?.includes('Close') || b.querySelector('svg')
      );
      expect(closeBtn).toBeTruthy();
      (closeBtn as HTMLElement).click();
      expect(['breeder-list', 'breeder-editing', 'breeder-confirm-delete']).not.toContain(
        getSubKind(editorEl)
      );
    });
    ...
    it('renders and interacts with Breeder Dialog - Breeder Editor mode', async () => {
      ...
      const logoBtn = Array.from(overlay?.querySelectorAll('button') || []).find(
        (b) => b.textContent?.includes('Change Logo') || b.textContent?.includes('Upload Logo')
      );
      ...
      const compressSpy = vi
        ...
        .mockResolvedValue('new-compressed-logo');
      Object.defineProperty(fileInput, 'files', {
      ...
      const saveBtn = Array.from(overlay?.querySelectorAll('button') || []).find((b) =>
      ...
      await editorEl.updateComplete;
      const cancelBtn = Array.from(
        editorEl.shadowRoot?.querySelectorAll('ha-dialog button') || []
      ).find((b) => b.textContent?.includes('Cancel'));
      expect(cancelBtn).toBeTruthy();
      (cancelBtn as HTMLElement).click();
      expect(getBreederDraft(editorEl)).toBeNull();
    ...
    it('clears breeder logo in Breeder Editor mode', async () => {
      ...
      const deleteLogoBtn = buttons.find(
        (b) =>
          b.querySelector('svg') && b.getAttribute('style')?.includes('color:var(--error-color')
      );
    ...
    it('renders and interacts with Breeder Dialog - Delete Confirmation mode', async () => {
      ...
      const deleteBtn = Array.from(overlay?.querySelectorAll('button') || []).find((b) =>
  ...
  describe('View Lineage', () => {
    it('dispatches view-lineage event when View lineage button is clicked', async () => {
      ...
      const viewBtn = Array.from(editorEl.shadowRoot?.querySelectorAll('.sd-btn-text') || []).find(
        (b) => b.textContent?.includes('View lineage')
      );
      expect(viewBtn).toBeTruthy();
  ...
  describe('_handleSave with onSave callback', () => {
    it('calls onSave prop instead of dispatching save-strain', async () => {
      ...
      expect(onSaveMock).toHaveBeenCalledWith(
        expect.objectContaining({ strain: 'Callback Strain' })
      );
  ...
  describe('_handleSave with remote images', () => {
    ...
    it('promotes first image as thumbnail when no thumbnail present after download', async () => {
      ...
      setDraft(editorEl, {
        strain: 'Remote Strain',
        images: [{ path: 'http://example.com/img1.jpg', is_thumbnail: false }],
      });
  ...
  describe('Gallery UI interactions', () => {
    ...
    it('clicking star button on non-thumbnail sets it as thumbnail', async () => {
      const galleryItems = editorEl.shadowRoot?.querySelectorAll(
        '.gallery-drop-area > div[style*="aspect-ratio"]'
      );
    ...
    it('clicking Add button opens the camera-capture menu', async () => {
      ...
      const addBtn = Array.from(
        editorEl.shadowRoot?.querySelectorAll('.gallery-drop-area > button') || []
