...
describe('StrainLibraryDialog - Final Coverage', () => {
    ...
    element.seedBatches = [
      {
        ...
        generation: 'F1',
        parent_1_strain: 'P1',
        parent_1_phenotype: 'Ph1',
        parent_2_strain: 'P2',
        parent_2_phenotype: 'Ph2',
        lineage: '',
        notes: '',
      },
    ];
  ...
  describe('Additional Coverage Edge Cases', () => {
    ...
    it('covers _buildTreeNodes lineage, parents array, and pendingParents loops', () => {
      const strains = [
        ...
        },
        {
          ...
          parents: [{ name: 'Grandmother' }, { name: 'Grandfather' }],
          phenotype: '',
          breeder: '',
          image: '',
        },
    ...
    it('covers clicking on library filter chips in _renderFilterChips', async () => {
      ...
      const activeChip = (Array.from(chips || []) as HTMLElement[]).find(
      ...
      const allChip = (Array.from(chips || []) as HTMLElement[]).find(
        (c) => c.textContent?.trim() === 'All'
      );
