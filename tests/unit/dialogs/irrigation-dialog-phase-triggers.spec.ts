...
async function openOnSteeringTab(element: IrrigationDialog): Promise<void> {
  ...
  const steeringTab = Array.from(tabs ?? []).find((t) => t.textContent?.includes('Steering')) as
    | HTMLElement
    | undefined;
  steeringTab?.click();
  await element.updateComplete;
...
describe('IrrigationDialog – Phase Triggers', () => {
  ...
  it('clicking the active phase card is a no-op', async () => {
    ...
    (element as any)._sm = {
      ...(element as any)._sm,
      tabs: {
        ...(element as any)._sm.tabs,
        steering: { ...(element as any)._sm.tabs.steering, phase: 'p2' },
      },
    };
    await element.updateComplete;
    ...
    const activeCard = Array.from(phaseCards ?? []).find((card: Element) =>
      card.textContent?.includes('Phase · P2')
    ) as HTMLElement;
  ...
  it('clicking an inactive phase card opens the confirmation dialog without updating active phase state immediately', async () => {
    ...
    const sr = await steeringChild(element);
    const phaseCards = sr.querySelectorAll('.phase-card');
    const inactiveCard = Array.from(phaseCards ?? []).find((card: Element) =>
      card.textContent?.includes('Phase · P1')
    ) as HTMLElement;
    ...
    expect(
      (element as any)._sm.tabs.steering.sub.kind === 'confirm-phase'
        ? (element as any)._sm.tabs.steering.sub.pending
        : undefined
    ).toBe('p1');
    expect((element as any)._sm.tabs.steering.phase).toBe('p2');
  });
  ...
  it('calling _cancelPhaseChange closes the dialog and keeps the active phase unchanged', async () => {
    ...
    child.dispatchEvent(
      new CustomEvent('phase-change-cancelled', { bubbles: true, composed: true })
    );
    await element.updateComplete;
  ...
  it('clicking Cancel button in dialog closes the dialog and keeps active phase unchanged', async () => {
    ...
    const cancelBtn = phaseConfirmBtn
      .closest('gs-dialog')!
      .querySelector('button.tonal') as HTMLElement;
  ...
  it('Phase Triggers section has no "Coming soon" badge', async () => {
    ...
    const badges = Array.from(element.shadowRoot?.querySelectorAll('.stub-badge') ?? []).filter(
      (el) => el.textContent?.includes('Coming soon')
    );
