...
describe('SensorTypeUtils', () => {
  ...
  it('should identify light sensors', () => {
    ...
    // Negative cases
    expect(SensorTypeUtils.isLight(device, hassWithHeuristics, 'sensor.humidifier_light')).toBe(
      false
    );
  });
  ...
  it('should correctly get sensor icons for all types', () => {
    ...
    expect(SensorTypeUtils.getSensorIcon(device, hass, 'sensor.co2_mapped')).toBe(
      'mdi:weather-cloudy'
    );
    expect(SensorTypeUtils.getSensorIcon(device, hass, 'sensor.h_mapped')).toBe(
      'mdi:air-humidifier'
    );
    expect(SensorTypeUtils.getSensorIcon(device, hass, 'sensor.dh_mapped')).toBe(
