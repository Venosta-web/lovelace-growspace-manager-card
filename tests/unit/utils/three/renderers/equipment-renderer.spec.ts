...
vi.mock('three', async () => {
  ...
  const mockVector3 = function (this: any, x = 0, y = 0, z = 0) {
    ...
    this.add = vi.fn(function (this: any, v) {
      this.x += v.x;
      this.y += v.y;
      this.z += v.z;
      return this;
    });
    this.sub = vi.fn(function (this: any, v) {
      this.x -= v.x;
      this.y -= v.y;
      this.z -= v.z;
      return this;
    });
    this.multiplyScalar = vi.fn(function (this: any, s) {
      this.x *= s;
      this.y *= s;
      this.z *= s;
      return this;
    ...
    this.applyAxisAngle = vi.fn(function (this: any) {
      return this;
    });
    this.copy = vi.fn(function (this: any, v) {
      this.x = v.x;
      this.y = v.y;
      this.z = v.z;
      return this;
  ...
  const createObject3DNode = function (this: any) {
    ...
    this.traverse = vi.fn((cb) => {
      cb(this);
      children.forEach((c) => c.traverse?.(cb));
    });
  };
  ...
  return {
    ...
    TubeGeometry: vi.fn().mockImplementation(function () {
      return { rotateX: vi.fn(), rotateZ: vi.fn() };
    }),
...
describe('EquipmentRenderer', () => {
  ...
  it('should handle humidifier outside with hose', () => {
    ...
    context.device.environmentAttributes.sensorCoordinates = {
      h_out: { x: -20, y: 50, z: 0, rotation: 90 },
    };
  ...
  it('should animate particles for humidifier, dehum and pumps', () => {
    ...
    expect(
      (renderer as any)._humidifierParticles.geometry.attributes.position.needsUpdate
    ).toBeTruthy();
    expect(
      (renderer as any)._dryAirParticles.geometry.attributes.position.needsUpdate
    ).toBeTruthy();
    expect(
      (renderer as any)._pumpWaterParticles.geometry.attributes.position.needsUpdate
    ).toBeTruthy();
  });
  ...
  it('should handle wind particles with exhaust fans (suction and blow)', () => {
    context.device.environmentAttributes.exhaustFanEntities = ['ex1'];
    context.device.environmentAttributes.sensorCoordinates = {
      ex1: { x: 50, y: 50, z: 100, rotation: 45 },
    };
    context.hass.states['ex1'] = { state: 'on' };
  ...
  it('should hit all pump state logic branches', () => {
    context.device.irrigationConfig = { irrigationPumpEntity: 'p1', drainPumpEntity: 'p2' };
    context.device.environmentAttributes.sensorTypes = {
      p3: 'irrigation_pump',
      p4: 'irrigation_pump',
    };
    context.device.environmentAttributes.sensorCoordinates = {
  ...
  it('should hit remaining create and update branches for dehum and pumps', () => {
    ...
    context.device.environmentAttributes.sensorCoordinates['p_rot'] = {
      x: -10,
      y: 10,
      rotation: 180,
    };
  ...
  it('should hit edge cases for rotation and defaults', () => {
    ...
    context.device.environmentAttributes.exhaustFanEntities = ['ex1'];
    context.device.environmentAttributes.sensorCoordinates['ex1'] = {
      x: 50,
      y: 50,
      z: 100,
      rotation: 0,
  ...
  it('should hit exhaust fan filter or branches', () => {
    ...
    renderer.render();
    const fan2 = (renderer as any)._exhaustFans.find(
      (f: any) => f.userData.entityId === 'exhaust_fan_2'
    );
