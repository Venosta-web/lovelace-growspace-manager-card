...
vi.mock('three', async () => {
  ...
  return {
    ...
    Vector2: vi.fn().mockImplementation(function (this: any, x = 0, y = 0) {
      return { x, y };
    }),
    // Other needed mocks
  };
...
describe('InteractionManager', () => {
  ...
  it('should handle mouse move and detect hover', () => {
    ...
    expect(callback).toHaveBeenCalledWith(
      'hover',
      expect.objectContaining({ plant: { id: 'p1' } })
    );
  ...
  it('should handle drag events', () => {
    ...
    // dragstart
    const dragStartHandler = (dragControls.addEventListener as any).mock.calls.find(
      (c: any) => c[0] === 'dragstart'
    )[1];
    dragStartHandler();
    ...
    // drag
    const dragHandler = (dragControls.addEventListener as any).mock.calls.find(
      (c: any) => c[0] === 'drag'
    )[1];
    ...
    // dragend
    const dragEndHandler = (dragControls.addEventListener as any).mock.calls.find(
      (c: any) => c[0] === 'dragend'
    )[1];
  ...
  it('should handle edge cases in handleMouseMove', () => {
    ...
    expect(callback).toHaveBeenCalledWith(
      'hover',
      expect.objectContaining({ plant: { id: 'slot1' } })
    );
  });
