...
describe('ChartUtils', () => {
  describe('generateSparklinePath', () => {
    it('should return empty string for insufficient data', () => {
      ...
      expect(
        ChartUtils.generateSparklinePath([{ state: '1', last_changed: '2023-01-01' }], 100, 50)
      ).toBe('');
    });
  ...
  describe('getSparklineColor', () => {
    it('should return VPD status colors', () => {
      ...
      expect(ChartUtils.getSparklineColor('vpd', 'warning')).toBe(
  ...
  describe('generateVpdSparklineSegments', () => {
    ...
    it('should handle raw HA history and internal normalization', () => {
      ...
      const rawLight = [{ state: 'on', last_changed: '2023-01-01T09:00:00Z' }];
      const segments = ChartUtils.generateVpdSparklineSegments(
        rawData,
        100,
        50,
        thresholds,
  ...
  describe('generatePathFromValues', () => {
    ...
    it('should cull redundant points based on EPSILON', () => {
      ...
      const path = ChartUtils.generatePathFromValues(culledData, 100, 100, {
        min: 0,
        max: 100,
        startTime: 0,
        endTime: 100,
    ...
    it('should handle step path points culled by EPSILON', () => {
      ...
      const path = ChartUtils.generatePathFromValues(culledData, 0.05, 0.05, {
        type: 'step',
        min: 0,
        max: 100,
        startTime: 0,
    ...
    it('should handle partial options overrides for min/max/time', () => {
      ...
      const path2 = ChartUtils.generatePathFromValues(data, 100, 100, {
        startTime: 0,
        endTime: 4000,
      });
  ...
  describe('generateStepPath', () => {
    ...
    it('should return empty for empty/invalid data', () => {
      ...
      expect(
        ChartUtils.generateStepPath(
          [{ state: 'on', last_changed: '2023-01-01' }, { state: 'unavailable' }],
          100,
          50
        )
      ).toBe('');
    ...
    describe('Coverage Gap Fillers', () => {
      ...
      describe('generatePathFromValues Time Ranges', () => {
        ...
        it('should fallback to default for unknown range', () => {
          const path = ChartUtils.generatePathFromValues(data, 100, 50, {
            timeRange: 'foo' as any,
          });
          // No downsampling: 361 points
          expect(path.split(' L ').length).toBe(361);
        });
  ...
  describe('Edge Case Coverage', () => {
    ...
    it('should return empty array if filtering results in < 2 points in generateVpdSparklineSegments', () => {
      const thresholds = {
        day: { targetMin: 0, targetMax: 10, dangerMin: 0, dangerMax: 10 },
        night: { targetMin: 0, targetMax: 10, dangerMin: 0, dangerMax: 10 },
      };
      const data = [
        { state: '1.0', last_changed: '2023-01-01T10:00:00Z' },
        { state: 'unavailable', last_changed: '2023-01-01T11:00:00Z' },
    ...
    it('should handle non-metric parsing failure in normalizeHistory', () => {
      const data = [{ state: 'invalid_float', last_changed: '2023-01-01T10:00:00Z' }];
