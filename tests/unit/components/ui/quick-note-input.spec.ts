...
describe('QuickNoteInput', () => {
  ...
  it('should render with custom placeholder', async () => {
    element = await fixture(
      html`<quick-note-input placeholder="Custom placeholder"></quick-note-input>`
    );
    await element.updateComplete;
    const textarea = element.shadowRoot?.querySelector('textarea');
  ...
  it('should add compressed images when camera-capture emits files', async () => {
    ...
    await (element as any)._handleCapture(
      new CustomEvent('capture', { detail: { files: [file] } })
    );
    await element.updateComplete;
  ...
  it('should hide camera button when allowImages is false', async () => {
    ...
    const cameraBtn = Array.from(element.shadowRoot?.querySelectorAll('button') || []).find(
