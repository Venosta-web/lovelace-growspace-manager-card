...
describe('GrowspaceLogbook', () => {
  ...
  it('should filter events', async () => {
    ...
    const irrigationEvents = Array.from(cards || []).filter((c) =>
      c.textContent?.includes('irrigation')
    );
  ...
  describe('Private Helpers Coverage', () => {
    ...
    it('should _getSeverityColor for optimal sensors', () => {
      ...
      expect((element as any)._getSeverityColor(0.95, 'optimal')).toBe(
        'var(--success-color, #4CAF50)'
      );
      expect((element as any)._getSeverityColor(0.8, 'optimal')).toBe('var(--warning-color)');
      expect((element as any)._getSeverityColor(0.5, 'optimal')).toBe('var(--error-color)');
    });
    ...
    it('should _getSeverityColor for default (alert) sensors', () => {
      ...
      expect((element as any)._getSeverityColor(0.8, 'temp')).toBe('var(--warning-color)');
      expect((element as any)._getSeverityColor(0.5, 'temp')).toBe('var(--primary-text-color)');
    });
  });
  ...
  describe('Coverage Improvements', () => {
    it('should handle rendering fallbacks for title', async () => {
      ...
      const types = Array.from(cards || []).map((c) =>
        c.querySelector('.event-type')?.textContent?.trim()
      );
