...
vi.mock('three', async () => {
  ...
  const createTraverseFn = () =>
    vi.fn(function (this: any, callback: (obj: any) => void) {
      callback(this);
      if (this.children) {
  ...
  const createGetObjectByNameFn = () =>
    vi.fn(function (this: any, name: string): any {
  ...
  const createMockCamera = () => ({
    position: {
      set: vi.fn(function (this: any, x: any, y: any, z: any) {
        this.x = x;
        this.y = y;
        this.z = z;
        return this;
  ...
  return {
    ...
    Group: vi.fn().mockImplementation(function () {
      return { ...createMockGroup(), rotateX: vi.fn(), rotateY: vi.fn(), rotateZ: vi.fn() };
    }),
    Mesh: vi.fn().mockImplementation(function () {
      return { ...createMockMesh(), rotateX: vi.fn(), rotateY: vi.fn(), rotateZ: vi.fn() };
    }),
    Vector3: vi.fn().mockImplementation(function (x = 0, y = 0, z = 0) {
      const v = {
        ...
        y,
        z,
        set: vi.fn(function (this: any, nx, ny, nz) {
          this.x = nx;
          this.y = ny;
          this.z = nz;
          return this;
        }),
        sub: vi.fn(function (this: any, v) {
          this.x -= v.x;
          this.y -= v.y;
          this.z -= v.z;
          return this;
        ...
        add: vi.fn(function (this: any, v) {
          this.x += v.x;
          this.y += v.y;
          this.z += v.z;
          return this;
        }),
        multiplyScalar: vi.fn(function (this: any, s) {
          this.x *= s;
          this.y *= s;
          this.z *= s;
          return this;
        }),
        copy: vi.fn(function (this: any, v) {
          this.x = v.x;
          this.y = v.y;
          this.z = v.z;
          return this;
        ...
        clone: vi.fn(function (this: any) {
          return new (vi.mocked(THREE.Vector3))(this.x, this.y, this.z);
        }),
    ...
    Vector2: vi.fn().mockImplementation(function (x = 0, y = 0) {
    ...
    CylinderGeometry: vi.fn().mockImplementation(function () {
      return { dispose: createDisposeFn(), rotateZ: vi.fn(), rotateX: vi.fn() };
    }),
    PlaneGeometry: vi.fn().mockImplementation(function () {
      return { dispose: createDisposeFn(), rotateZ: vi.fn(), rotateX: vi.fn() };
    }),
    CircleGeometry: vi.fn().mockImplementation(function () {
      return { dispose: createDisposeFn(), rotateZ: vi.fn(), rotateX: vi.fn() };
    }),
    SphereGeometry: vi.fn().mockImplementation(function () {
      return { dispose: createDisposeFn(), rotateZ: vi.fn(), rotateX: vi.fn() };
    }),
    LatheGeometry: vi.fn().mockImplementation(function () {
      return { dispose: createDisposeFn() };
    }),
    ...
    MeshStandardMaterial: vi.fn().mockImplementation(function () {
      return { dispose: createDisposeFn(), color: { set: vi.fn() } };
    }),
    MeshBasicMaterial: vi.fn().mockImplementation(function () {
      return { dispose: createDisposeFn(), color: { set: vi.fn() } };
    }),
    ShaderMaterial: vi.fn().mockImplementation(function () {
      return { uniforms: {}, dispose: createDisposeFn(), color: { set: vi.fn() } };
    }),
...
describe('Heatmap3D Logic', () => {
  ...
  describe('UI and Playback Logic', () => {
    ...
    it('should format time correctly', () => {
      (element as any).historyData = {
        'sensor.temp1': [{ lu: '2026-01-24T10:00:00Z', s: '20' }],
      };
  ...
  describe('UI Rendering and Interactions', () => {
    it('should toggle visibility flags on checkbox click', async () => {
      const plantToggle = element.shadowRoot?.querySelector(
        '.toggles-container .toggle-item:nth-child(1)'
      ) as HTMLElement;
      plantToggle.click();
      expect(element['showPlants']).toBe(false);

      const lightToggle = element.shadowRoot?.querySelector(
        '.toggles-container .toggle-item:nth-child(2)'
      ) as HTMLElement;
      lightToggle.click();
      expect(element['showLights']).toBe(false);

      const fanToggle = element.shadowRoot?.querySelector(
        '.toggles-container .toggle-item:nth-child(3)'
      ) as HTMLElement;
      fanToggle.click();
      expect(element['showFans']).toBe(false);

      const heatmapToggle = element.shadowRoot?.querySelector(
        '.toggles-container .toggle-item:nth-child(4)'
      ) as HTMLElement;
      heatmapToggle.click();
      expect(element['showHeatmap']).toBe(false);

      const tooltipsToggle = element.shadowRoot?.querySelector(
        '.toggles-container .toggle-item:nth-child(5)'
      ) as HTMLElement;
    ...
    it('should update selected metric on button click', async () => {
      const tempButton = element.shadowRoot?.querySelector(
        '.metric-selector button:nth-child(1)'
      ) as HTMLElement;
      tempButton.click();
      expect(element['selectedMetric']).toBe('temperature');

      const hButton = element.shadowRoot?.querySelector(
        '.metric-selector button:nth-child(2)'
      ) as HTMLElement;
      hButton.click();
      expect(element['selectedMetric']).toBe('humidity');

      const vButton = element.shadowRoot?.querySelector(
        '.metric-selector button:nth-child(3)'
      ) as HTMLElement;
      vButton.click();
  ...
  describe('Keyboard and Resize', () => {
    it('should handle resize', () => {
      ...
      Object.defineProperty(element.shadowRoot?.querySelector('#container'), 'clientWidth', {
        value: 800,
      });
      Object.defineProperty(element.shadowRoot?.querySelector('#container'), 'clientHeight', {
        value: 600,
      });
  ...
  describe('Rendering Toggles and Helpers', () => {
    ...
    it('should update scene when hass states change', async () => {
      ...
      element.hass = {
        ...element.hass,
        states: { ...element.hass.states, 'sensor.temp1': { state: '26.0' } },
      };
      await elementUpdated(element);
      expect(spy).toHaveBeenCalled();
    });
    ...
    it('should fetch history with sensor groups', async () => {
      element.device = {
        ...
        environmentAttributes: {
          sensorGroups: [
            { temperature_sensors: ['s.t1'], humidity_sensors: ['s.h1'], vpd_sensors: ['s.v1'] },
          ],
        },
      };
      const { getHistoryStats } = await import('../../../src/store/history/history-store');
      await (element as any).fetchHistory();
      expect(getHistoryStats).toHaveBeenCalledWith(
        expect.arrayContaining(['s.t1', 's.h1', 's.v1']),
        expect.any(Date)
      );
    });
  ...
  describe('Tooltip', () => {
    beforeEach(() => {
      element.strainLibrary = [
        {
          key: 'sour-diesel-pheno-1',
          strain: 'Sour Diesel',
          phenotype: 'Pheno 1',
          breeder: 'FastBuds',
        },
        {
          key: 'sour-diesel-default',
          strain: 'Sour Diesel',
          phenotype: 'default',
          breeder: 'Unknown',
        },
