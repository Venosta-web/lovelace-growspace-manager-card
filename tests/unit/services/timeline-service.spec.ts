...
describe('TimelineService', () => {
  ...
  describe('fetchPlantEvents', () => {
    it('should fetch plant events successfully', async () => {
      const mockLogs = [{ event_id: '1', timestamp: '2026-01-15T10:00:00Z', category: 'note' }];
      const mockAlerts = [{ event_id: '2', timestamp: '2026-01-15T11:00:00Z', category: 'alert' }];
    ...
    it('should use custom limit when provided', async () => {
      ...
      expect(mockHass.callWS).toHaveBeenCalledWith(expect.objectContaining({ limit: 100 }));
    });
    ...
    it('should sort combined events by timestamp descending', async () => {
      ...
      const alerts = [{ event_id: 'b', timestamp: '2026-01-15T10:00:00Z', category: 'alert' }];
    ...
    it('should rethrow errors', async () => {
      ...
      await expect(service.fetchPlantEvents('plant_123', 'gs_abc')).rejects.toThrow(
        'WS plant error'
      );
