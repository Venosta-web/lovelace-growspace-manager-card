...
test('growspace-ai-insight-card visual snapshot', async () => {
  const element = await fixture<GrowspaceAiInsightCard>(
    html`<growspace-ai-insight-card></growspace-ai-insight-card>`
  );
  ...
  (element as any)._response =
    'Gorilla Glue #4 is in week 2 of flower and looks healthy. Consider raising EC to 1.8 and maintaining 12/12 light. VPD is optimal at 1.2 kPa. Blue Dream in veg is on track for a 5-week cycle.';
