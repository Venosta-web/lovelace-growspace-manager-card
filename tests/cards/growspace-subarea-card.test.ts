...
import {
  ...
  deriveSubareaMetricEntities,
} from '../../src/cards/growspace-subarea-card';
...
describe('GrowspaceSubareaCard', () => {
  ...
  test('renders unconfigured state when default_growspace is missing', async () => {
    ...
    await element.updateComplete;
    expect(element.shadowRoot?.querySelector('.no-data')?.textContent).toContain(
      'Please configure'
    );
  });
  ...
  test('renders no-sensors message when environment_config is empty', async () => {
    ...
    await (element as any)._loadSubarea();
    await element.updateComplete;
    expect(element.shadowRoot?.querySelector('.no-sensors')?.textContent).toContain(
      'No environment sensors'
    );
  });
  ...
  test('renders VPD and CO2 sensors', async () => {
    ...
    mockHass.states['sensor.vpd'] = {
      state: '0.8',
      attributes: { friendly_name: 'VPD', unit_of_measurement: 'kPa' },
    };
    mockHass.states['sensor.co2'] = {
      state: '800',
  ...
  test('renders multiple entity IDs in hero card (multi-sensor path)', async () => {
    ...
    mockHass.states['sensor.temp1'] = {
      state: '22.0',
      attributes: { friendly_name: 'Temp 1', unit_of_measurement: '°C' },
    };
    mockHass.states['sensor.temp2'] = {
      state: '23.0',
      attributes: { friendly_name: 'Temp 2', unit_of_measurement: '°C' },
    };
    element.hass = mockHass;
    await (element as any)._loadSubarea();
    await element.updateComplete;
  ...
  test('renders additional sensors (substrate temp, pH, feed EC, bulk EC, pore EC)', async () => {
    ...
    mockHass.states['sensor.substrate_temp'] = {
      state: '21.0',
      attributes: { unit_of_measurement: '°C' },
    };
    mockHass.states['sensor.ph'] = { state: '6.2', attributes: { unit_of_measurement: 'pH' } };
    mockHass.states['sensor.feed_ec'] = {
      state: '1.8',
      attributes: { unit_of_measurement: 'mS/cm' },
    };
    mockHass.states['sensor.bulk_ec'] = {
      state: '2.1',
      attributes: { unit_of_measurement: 'mS/cm' },
    };
    mockHass.states['sensor.pore_ec'] = {
      state: '2.3',
      attributes: { unit_of_measurement: 'mS/cm' },
    };
    element.hass = mockHass;
  ...
  test('renders no-data state when subarea is null after loading', async () => {
    ...
    (element as any)._subarea = null;
    await element.updateComplete;
    expect(element.shadowRoot?.querySelector('.no-data')?.textContent?.trim()).toBe(
      'Subarea not found.'
    );
  });
  ...
  test('renders legacy single sensor fields (temperature_sensor, humidity_sensor, vpd_sensor)', async () => {
    ...
    mockHass.states['sensor.vpd_legacy'] = {
      state: '0.9',
  ...
  test.each([
    ...
    '_calculateHistoryStart returns correct start date for %s range',
    (range, expectedOffsetMs) => {
  ...
  test('desktop secondary chips fire toggle-graph on growspace-header-secondary-ui', async () => {
    ...
    const toggleSpy = vi.mocked(uiSlice.toggleEnvGraph);
    const secondaryUI = element.shadowRoot?.querySelector(
      'growspace-header-secondary-ui'
    ) as HTMLElement;
    expect(secondaryUI).not.toBeNull();
    secondaryUI.dispatchEvent(
      new CustomEvent('toggle-graph', {
        detail: { metric: 'substrate_temperature' },
        bubbles: true,
        composed: true,
      })
    );
  ...
  test('resolves multiple calculated VPD fallback sensors when multiple T/H pairs are configured', async () => {
    ...
    mockHass.states['sensor.veg_temp_1'] = {
      state: '22.0',
      attributes: { unit_of_measurement: '°C' },
    };
    mockHass.states['sensor.veg_temp_2'] = {
      state: '24.0',
      attributes: { unit_of_measurement: '°C' },
    };
    mockHass.states['sensor.veg_humidity_1'] = {
      state: '50',
      attributes: { unit_of_measurement: '%' },
    };
    mockHass.states['sensor.veg_humidity_2'] = {
      state: '60',
      attributes: { unit_of_measurement: '%' },
    };
