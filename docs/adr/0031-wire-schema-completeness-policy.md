...
3. **Inbound: strip** (plain `z.object`). Combined with (2), a field the card
   ...
   `.optional()` so the card stays [[Backward-Safe|backward-safe]] against older
...
4. **Outbound: strict** (`z.strictObject`). Request payloads are authored by the
   ...
   it: the fixture is a payload GSM *emits*. This is the one place strictness
   belongs in production. `UpdatePlantPayloadSchema`'s `.passthrough()` inverts.
...
| Latest GSM **release** | **Backward-safety** — the schema's non-optional keys ⊆ fixture keys | The card requires a field the released backend does not send |
...
The diff is also a better failure report than strict parse: it names which keys
drifted in which object, rather than emitting a zod error dump. Everything else
in ADR 0029 stands — two refs, one test, joined to the required contexts, and
the release-ref result remains the mechanical definition of a
