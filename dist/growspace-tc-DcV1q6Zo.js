...
let GrowspaceTcPairings = class GrowspaceTcPairings extends i$1 {
      ...
      <fieldset ?disabled=${this._saving}>
        ...
        <label
          ...
          >
            ...
            ${this._media.map((m) => x `<option value=${m.id} ?selected=${m.id === draft.medium_id}>${m.name}</option>`)}
          </select></label
        >
        <label
    ...
    render() {
        return x `
      ${this._loading ? x `<p role="status">${this._t('pairing_loading')}</p>` : E}
      ${this._loadError
    ...
    _renderPairings() {
      ...
      ${!this._pairings.length ? x `<p>${this._t('pairing_empty')}</p>` : E}
      ${[...groups.values()]
...
//# sourceMappingURL=growspace-tc-DcV1q6Zo.js.map
