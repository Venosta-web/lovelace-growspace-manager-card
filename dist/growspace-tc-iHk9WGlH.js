/*! growspace-e2e-build source=10260ac10de09fa31f9a63709c05046f5bdc77e287a569f499e15cd18b010d99 id=24392fbaf0ab20ddb6ad53544fef33ef */
const { cF: variables, bK: sharedStyles, i, _: __decorate, n, A: r, t, g: i$1, bQ: localize, eg: draftReplate, bR: localizeWithParams, x, E, eh: draftIntroduction, ei: devices$, ej: cultureLines$, d5: strainLibrary$, ek: cultureMedia$, el: fetchCultureLines, W: fetchStrainLibrary, em: phenotypeOptions, en: phenotypeNameIndex, eo: resolvePhenotype, ep: worklistEntries, eq: fetchMaintenanceHistory, er: recordMaintenance, es: introduceCultureLine, et: relinkPhenotype, eu: setCultureLineArchived, ev: locationOptions, dE: portalVariables, ew: pairings$, ex: fetchPairings, ey: savePairing, ez: deletePairing, eA: draftFromMedium, eB: TC_FEATURE_CULTURE_MEDIA, eC: TC_FEATURE_CULTURE_LINES, eD: TC_FEATURE_MAINTENANCE, eE: fetchCultureMedia, eF: updateCultureMedium, eG: createCultureMedium, eH: deleteCultureMedium, eI: TC_FEATURE_PAIRINGS } = await import(window.__growspaceEntryUrl ?? './growspace-manager-card.js');
...
let GrowspaceTcCultures = GrowspaceTcCultures_1 = class GrowspaceTcCultures extends i$1 {
    ...
    _renderActionDialog(acting) {
      ...
      .growspaces=${this._devices.filter((device) => device.type === 'normal' || device.type === 'clone')}
...
let GrowspaceTcPairingEditor = class GrowspaceTcPairingEditor extends i$1 {
    constructor() {
        super(...arguments);
        this.pairings = [];
        this.media = [];
    ...
    finishEditing() {
        ...
        void this.updateComplete.then(() => {
            const target = this._returnFocus?.isConnected
                ? this._returnFocus
                : this.shadowRoot?.querySelector('.toolbar button');
            target?.focus();
        });
    }
    _edit(pairing, event) {
        if (event?.currentTarget instanceof HTMLElement)
            this._returnFocus = event.currentTarget;
        this._editingId = pairing?.id;
        this._draft = pairing
            ? {
                phenotype_id: pairing.phenotype.id,
                phenotype_name: this.phenotypes.find((p) => p.id === pairing.phenotype.id)?.name ??
    ...
    _request(name, detail) {
        this.dispatchEvent(new CustomEvent(name, { detail, bubbles: true, composed: true }));
    }
    _renderForm(draft) {
        const duplicate = this.pairings.some((row) => row.id !== this._editingId &&
            row.medium_id === draft.medium_id &&
            row.phenotype.id === draft.phenotype_id);
        const valid = draft.phenotype_id && this.media.some((m) => m.id === draft.medium_id) && !duplicate;
        return x `<form
      @submit=${(event) => {
            event.preventDefault();
      ...
      <fieldset ?disabled=${this.saving}>
        ...
        <label
          >${this._t('pairing_medium')}<select
            ...
            @change=${(event) => (this._draft = { ...draft, medium_id: event.target.value })}
          >
            <option value="">${this._t('pairing_choose_medium')}</option>
            ${this.media.map((m) => x `<option value=${m.id} ?selected=${m.id === draft.medium_id}>${m.name}</option>`)}
          </select></label
        >
        <label
    ...
    render() {
      ...
      <div class="toolbar">
        ...
        <button
          ...
          @click=${(event) => this._edit(undefined, event)}
        >
          ${this._t('pairing_add')}
        </button>
      </div>
      ...
      ${!this.pairings.length ? x `<p>${this._t('pairing_empty')}</p>` : E}
      ${[...groups.values()]
            .sort((a, b) => a.name.localeCompare(b.name))
            .map((group) => x `<h4>${group.name}</h4>
              <ul>
                ${group.rows.map((row) => {
            const phenotype = resolvePhenotype(row.phenotype, names, this.libraryLoaded);
            return x `<li data-pairing-id=${row.id}>
                    <strong
                      >${this._by === 'medium'
                ? phenotype.name
                ...
                ? x `<p class="error">${this._t('pairing_missing_phenotype')}</p>`
                : E}
                    ${row.notes ? x `<p class="notes">${row.notes}</p>` : E}
                    <div class="actions">
                      <button
                        ?disabled=${this.saving}
                        @click=${(event) => this._edit(row, event)}
                      >
                ...
                ? x `<span>${this._t('pairing_remove_confirm')}</span
                            ><button
                              ?disabled=${this.saving}
                              @click=${() => this._request('pairing-delete-requested', { id: row.id })}
                            >
...
GrowspaceTcPairingEditor.styles = [
    portalVariables,
    sharedStyles,
    i `
      :host {
...
__decorate([
    n({ attribute: false })
], GrowspaceTcPairingEditor.prototype, "pairings", void 0);
__decorate([
    n({ attribute: false })
], GrowspaceTcPairingEditor.prototype, "media", void 0);
__decorate([
    n({ attribute: false })
], GrowspaceTcPairingEditor.prototype, "phenotypes", void 0);
__decorate([
    n({ type: Boolean })
], GrowspaceTcPairingEditor.prototype, "libraryLoaded", void 0);
__decorate([
    n({ type: Boolean })
], GrowspaceTcPairingEditor.prototype, "saving", void 0);
__decorate([
    n({ type: String })
], GrowspaceTcPairingEditor.prototype, "error", void 0);
__decorate([
    n({ type: String })
], GrowspaceTcPairingEditor.prototype, "language", void 0);
__decorate([
    r()
], GrowspaceTcPairingEditor.prototype, "_by", void 0);
__decorate([
    r()
], GrowspaceTcPairingEditor.prototype, "_draft", void 0);
__decorate([
    r()
], GrowspaceTcPairingEditor.prototype, "_editingId", void 0);
__decorate([
    r()
], GrowspaceTcPairingEditor.prototype, "_removing", void 0);
GrowspaceTcPairingEditor = __decorate([
    t('growspace-tc-pairing-editor')
], GrowspaceTcPairingEditor);
...
let GrowspaceTcPairings = class GrowspaceTcPairings extends i$1 {
    ...
    render() {
        return x ` ${this._loading
            ? x `<p role="status">${localize('tc.pairing_loading', '', '', this.language)}</p>`
            : E}
      ${this._loadError
            ? x `<p role="alert">${this._loadError}</p>
      ...
      <growspace-tc-pairing-editor
        ...
        .error=${this._error}
        .language=${this.language}
        @pairing-save-requested=${(event) => this._mutate(() => savePairing(event.detail.draft, event.detail.id))}
        @pairing-delete-requested=${(event) => this._mutate(() => deletePairing(event.detail.id))}
      ></growspace-tc-pairing-editor>`;
    }
};
__decorate([
...
let GrowspaceTcView = GrowspaceTcView_1 = class GrowspaceTcView extends i$1 {
    ...
    render() {
      ...
      <div role="region" aria-label=${this._t('view_title')}>
        ...
        ${this.manifest?.features.includes(TC_FEATURE_PAIRINGS)
            ? x `<growspace-tc-pairings .language=${this.language}></growspace-tc-pairings>`
            : E}
      </div>
    `;
    }
...
//# sourceMappingURL=growspace-tc-iHk9WGlH.js.map
