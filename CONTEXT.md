...
**Config Dialog Shell**
The Config Dialog's [[Dialog Shell]] (the reduced `config-dialog.ts` host after decomposition): owns the `ConfigDialogSM` atom, the nav rail (13 tabs), the [[MutationRunController]], the tab-switch dirty guard, and the Save orchestration. On Save it calls the pure [[Environment Save Composer]] and sequences the one-or-two backend calls. Like the Irrigation [[Dialog Shell]], during migration it renders extracted `<config-x-tab .vm=…>` components alongside still-inline `_renderXSection()` methods, tab-by-tab, both forms coexisting.
...
The pure function (`composeEnvironmentConfig(draft) → EnvPayload`, plus `needsExhaustCall(draft) → boolean`) that turns the [[Shared Environment Draft]] into the `configure_environment` payload. It exists because `configure_environment` is a **full replace** — it rebuilds `EnvironmentConfig` and silently resets any field not present in the payload (see also the integration's own note that `configure_environment` is a full replace). Composing the *whole* seeded draft on every env-tab save re-sends a complete config, so a sensor-only edit no longer clobbers fan/irrigation fields. `needsExhaustCall` gates the **second** service call, `configure_exhaust_fan`, which `configure_environment` cannot carry (see [[Exhaust Fan Controller Panel]]). The composer is pure (the deletion-test of the full-replace bug class becomes a unit assertion); the [[Config Dialog Shell]] owns the one-or-two-call orchestration via [[MutationRunController]], mirroring `mergeTankDraft` + ADR-0015. Modelled on the Irrigation Tanks tab's `mergeTankDraft`.
