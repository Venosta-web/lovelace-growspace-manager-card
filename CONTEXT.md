...
A sub-component (`<stage-vpd-overrides-table>`) rendered inside the [[Fan Controller Panel]] only when `stage_vpd_enabled` is true. Displays one row per [[Fan VPD Stage]] (9 rows total), each with a Day and Night number input (min 0.1, max 3.0, step 0.01). Inputs are pre-populated from `stage_vpd_overrides` when an override exists, or from the local `FAN_VPD_STAGE_DEFAULTS` const otherwise. Every input change writes the full `{ day, night }` pair for that stage into the local overrides draft (sparse — only edited stages are present). A "Reset all to defaults" button clears the entire overrides draft and re-renders from defaults; it does not persist until the user saves the dialog. Toggling Stage-Aware VPD off hides the table but preserves the draft — re-enabling it restores the edited state. Emits a single `overrides-change` custom event carrying the new sparse dict.
