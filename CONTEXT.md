...
Pure expand/collapse/accordion view state — the Climate tab's Temperature-Override and Critical-Temperature expanders (`_fanTempOverrideExpanded` / `_exhaustCriticalTempExpanded`), and the still-inline humidity/VPD accordions (`_openHumidityStageId` / `_openVpdStageId`) — lives as **[[Config Dialog Shell]] `@state`**, *not* in the SM. The dumb [[Tab Component]] emits a toggle [[Tab Intent]] (`toggle-fan-temp-override`, `toggle-exhaust-critical-temp`); the Shell flips its own `@state` and the flag is projected into the [[Tab ViewModel]]. This satisfies the no-`@state` component contract while keeping ephemeral view fluff out of the persistent SM. The ADR-0019 "draft/edit state → SM" rule targets *draft* state (the inline-edit-after-CANCEL bug class); expanders have no draft semantics, so they are deliberately exempt. Sets the rule for the humidity/VPD tabs (#4a-d) which carry the same accordions.
