...
**Env Graph**
An inline historical chart that appears when a Hero Card or Chip is clicked, showing sensor data over time for that metric. Toggled via `_toggleEnvGraph` / `_toggleMetricGraph`. Its render input is [[Env Series]] (value space); `<growspace-env-chart>` owns the geometry and nothing else — it derives no series of its own and reads no `hass.states`, taking the [[Metric Descriptor]] table as a property from `growspace-analytics.container` (#472). [[Guide Mark]]s are drawn from that same table's [[Metric Target]]s, so a graph never reads a target off the device itself. With no history in the window it renders **one** message, in the empty pane, naming the metric and the range — the header still names the graph so it can be closed, but the reading slot beside it stays empty rather than repeating "No Data" over the same fact (#868).

**EnvSnapshot**
A normalized point-in-time view of a _growspace's_ sensor values (one snapshot per growspace — not per subarea), exposed as an atom by the Environment slice. The single place that reads `hass.states` for environmental sensors. Covers the full sensor set from the config dialog: air metrics (temperature, humidity, VPD, CO₂, DLI, optimal conditions), substrate/medium metrics (soil moisture, substrate temperature), and irrigation monitoring metrics (pH, feed EC, bulk EC, pore EC, runoff EC, drain volume, irrigation flow, power, energy). Multi-sensor fields carry a `SensorReadings` object (`avg`, `perSensor`, `entityIds`) to support the "Multiple + per-sensor" chip display pattern. Cards and derived modules subscribe to it instead of reaching into `hass`.

**HeaderMetrics module**
Pure derived module that, given a `(growspaceId, viewContext)`, returns `{ hero, chips }` for rendering. `viewContext` is `'main' | 'subarea' | 'analytics'` — the main and subarea cards get both hero and chips; the analytics card gets chips only. Inputs are `EnvSnapshot`, plant state, irrigation state, and the device-state slice's `DeviceSnapshot` — all atom-sourced; per its own documented constraint it never touches `hass`/`hass.states`. It fully replaced the legacy `MetricsUtils` (`metrics-utils.ts`, deleted in #269): hero, secondary, and device chips (exhaust, fan, humidifier, dehumidifier, lights, and — main view only, read off the irrigation config — the irrigation and drain pumps, #1006) for both the main and subarea views all come from `computeHeaderMetrics()`, with subareas computed over their subarea-scoped env/device snapshots (#266/#267). `HeaderChip` and `DominantStageInfo` are defined here — this module is their only source.

**Metric Descriptor**
The per-`MetricKey` record of everything both a [[Chip]] and an [[Env Graph]] must know about a metric: its backing sensor entities as structured `{ entityId, name }` pairs (not `'metric:entityId'` strings), display unit, colour, title, chart type (line vs step), axis scale (fixed bounds or auto), and its [[Metric Target]]s — the normalized guide-mark records that **absorb** what was a raw, VPD-only day/night threshold field (ADR-0050). Derived by `computeMetricDescriptors()` in `slices/metric-descriptors/`. It covers **every metric in `METRIC_CONFIG`** — a closed set, and the same set `history-store` fetches within, so an absent key means "not a metric", never "not migrated". Like the [[HeaderMetrics module]] it takes **explicit data parameters and reads no atoms**. Unlike HeaderMetrics it is not atom-_only_: it takes its inputs the way `computeEnvSnapshot(device, hassStates)` and the [[Hydration orchestrator]] do — **pure of `hass` injection, with a `hass.states` snapshot passed as an argument**. That difference is forced, not stylistic: HeaderMetrics is atom-only because everything it needs already sits on the snapshots, whereas three of the descriptor's inputs sit on no atom today — the day/night VPD threshold table (`EnvSnapshot` carries only the resolved `vpdStatus`; `day_vpd_target_*` / `night_*` are read off `overviewEntityId`), the light sensor's `unit_of_measurement` (`DeviceSnapshot` holds a _display_ string like `"70%"`, not the unit), and the per-sensor `friendly_name` used for multi-sensor series titles. What _is_ already atom-covered: `DeviceEntry.entityIds` supplies the fan/light/exhaust/humidifier entities, and [[Fan Entity Mode]]'s type facet is id-derived, so fan axis scale and the `%` unit need no state lookup at all. Note also that the available inputs differ by `viewContext` — the growspace adapter leaves the four hero metrics' `SensorReadings` null, while the subarea adapter populates them. It is the **single owner of metric→entity resolution**, subsuming `history-store`'s `METRIC_ENTITY_KEYS` / `getEntityIdsForMetric` (which now delegates to `resolveMetricEntityIds`), so the multi-sensor grouping is carried structurally rather than flattened into a `':'`-joined key that every consumer re-parses. (No conflict with ADR-0005: that ADR keeps _transport_ — `getHistory` / `getBatchHistory` / `getHistoryStats` — in `history-store`; metric→entity mapping is not transport.) Three consumers make this a real seam rather than a hypothetical one: `history-store` (which entities to fetch), the [[HeaderMetrics module]] (chip and hero display), and [[Env Series]] (graph shaping) — one table decides unit, colour and axis, so a chip and its own graph cannot disagree.
_Resolves an ambiguity:_ VPD status was derived twice — the Environment slice's `_resolveVpdStatus` (current value, `vpd_target_*`/`vpd_danger_*`) and the chart's own threshold lookup (historical points, with a `day_`/`night_` split and a night-falls-back-to-day rule the slice lacks). These are not redundant — the slice answers "status _now_", the chart needs a threshold _function_ over past points with `ChartUtils.getIsDay` deciding day vs night per point — but the threshold _table_ was duplicated, and the Metric Descriptor now owns it. See ADR-0030.
_Avoid_: metric config (that is `METRIC_CONFIG`, one of its inputs), metric spec.

**Phase Chart Series**
The value-space render input of the [[Steering Phase Chip]]'s VWC chart: `{ points: { atMs, vwc }[], window: { startMs, spanMs }, min, max, targetVwc, triggerVwc, currentVwc }` — timestamps and VWC percent only, **no pixels and no SVG paths**, the same split [[Env Series]] made. Built by the pure `computePhaseChartSeries(history, targetVwc, triggerVwc)` in `features/environment/phase-chart-series.ts`, alongside `computePhaseWindowSegments` (the [[Phase Windows]] laid over the chart's own time window, day-aligned, repeated per calendar day on `7d`) and `samplePhaseChartAt` (the scrub lookup, by fraction of the window rather than by pixel offset); `growspace-header-hero-ui` owns the chart's width and height and turns all three into SVG and percentage offsets at render. The trace is `VwcSample[]` so the same points also feed `resolveSaturationCrossing`, which decides the P1→P2 boundary the phase bar draws — one parse of the history, so the bar cannot disagree with the line above it (#826).
_Avoid_: "phase chart builder" (the pre-split private method, which returned path strings and pixel-space Y coordinates against a hardcoded 300 × 68).

**Env Series**
The value-space render input of an [[Env Graph]]: per series `{ id, title, color, unit, points: { time, value, meta }[], min, max, avg, chartType, darkPeriods, vpdBands?, guideBands?, guideLines?, sensor? }` — `sensor` present only on a multi-sensor metric's series, carrying the `{ entityId, name }` ref it traces — domain units and timestamps only, **no pixels and no SVG paths**. The window's photoperiods are resolved **once per call**, so every series' [[Guide Mark]] steps and its [[Dark Period]] shading are cut from one list. Built by the pure `computeEnvSeries(descriptors, histories, range, now)` in `features/environment/env-series.ts`, alongside the existing pure `crop-steering-model.ts`; `<growspace-env-chart>` owns width and height and turns Env Series into path strings at render time. Keeping geometry out of the derivation is what makes multi-sensor grouping, step-vs-line detection, per-metric axis scaling and the VPD day/night status bands assertable without mounting the element — the private-field reaches (`(element as any)._renderSeries`, `_computeGraphSeries`) that the pre-split shape forced on its tests are gone, and `env-chart.spec` now covers only scrolling, tooltips, observers, event dispatch and the no-data render (#472). See ADR-0030.
...
**Dark Period**
A stretch of an [[Env Graph]]'s window during which the growlights were off, shaded low-contrast behind the gridlines. Every Env Graph has it, including one carrying no [[Guide Mark]] at all: a stepping mark with no visible reason to step reads as a rendering glitch, a temperature chart's nightly dip is otherwise left for the grower to infer, and shading that appeared only when a target happened to step would come and go with unrelated config changes. It is the unlit complement of the photoperiods a step line is cut on, taken from that one list rather than resolved a second time — a mark and the shading beneath it cannot disagree about when night began if neither of them asked separately. A growspace with no light history has none, and its marks fall back to their day values. Carried per series as `darkPeriods` on [[Env Series]], in time space only.
_Avoid_: "night" (the shading follows the growlights, which need not follow the clock), lights-off period (names the boundary, not the stretch).

**Scrub Readout**
What an [[Env Graph]] reports at the instant the pointer is over, and the cursor that locates it: `<chart-scrub-tooltip>`. **One component serves every entry point** — a standalone Env Graph mounts it itself, a [[Curated Combo]] mounts it across both of its panes with the Env Graph delegating through a `chart-scrub` event, and the [[Tank Water Chart]]'s usage pane mounts it too. Whether a metric happens to have a combo recipe is an editorial claim about which metrics pair with which context (ADR-0051) and decides nothing about how its scrub reads; before #866 it decided the markup, the clock and the cursor's thickness. A scrub *is* one instant, so the moment is printed **once, as the readout's heading**, on the clock the user's Home Assistant locale asks for. Rows carry no timestamp of their own: a row reading an [[Interval Metric]] bucket rather than the instant says so with its swatch, a bar where a moment row has a dot — the shape each was read off in the pane below. Row values are formatted by the [[Env Graph]] family's one formatting owner (`metric-value-format.ts`, #855), never by the readout. The readout owns a fixed-dark ground and takes `on-overlay` foreground roles for it (ADR-0039); the cursor is drawn on the chart's own theme-owned pane instead, so it takes a theme-owned rule colour.
_Avoid_: tooltip (it is not hover help — it reads the chart at a position), hover readout.

**Guide Mark**
A horizontal reference drawn across an [[Env Graph]] to say where a metric's reading *should* sit. Exactly three kinds, and the kind is a property of the underlying configuration rather than of the chart: [[Optimal Band]], [[Setpoint]], [[Limit]]. Resolved from a [[Metric Target]] on the [[Metric Descriptor]], never read off the device by a chart. A guide mark whose configuration is period-indexed (VPD, humidifier thresholds) is drawn as a **step line** that jumps at lights-on and lights-off, with its label anchored to the segment under the current time — a flat line would sit at a value that was wrong for half of a 24h or 7d window. See ADR-0048.
_Avoid_: target line, threshold line (both name one kind and get used for all three).

**Optimal Band**
The [[Guide Mark]] for a region a grower wants a metric to stay inside — a biological optimum, not a controller's behaviour. Drawn as a tinted region with dashed edges in the metric colour, and the only kind that carries inline **value** labels (a [[Setpoint]] is labelled by name, a [[Limit]] not at all). Sourced from paired bounds: the VPD day/night target window, `soilMoistureBand`, [[Pore EC Target Band]], `ecTargetRanges`. A controller hysteresis pair is **not** one — see [[Setpoint]].
_Avoid_: optimal range, target band (the latter is taken by [[Pore EC Target Band]], a specific one).

**Setpoint**
The [[Guide Mark]] for a single value a controller acts on. Drawn as one dashed line in the metric colour, looser than a band edge's dash so it is not read as a [[Limit]], and labelled with its **name** rather than its value — a metric routinely carries several from different sources, and which line is which is the thing a grower cannot recover from the chart. Sourced from `temperature_target` / `humidity_target` / `vpd_target` on the fan configs, from the irrigation **Saturation Target** and **P2 trigger threshold**, and from each half of a `{on, off}` hysteresis pair — which renders as **two setpoints, never an [[Optimal Band]]**, because `{on, off}` describes what the controller does and not where the grower wants the metric to sit. A fan's `tolerance` rides along on the record as the controller's **deadband**, drawn as a faint region with no edges of its own: `target ± tolerance` says how far the metric may drift before the controller responds, which is not a preference and must not be drawn as a band. Normalized only where the controller is acting: a disabled fan, a signal a circulation fan is not regulating on, and an appliance the card does not control all yield no setpoint.
_Avoid_: target (ambiguous across all three kinds).

**Limit**
The [[Guide Mark]] for a boundary that should not be crossed. Drawn in the status colour with a tighter dash, and — unlike the other two kinds — carries no inline label, because a danger boundary is read as a region by colour and its exact value belongs in the scrub tooltip. Sourced from `vpdThresholds.danger*`, `critical_temp_low`/`_high`, `warningLevel`, `moldThreshold`, `stressThreshold`, `haltOnRunoffEcThreshold`. **Limits are not unioned into the value axis**: a threshold far from the data would flatten the real trace, so an off-scale limit renders as an edge chevron instead. [[Optimal Band]] and [[Setpoint]] marks *are* unioned in, so they cannot clip to the frame edge.
_Avoid_: danger line, alarm threshold.

**Metric Target**
The normalized record a [[Guide Mark]] is drawn from, carried per metric on the [[Metric Descriptor]] as `targets: MetricTarget[]`. It is where five differently-shaped device-config values become one thing a chart can draw: paired bounds, `target + tolerance`, `{min, max}`, `{on, off}` per stage and period, and bare scalars. Several targets on one metric is the normal case rather than a conflict — VPD alone can hold its optimal window, its danger bounds, a fan's control setpoint and both appliances' switching thresholds — so each record carries a stable `id` and the chart tells them apart by it. Two of the bare scalars are not normalized: `targetRunoffPercent` has no `MetricKey` to mark, and the growspace payload does not carry `dli_target_veg` / `dli_target_flower` at all. Typed by guide-mark kind, and **always** period-indexed: a source that does not vary with the photoperiod resolves `day` and `night` to the same numbers, so a consumer never asks whether this particular target steps — it compares the two. Absorbs the descriptor's former raw `vpdThresholds` field rather than sitting beside it — a raw field next to a normalized array guarantees VPD gets drawn from one and every other metric from the other. Resolving `ecTargetRanges` needs `biologicalMetrics.granularStage`, which is why the descriptor reads it. See ADR-0050.

**Curated Combo**
A fixed pairing the card asserts belongs in one chart — a primary metric plus the secondaries that give it context (temperature with exhaust duty, VPD with temperature and humidity). Its own `AnalyticsItem` type, **orthogonal to [[Metric Comparison]]**: a combo makes an editorial claim about which metric is primary and which is context, where a Comparison is a grouping the grower owns and can dismantle. The recipe set is a hard-coded table beside `METRIC_CONFIG`, not card YAML. Its geometry follows from its secondaries' data shape — see [[Instantaneous Metric]], [[Interval Metric]] and ADR-0049, ADR-0051. Rendered by `<metric-combo-chart>`, which does not draw the primary itself: it mounts the ordinary `<growspace-env-chart>` — [[Guide Mark]]s, [[Dark Period]] shading and scrub included — then either projects interval bar panes through a `secondary-pane` slot or hands instantaneous context to the Env Graph as faint traces on a labelled second value axis. All are drawn against one window, computed by the combo and handed to the Env Graph as `chartWindow`, because two now-anchored windows resolved a moment apart are a silently misaligned axis. Routing is on the `AnalyticsItem` type rather than on a `MetricKey` (unlike [[Custom Graph Routing]]): the pairing is what the type carries, and the item's `metrics` is the combo's flat *identity* — what keys the render and says what history is needed — while its structure stays in the recipe.

A recipe holds a **list** of [[Combo Secondary]] contexts in render order, and a secondary the growspace has no sensor for drops out on its own, so a combo degrades one context at a time rather than all-or-nothing. Seven recipes ship: Temperature and CO2 with exhaust duty, Humidity with humidifier *and* dehumidifier duty, Soil Moisture with irrigation duty, Pore EC with the runoff-minus-feed EC delta, Energy with power, and VPD with instantaneous Temperature and Humidity overlays. Interval panes are derived by `computeComboIntervalPane` in `features/environment/combo-series.ts`, in value space only; instantaneous overlays use the ordinary [[Env Series]] path.
_Avoid_: combo chart, default comparison, seeded comparison.

**Combo Secondary**
One contextual metric in a [[Curated Combo]]. An [[Instantaneous Metric]] becomes a faint overlay on the labelled second value axis. An [[Interval Metric]] becomes a subordinate pane and may be read `relativeTo` another metric so the pane reports the *difference* — feed EC against runoff EC says whether the substrate is accumulating salt, which neither absolute trace says alone. What an interval bar is depends on the metric's axis: one with a fixed axis reports **duty**, the time-weighted mean over the bucket as a percentage of its own full scale, because that is what a grower reads an actuator by; one that scales to its data has no full scale to be a percentage of and reports the mean in its own unit, as does every delta (the difference of two duties is percentage points, not duty). A duty pane is read against an absolute **0–100**: stating a full scale is what duty means, and using it is what makes the two duty panes of one combo comparable with each other and a pane comparable with itself across time ranges — peak-scaled, a fan holding 55% and a fan pinned at 100% are the same wall of full-height bars. A pane carrying a reading in its own unit has no such ceiling, so holding back headroom would shrink every bar for nothing and its tallest bar spends the box. A recipe may also name a `limitOf` — *where* a configured ceiling comes from on the growspace, never the number — and where one resolves it, and not the metric's own ceiling, is the pane's scale, drawn as a dashed rule the bars are read against. The bars still fit when they breach whichever ceiling applies, and the cap says which one it is.
_Avoid_: sub-chart, mini chart, sparkline, secondary pane (when the context overlays).

**Instantaneous Metric**
A metric with a meaningful value at a point in time (temperature, humidity, VPD, EC, tank level). As a [[Curated Combo]]'s secondary it may **overlay** the primary on a second value axis, the way the [[Crop Steering Day Chart]] overlays EC on VWC. Informally the *stock* half of a stock/flow pair.

**Interval Metric**
A metric whose value is an aggregate over a time bucket (fan duty, litres drawn, kWh, irrigation shots). As a [[Curated Combo]]'s secondary it always takes **its own pane, drawn as bars** — a bar has no value at an instant, so overlaying one on a line's value axis is a category error rather than a legibility trade-off. This is the whole rule for combo geometry: two panes is the primitive, and an [[Instantaneous Metric]] secondary is the exception that earns an overlay. A secondary pane gets a cap rather than a value axis, reading whatever that pane's scale is — see [[Combo Secondary]] for the 0–100 a duty pane is read against, for the peak a metric with no full scale falls back to, and for the configured ceiling that displaces both. Informally the *flow* half of a stock/flow pair. See ADR-0049.
...
Its chart normalizes each series to that series' own min/max and therefore renders no shared value axis; this is labelled as normalized rather than hidden, and normalizing to each metric's [[Optimal Band]] instead is the intended replacement (ADR-0052). Distinct from a [[Curated Combo]], which the card asserts rather than the grower.
_Avoid_: graph link, linked graph group, curated combo
...
The config dialog panel (inside the Climate tab, between the Climate Control panel and the Humidity Control panel) that exposes all `CirculationFanConfig` fields: enabled toggle, regulation mode selector, active-mode target+tolerance pair, VPD-mode-only temperature override sub-section (collapsed by default), min/max speed, dynamic wind settings (period + amplitude revealed when wind*enabled is toggled on), and — when Stage-Aware VPD is on — the [[Stage VPD Overrides Table]]. Disabled fields are greyed out when `enabled` is false, not hidden. A dirty circulation config is part of an [[Environment Change]] and rides inside its sparse `configure_environment` payload as `circulation_fan_config`; there is no separate circulation action in this save path. Contrast the [[Exhaust Fan Controller Panel]], whose config uses the dedicated second action.

**Exhaust Fan Controller**
The backend controller (`ExhaustFanCoordinator`) regulating exhaust fan speed. Configured via `ExhaustFanConfig` (a sub-object of `EnvironmentConfig`). Unlike the [[Circulation Fan Controller]] it has **no [[Fan Regulation Mode]]** — exhaust demand is always _combined_ (temperature, humidity, and VPD targets are all live simultaneously) — and **no wind effect**. Otherwise shares the same field vocabulary: `enabled`, `min_speed`/`max_speed`, per-metric target+tolerance, `stage_vpd_enabled` + `stage_vpd_overrides`, and `critical_temp_low`/`high`/`hysteresis`. Source-air gating reuses the existing `minimum_source_air_temperature` + lung-room sensors (no new fields). `ExhaustFanConfig` is an independent backend dataclass, **not** a subclass of `CirculationFanConfig`.

**Exhaust Fan Controller Panel**
The config dialog panel in the Climate tab rendered **directly below** the [[Fan Controller Panel]] (circulation). Mirrors its look but for `ExhaustFanConfig`: enabled toggle; [[Stage VPD Overrides Table]] (always available — no VPD-mode gate, since there is no mode); all three target+tolerance pairs shown at once (combined demand); min/max speed inputs; and a collapsed-by-default **Critical Temperature** expander holding low/high/hysteresis. When Stage-Aware VPD is on, the VPD target relabels to "Fallback VPD Target" and the overrides table appears — same behaviour as the circulation panel's VPD mode. **No** regulation-mode selector and **no** wind controls. Persistence differs from circulation: `configure_environment` does not accept `exhaust_fan_config`, so [[Environment Change]] invokes `configure_environment` first and the dedicated `configure_exhaust_fan` action second, then refreshes. Failure rejects without rollback and retains the caller's draft and dirtiness for retry.

**AC Infinity Device** (also: AC Infinity Port)
A fan/humidifier actuator that exposes **no single `fan`/`switch` entity** and so cannot ride the plain entity multi-selects. It is driven as a _bundle_: a mode `select` entity + a speed `number` entity + an on-speed value (1–10). Configured per role — the [[Fan Controller Panel]] and [[Exhaust Fan Controller Panel]] each hold an _array_ of them (`circulation_fan_ac_infinity_devices` / `exhaust_fan_ac_infinity_devices`), and the Humidity tab holds `humidifier_`/`dehumidifier_ac_infinity_devices`. Rendered by the shared `renderAcInfinityDevices` helper: one bordered "Port N" card per bundle (mode picker, speed picker, on-speed) plus an add button. **Identity rule:** an entity counts as AC Infinity only when its entity-registry `platform` is `ac_infinity` — the mode picker offers `select.*` and the speed picker `number.*` entities _of that platform only_ (read from `hass.entities[eid].platform`, an untyped-but-present runtime field on the frontend hass object). An already-saved entity is always kept in its picker's option list even if it no longer matches the filter (e.g. the integration is unavailable), so existing configs never render blank. Bundles can be filled in one step via [[Port Pre-fill]]. See ADR-0022 _in the integration repo_ for the backend bundle model.

**Automated Mode Conflict**
When a bound [[AC Infinity Device]] port's mode `select` sits in one of the controller's self-running modes — any state other than `Off`/`On` (e.g. `Auto`, `VPD`, `Timer to On/Off`, `Cycle`, `Schedule`) — the AC Infinity controller/cloud keeps re-asserting the port and Growspace Manager's `On`/`Off` writes do not reliably stick. This is a limitation of the `ac_infinity` integration, not GSM, and GSM cannot fix it (the setting lives on the device/app). The config dialog surfaces a **passive, non-blocking warning** beside the port's mode picker whenever the selected `select` reads a non-`Off`/`On` state, telling the user to take that device out of its automated mode in the AC Infinity app/entity. Detection is **card-side and reactive** — a value derived live from `hass.states[modeEntity].state`, so it fires on fresh selection _and_ on dialog reopen for a port left in an automated mode, and clears live once the mode changes. Applies to all four roles (`circulation`/`exhaust`/`humidifier`/`dehumidifier`). `unavailable`/`unknown` are treated as "not automated" (no warning), matching the backend `ACInfinityDriver.is_on()` rule. The device is named from the device-registry name (friendly-name fallback), and the message includes the current mode string. See the identical _Automated Mode Conflict_ term in the integration `CONTEXT.md`.

**Port Pre-fill**
The device-picker affordance on every [[AC Infinity Device]] editor (grow light tab + the four actuator roles): the user picks an `ac_infinity` _device_ (a port) and the card resolves the bundle's member entities from the frontend entity registry, matching by `device_id` + `platform === 'ac_infinity'` + `translation_key` + domain (`select`/`active_mode` → mode; `number`/`on_power` → speed/power; `time`/`schedule_mode_on_time`/`schedule_mode_off_time` → on/off time; `switch`/`sunrise_timer_enabled` + `number`/`sunrise_timer_minutes` → sunrise). Picking a device **overwrites all role fields**: resolved roles are filled, unresolved roles are _cleared_ (never left stale from another port) with an inline warning naming what wasn't found (disabled entities don't appear in `hass.entities`, so they resolve as missing). Pure UI sugar — the saved config remains the explicit entity bundle; resolution happens only at the moment of the pick, never at save time or runtime, and a mis-resolution degrades to the manual pickers. The picker is ephemeral (not part of the environment draft — no seeder involvement); on dialog reopen it displays the device derived live from the saved mode entity's `device_id`. Only devices exposing an `ac_infinity` `active_mode` select are listed (what makes a device a controllable port, vs. the controller parent device). See card ADR-0028; amends the "one picker rejected" verdict of integration ADR-0024.

**Duplicate Port Warning**
A passive, non-blocking warning (same conventions as [[Automated Mode Conflict]]) rendered when a port's mode entity appears in more than one role bundle within the growspace's environment draft — e.g. the same port configured as both exhaust fan and dehumidifier, where two GSM controllers would fight over one device. Draft-scoped: it does not detect the same port claimed by a _different growspace_.
...
**ActionDispatcher** _(legacy — being retired)_
The pre-slice _write_-path counterpart to [[DataService]] (`store/core/action-dispatcher.ts`, reached as `store.actions.*` from ~22 cards/dialogs/containers, ~199 call sites). It is a shallow module that **conflates two modules wearing one coat**: a _domain-write_ router and a _UI-reaction_ router. The domain half already migrated — the `*-actions.ts` files it delegates to are now thin wrappers over the slice [[Mutator]]s (`plant-actions.ts` imports 32 slice symbols, `environment-actions.ts` 15). What survives in the wrapper is **UI orchestration glued to the write** (`ctx.ui.*` ×75, `refreshData` ×31, `closeDialog` ×11). It fails the deletion test only because that glue has no home _in the dispatcher's shape_ — but the glue is **call-site-specific, not domain-specific** (proven by `environment.configure`'s two sites diverging: one closes the dialog and toasts, the other only `console.error`s). So the honest split is: **domain writes → slice Mutators; UI reactions → the UI slice + the calling container/dialog** (orchestration lives at the UI seam, never in a write layer). Of the ~199 `store.actions.*` calls, **96 (`.actions.ui.*`) are pure UI-state ops** that re-point to existing `slices/ui` setters with no orchestration question; the remaining ~103 domain calls move their per-site orchestration inline (preserving shared error messaging via the UI slice's [[`showError` helper]]). Every `*-actions.ts` is now a wrapper over an _existing_ slice [[Mutator]] — there is **no** unmigrated domain still needing a slice: `growspace-actions.ts` duplicates the Growspace slice's `addGrowspace`/`updateGrowspace`/`removeGrowspace` (repointing also _fixes_ legacy `updateGrowspace`'s optimistic-without-inverse rollback bug), and `device-actions.ts`'s `handleDeviceChange` is just `grid.setSelectedDevice` (a Tier-1 UI re-point, not a domain). End-state: `ActionDispatcher`, `ActionContext`, `withAction`, and `store/**/*-actions.ts` deleted; completes ADR-0001 for the write path (the [[DataService]] entry's counterpart). Retire low-risk-first: the 96 UI-leaf re-points, then repoint the growspace/device CRUD to the existing slice mutators, then repoint+inline-orchestrate the remaining already-delegating domains.

**Active growspace** _(per-card selection)_
Which growspace a card is currently showing/acting on. Selection is **per-card only**: `makePerCardGridSlice()` mints an isolated `$selectedDevice` atom that every [[GrowspaceDataStore]] owns (`store.grid.$selectedDevice`), written by `handleDeviceChange` → `setSelectedDevice` and by every [[Bootstrap controller]]. Per-card isolation is deliberate so carousel and standalone cards don't clobber each other's selection (same motivation as the page-global-UI-state fixes, #406/#407/#408). **Any code that resolves the active growspace reads the per-card atom (or an explicit `growspaceId` threaded through the dialog payload) — there is no module-global selection.** A page-global `selectedDeviceId$` atom + `gridSlice` facade used to coexist; after the [[ActionDispatcher]] retirement (#342–344) nothing wrote it, so every reader that still consulted it silently broke — the add-plant save ("No growspace selected"), the no-arg IPM open, and the crop-steering toggle. Those readers were repointed to the per-card atom (ADR-0027, #437/#440) and the dead global was then removed (#438). Historical note: the retirement-era bug was invisible because the dialog _rendered_ the right growspace (its `render()` always read the per-card atom) while the write path read the dead global.
...
The rule that makes a dialog-open a _pure UI fact_: `openDialog({ type })` only sets [[the active-dialog atom]]; the dialog component fetches the data it needs itself (in `connectedCallback` / first render), the same lazy on-subscribe pattern the slices already use. Replaces the legacy `open*Dialog` helpers that glued a data fetch onto the open call (`openIPMDialog` → `fetchIPMPresets`, plus `openAddPlantDialog`, `openStrainRecommendationDialog`, `openNutrientPresetsDialog` — 4 of 18 `open*` helpers). The point is to _remove_ the UI-op⊕domain-fetch conflation rather than relocate it: a self-fetching dialog is also self-sufficient to test (mount it, assert it fetches), and every site that opens it stays a one-line UI call.

**Dialog Portal Identity**
Which dialog-host portal a dialog belongs to, carried as `portalId` in its open payload. One page-global `activeDialog$` feeds every portal, and a card mounts one per `growspace-manager-card` / `growspace-grid-card`, so a dialog that names no portal renders in all of them — two carousel cards stacked two irrigation dialogs (#913). The opener captures the opening card's `store.instanceId`: a store is created once per card and handed to both its subtree and its portal, so the store instance *is* the portal identity. ADR-0055, the same bind-at-open-time rule [[Active growspace]] follows for the target growspace (ADR-0027). **Fails open twice over** — a payload naming no portal, or naming one that is not mounted, renders everywhere — because several cards provide the store context without mounting a portal of their own (analytics, subarea, tank, logbook, AI-insight) and a dialog that opens nowhere is worse than one that opens twice. `slices/ui/dialog-portals.ts` holds the mounted-portal registry, which is reactive because portals mount lazily on the first dialog open. Adoption is per dialog: only the irrigation dialog names its portal today. **Not** the device-ownership test beside it in the host, which suppresses a portal whose `devices` lacks the payload's growspace — that one cannot separate two portals, because `makePerCardGridSlice` computes `devices` from the page-global `devices$`.
_Avoid_: "multi-instance portal guard" for the device-ownership test — that name is what hid #913.
...
**Config Dialog decomposition (planned — ADR-0019 extended)**
The Config Dialog (`dialogs/config-dialog.ts`, ~3,959 lines, 13 tabs) is the next dialog to decompose into [[Tab ViewModel]]s + [[Tab Component]]s following the Irrigation Dialog's pattern. Its [[DialogStateMachine]] half already exists (`dialogs/config-dialog-sm.ts` extends `DialogStateMachine<ConfigTabId, ConfigTabStates>`); what is missing is the per-tab `createXTabViewModel($sm, …deps)` factories, the dumb `<config-x-tab>` components, and the reduced [[Config Dialog Shell]]. The one structural difference from the Irrigation Dialog is the [[Shared Environment Draft]]: six of the thirteen tabs (`sensors`, `climate`, `humidity`, `irrigation`, `vision`, `vpd_targets`) do **not** own independent sub-state — they all project and mutate one `environmentDraft`. Per ADR-0019 this is still **one factory per tab** (each `computed`s its own slice of the shared draft + its own derived values), **not** a merged "environment mega-VM". Decomposition order lands an _independent-draft_ tab first — **Notifications** (its own `NotificationsDraft` / `SAVE_NOTIFICATIONS` / `isNotificationsDirty`), the true analog of irrigation's Tanks tab — to prove the gesture→intent→effect loop _before_ taking on the env-save knot; then a single env tab (Sensors) to prove shared-draft projection, then Climate to prove the conditional second service call. The tab-switch dirty guard (`requestTabSwitch` / `discardAndSwitch` / `isActiveTabDirty`) is [[Config Dialog Shell]] wiring, not per-tab state.

**Config Dialog Shell**
The Config Dialog's [[Dialog Shell]] owns the Config Dialog state, navigation, dirty guard, and Save gestures. Environment saves submit the [[Shared Environment Draft]] and [[Dirty Write Set]] as an [[Environment Change]]; the Shell does not compose Home Assistant action payloads.
...
`envDraftFromDevice` (through `createInitialSM(device)`) is the single device-to-draft seeding seam. Config-dialog openers pass only a `growspaceId`; the dialog resolves that device from its injected list and seeds once per open. The once-only gate prevents a background device refresh from clobbering in-progress edits. This replaces the former `EnvironmentConfigData` DTO and the two hand-copied environment snapshots that could silently omit fields.

**Ephemeral expander state stays Shell-`@state`** _(carve-out, ADR-0019)_
Pure expand/collapse/accordion view state — the Climate tab's Temperature-Override and Critical-Temperature expanders (`_fanTempOverrideExpanded` / `_exhaustCriticalTempExpanded`), the Humidity tab's per-stage accordion (`_openHumidityStageId`), and the VPD-targets tab's per-stage accordion (`_openVpdStageId`) — lives as **[[Config Dialog Shell]] `@state`**, _not_ in the SM. The dumb [[Tab Component]] emits a toggle [[Tab Intent]] (`toggle-fan-temp-override`, `toggle-exhaust-critical-temp`, `toggle-stage`); the Shell flips its own `@state` and the flag is projected into the [[Tab ViewModel]]. This satisfies the no-`@state` component contract while keeping ephemeral view fluff out of the persistent SM. The ADR-0019 "draft/edit state → SM" rule targets _draft_ state (the inline-edit-after-CANCEL bug class); expanders have no draft semantics, so they are deliberately exempt. Both accordion tabs (Humidity #360, VPD Targets #363) followed this carve-out, each moving its read logic + stage list to the VM (`readThreshold`, `getVpdOptimal`) and forwarding `{ key, …, value }` edit intents the Shell merges against the live draft.

The Humidity tab adds two related patterns: (1) the two control-enable toggles are ordinary [[Shared Environment Draft]] fields and also fire an immediate `setHumidifierControl` / `setDehumidifierControl` backend service — so the component emits a `set-humidifier-control` / `set-dehumidifier-control` intent and the Shell updates the draft and performs the side effect. (2) Per-stage threshold edits forward `{ stage, cycle, point, value }` (the `stage` being the `DehumidifierStage`/`HumidifierStage` enum _value_ Record key the VM supplies, distinct from the display `id`); the Shell merges against the live draft via the retained `_updateThreshold` / `_updateHumidifierThreshold`. The threshold-read logic + stage list + defaults moved to `humidity-tab.viewmodel.ts` (exported `readThreshold`), so the read is pure and unit-tested at its new home.

**Climate fan/exhaust config edits — Shell merges against the live draft**
The Climate [[Tab Component]] (`<config-climate-tab>`) forwards a _partial_ fan/exhaust edit via `fan-config-changed` / `exhaust-config-changed` intents; the [[Config Dialog Shell]] merges it against `this._sm.environmentDraft.circulationFanConfig` (resp. `exhaustFanConfig`) and dispatches `UPDATE_ENV_DRAFT`. This (rather than the component merging against its own `.vm` prop and emitting the whole config) is what makes synchronous multi-field edits accumulate: the Shell reads the live, just-updated draft on each merge, whereas the component's `.vm` prop only refreshes on the next async render. The component still never reads the SM — it forwards a field partial; the merge-against-live-draft is the Shell's.

**Config Tanks tab — a hybrid (env-draft list + own sub-state)** _(`<config-tanks-tab>`, #364)_
The Config Dialog's Tanks tab is the first **independent-draft** tab decomposed, but it is really a _hybrid_: the tank **list** lives in `environmentDraft.irrigationTanks` (so the tab rides the [[Environment Change]] like the env-cluster tabs — add/delete dispatch `UPDATE_ENV_DRAFT`), while the inline add/edit **draft** is its own `tabs.tanks.sub` SM sub-state (`idle | adding | editing`, like the Notifications tab). The [[Tab ViewModel]] projects both — formatted list rows from the env draft, the open editor from the sub-state. The dumb component emits `add-/edit-/delete-tank-requested`, `tank-draft-changed`, `cancel-tank`, `save-tank-requested`; the Shell keeps `_openAddTank` / `_editTank` / `_deleteTank` / `_saveTank` / `_cancelTank` (which dispatch the `BEGIN_ADD_TANK` / `BEGIN_EDIT_TANK` / `COMMIT_TANK` SM events) as the intent handlers. **Not** the Irrigation _dialog_'s `<irrigation-tanks-tab>`, which edits live [[Tank Levels]] through the Irrigation slice — this one edits [[Tank Config]] in the config dialog's env draft (see [[Tank Config vs Tank Levels]]).

**Environment Change**
One requested mutation of a growspace's environment configuration, expressed as either a [[Shared Environment Draft]] plus [[Dirty Write Set]] or a [[Tank Config Change]]. It completes only after every required Home Assistant action and the resulting refresh succeed; failure retains caller-owned intent for retry.

**Environment Save Composer**
Superseded by [[Environment Change]]. The pure function (`features/config/environment-save.ts`) that turned the [[Shared Environment Draft]] plus its [[Dirty Write Set]] into a `configure-environment-submit` event detail, leaving mapping and action ordering to the [[Growspace Dialog Host]] and the Growspace slice. ADR-0019 and ADR-0032 describe it under this name.
...
**Atomic Dirty Group** _(accepted ADR-0032; implementation #519)_
A set of interdependent draft keys that becomes dirty and persists as one unit. `soilMoistureMin` plus `soilMoistureMax` is the first group: editing or clearing either marks both, and Save emits a valid numeric pair or two `null` values, never one bound. A dirty incomplete or invalid pair blocks Save.

## Irrigation

**Irrigation config ownership (migration in progress)**
The [[Irrigation slice]] (`irrigationConfigs$`) is the intended owner of a growspace's `IrrigationConfig` read-model, mirroring how it already owns tank levels. `irrigationConfigs$` is bootstrap-fed from `sync-service` and its mutators are the canonical write path. `GrowspaceDevice.irrigationConfig` is the _legacy_ read-model being retired — it is not a fresh field. The two are kept consistent on optimistic writes by a [[Cross-slice mutation]]: the irrigation mutators patch `irrigationConfigs$` and also call the Grid slice's `patchDeviceIrrigationConfig` setter, so the ~14 remaining `device.irrigationConfig` readers (header chip, crop-steering model, 3D equipment renderer) stay correct until they migrate to `irrigationConfigs$`. Once all readers move, drop the bridge and remove `irrigationConfig` from `GrowspaceDevice`. This completes ADR-0001 for the irrigation domain (supersedes the old `store/growspace/irrigation-actions.ts` → `OptimisticManager` → `DataService` path).

**Irrigation Command** _(accepted ADR-0054; implementation card#817)_
One requested change to a growspace's irrigation **configuration** — its settings or its strategy — expressed as the named mutator a caller actually means (`saveIrrigationSettings`, `updateIrrigationStrategy`, `toggleIrrigationMode`, `setProgramAutoAdvance`, `setSteeringPhase`, `applySteeringMode`) and compiled by one internal module (`slices/irrigation/irrigation-command.ts`). The module owns [[Irrigation Field Ownership]], the omission and clearing rules, the wire payload, strict outbound validation, the optimistic projection onto both the [[Irrigation slice]]'s read model and `GrowspaceDevice`, and the inverse that restores both. Its command union is deliberately not exported. Schedule collections, per-stage EC target ranges, Drain Monitoring, drain readings, manual cycles, analytics, the [[Recipe Stamp]] and [[Irrigation Program]] stamps, and hydration are outside it. The backend counterpart is growspace_manager's Irrigation Change seam (its ADR-0046), which draws the same boundary.

**Irrigation Field Ownership** _(accepted ADR-0054)_
The total classification of every `IrrigationStrategy` field and every settings field an [[Irrigation Command]] may carry: a **wire** field with its snake-case action key and its clearing rule, the nested Substrate Profile that **expands** onto the action's two flat keys, or a field another operation owns — **observed** by the backend (the detected lights-on time) or written by a **stamp** (the declared Steering Mode, the [[Recipe Stamp]], the [[Irrigation Program]] binding). Totality is enforced by the type checker, so a new strategy field without a classification is a compile error rather than a value dropped in silence. Non-writing owners are dropped rather than refused, because the Steering tab's draft is seeded from the whole strategy and legitimately carries them — and dropping is what keeps a strategy save from blanking a stamped mode locally.

**Irrigation clearing rules** _(accepted ADR-0054)_
`undefined` omits a field and leaves the growspace's stored value alone. `null` clears, but only where the Home Assistant action defines a clear: `soilTriggerPercent`, `dailyVolumeCapLiters`, `maxCyclesPerDay` and `haltOnRunoffEcThreshold` on settings, `poreEcTargetMin` and `poreEcTargetMax` on strategy. A null pump entity is the one asymmetry — "no pump" in the domain, the empty string on the wire, which is what the action has always read that way. Every other null is refused by name before anything is projected or sent, because the backend seam refuses a field it does not own by name and forwarding the null would turn that into a service trace naming a wire key no control is labelled with.

**Tank Config vs Tank Levels**
Two facets carried by a single `IrrigationTank` object, not two types. **Tank Config** is the user-editable identity of a tank — `sensorEntity`, `name`, `warningLevel`, `volumeLiters`; **Tank Levels** is live backend-derived telemetry and never belongs to a write.

**Tank Config Change**
An [[Environment Change]] that replaces the complete Tank Config collection while excluding Tank Levels. It is the narrow write intent used by the Irrigation Dialog when it does not own a [[Shared Environment Draft]].
...
**Today's Usage** (a.k.a. **Liters Today KPI**)
The **pump-only** KPI block at the top of the Water Analytics tab (`irrigation-water-analytics-tab.ts`, `_renderTodaysUsage`): "Liters today", "Per plant / day", "Water efficiency", "Avg runoff". The whole section is gated on `vm.hasPump` — a grower with no irrigation/drain _pump entity_ never sees it. "Liters today" itself is the live `waterUsage.litersToday`, i.e. the [[Aggregate Water Use]] figure, which per `growspace_manager`'s ADR-0017 folds in manual watering on top of the measurement source; "Per plant / day" is the backend's cycle-average `liters_per_plant_per_day` (`figures.cycle / plantCount / daysSinceCycleStart`, **not** a today figure). Despite the legacy "Tank-Derived Water Chip" name this is **not** a secondary-header [[Chip]] — no `MetricKey.WATER` chip is built in the header-metrics slice.

**Accepted tradeoff (gate narrowed to `hasPump` only):** earlier the section was gated `hasPump || hasTank` so a gravity-fed grower could see the additive `litersToday`. That was reversed: for a [[Tank-Derived Water Mode|tank-only]] grower the whole block is now hidden and their water headline is the Tank-Derived card's "Consumed today" (see [[Tank-Derived Water Usage]]). Consequence, knowingly accepted: a tank-only grower loses the _additive_ figure (so manual watering done from a separate source is no longer surfaced anywhere) and the manual-log "Avg runoff" number. `hasPump` (a pump entity is configured) is independent of [[Tank-Derived Water Mode]] (tanks present, no flow/drain sensors); a pump+tank grower sees both this block (pump per-plant) and the Tank-Derived block (tank per-plant) — they measure different things. Separately, `MetricKey.WATER` routes to a [[Tank Water Chart]] (`growspace-analytics-ui.ts`), which fetches its own bucketed history and does not read `litersToday`.

**Tank-Derived Water Usage**
The KPI block shown for a grower with tank sensors + history (`_renderTankDerivedUsage`, gated on `hasTankSensors && hasTankHistory`): "Consumed today", "Last 7 days", "Avg per day", and **"Per plant / day"**. The per-plant figure is **card-derived, not from the backend** — `tankAvgPerDay / device.plants.length` (renders "—" when either is 0), deliberately mirroring the displayed "Avg per day" ÷ plant count so the two rows reconcile by eye, and matching the cycle-average basis of the pump per-plant figure in [[Today's Usage]] rather than a noisy partial-day "today" number. Plant count uses the same population as the backend (`get_growspace_plants` = `len(plants)`), so no backend change is needed (card-only, respects the API→store→atom→card layering).

**Tank-Derived Water Mode**
The client-inferred condition under which a growspace's water is measured from tank-level deltas rather than a flow/drain sensor: `irrigationTanks.length > 0` AND no `irrigation_flow_sensors` AND no `drain_volume_sensors`. It gates the [[Tank Water Chart]] history aggregation and selects tank-derived inference as the measurement source in [[Aggregate Water Use]]. Note (`growspace_manager`'s ADR-0017): being in this mode no longer means the reported `litersToday` is tank-derived _only_ — manual watering is always added on top, so a grower hand-watering from a separate source is counted in addition to the tank-level drop.
...
**Irrigation Method**
The growspace's irrigation _hardware_ shape, distinct from [[Irrigation Mode]] (which is the delivery _behaviour_: Manual Schedule vs Crop Steering). Derived client-side on the shared [[Dialog Capabilities]] atom (`dialog-capabilities.ts`) as `irrigationMethod: 'pump' | 'tank' | 'none'` — `'pump'` when a pump/drain entity is configured, else `'tank'` when at least one [[irrigation tank|Tank]] exists (a gravity-fed or manual drip setup, overlaps [[Tank-Derived Water Mode]]), else `'none'`. The Irrigation Dialog uses it to **recognise tank-based growers as configured** rather than nagging them to add a pump: in `'tank'` mode the [[Dialog Shell|dialog]]'s setup hints show a positive "Tank-based irrigation detected" acknowledgement and suppress the pump-centric Schedules/Crop-Steering nags, the Config tab's **Pump Configuration** section is relabelled as optional ("Irrigation Pump (optional)" + a tank-based note) so the two `None` selects don't read as missing requirements, and the [[Liters Today KPI]] surfaces (gated on `hasPump || hasTank`). A `'tank'` method has **no actuator**, so crop-steering _actuation_ and the Schedules/Run-Now/Behaviour panels stay gated on `hasPump` (ADR-0016) — the pump is framed as the path to _enabling_ those, not a missing requirement. (Scope note: read-only crop-steering diagnostics remain pump-gated for now; surfacing them for pumpless tank+soil-moisture setups would need an ADR-0016 amendment.)

**Irrigation Mode**
The two mutually exclusive ways a growspace can receive water. Switched via a toggle in the Schedules tab of the Irrigation Dialog, saved immediately on toggle. Distinct from [[Irrigation Method]] (the hardware shape: pump vs tank).
...
Under [[Skip P2]] the P1→P2 boundary becomes the P1→P3 one and P2 is zero-width for a different reason.

**Skip P2** (`skipP2AfterP1`)
The `IrrigationStrategy` opt-in (default off) that sends a completed P1 straight to P3, so P2 never runs — the card's half of the backend rule of the same name. `computePhases` reads it: P3 starts where P1 completed and P2 comes back zero-width with `skipped: true` and the target `Skipped`, and `computeCropSteeringCycle` takes the same `saturationReachedAt` so the shot window closes there rather than at the P2 Stop Buffer. With no measured crossing there is no completion to move P3 to, and the projected day is drawn exactly as it would be without the option — the honest picture, the card having nothing yet to say about when P1 ends.

The distinction the UI has to carry is **bypassed, not cleared**: the P2 setpoints are still stored and still shown, so the Steering tab renders them disabled and dimmed with the reason, and the [[Crop Steering Day Chart]]'s legend keeps the P2 chip struck through instead of dropping it. A phase that vanished would read as one that was never configured. It is a steering-**draft** field, unlike the phase-trigger toggles it sits beside in Phase Triggers (which write the config draft), because the projected day has to follow the toggle before anything is saved (`strategyOverride`, growspace_manager_workspace#130).
...
- **Latest**: end of the _current_ phase's relevant boundary — P0 end for P0, the (single, shared) P2-stop time for both P1 and P2, and tomorrow's P2-stop when currently in P3
...
The chip row beneath the [[Crop Steering Day Chart]]. Distinct purpose from the chart's `.cm-readout`: the readout already supplies the color-to-trace mapping for whatever is configured, so the legend's only job is to flag what's _missing_. A `.cs-leg-chip` therefore renders **only** for a metric that has no configured sensors — muted (`opacity: 0.4`), with copy like "Pore EC not configured — add it in Environment Settings". A configured metric (including Substrate VWC, which is always configured) gets no legend chip at all. The Phase Window legend chips (phase color + label + shot count) are a separate row and unaffected by this rule.
...
**Irrigation Recipe**
A grower-authored, reusable snapshot of one growspace's irrigation settings, held in a **global** library every tent can see. The card reads it off `irrigation.recipes` on the growspace payload it already has — the same library on every payload, so there is no separate fetch — and hydration sets it once into the Irrigation slice's `irrigationRecipes$`. A recipe carries exactly one `kind`: `crop_steering` (the [[Irrigation Strategy]] setpoints) or `schedule` (times, durations, caps). Applying is a server-side stamp, so the identity and the [[Recipe Provenance]] are all the card needs to *apply* one; the setpoints are read on exactly one surface, the [[Recipe Library Editor]], where a grower corrects them. They are carried wire-shaped rather than camelised, because the editor sends the changed ones straight back under the same names. GSM owns the shape and the rules — see growspace_manager's CONTEXT.md and ADR-0045.
_Avoid_: irrigation preset (a preset is a table that ships with the product, like the [[Steering Mode]] one), irrigation profile (collides with [[Substrate Profile]]).

**Recipe Provenance**
The authoring context stamped onto an [[Irrigation Recipe]]: media type, liters per pot, pump flow rate, and the stage + week it was saved in. In the card it does exactly two things — it **orders** the [[Recipe Tab]]'s picker and pre-selects what the picker opens on, and it names the authoring medium when that differs from the target's. It never filters, never gates, and never blocks: applying a flower-week-3 recipe to a week-5 tent is a supported deliberate act. `stage` is null (and the week meaningless) when the authoring growspace held no live plants.

**Recipe Stamp**
What a growspace records when an [[Irrigation Recipe]] is applied to it: `appliedRecipeId` and `recipeAppliedAt` on the [[Irrigation Strategy]], plus the derived `appliedRecipeDrifted` beside them. Both ids null means **never applied** — a real third state, not a default. Applying always writes, so re-applying the recipe already showing is the way back after hand-tuning. Drift is computed by the backend on read (recipes are held by reference, so a hash stamped at apply time would go stale when the recipe itself changed); `null` means the question does not apply — nothing was ever applied, or the applied recipe has since left the library. The card renders those three answers and derives none of them.

**Recipe Tab**
The Irrigation Dialog's `recipes` tab — the whole [[Irrigation Recipe]] story in one place: the [[Recipe Stamp]] this growspace carries, the picker that applies another, and the form that saves the current settings as a new one. Saving lives here rather than on the tabs it snapshots because it is one gesture over all of them. Pump-gated, like every other tab whose subject needs an actuator. Two exclusions that are not the same rule: the picker offers only the half the growspace is running (a recipe is never half-applied, and the backend refuses the other), surfacing the count it left out rather than shortening the list silently; [[Recipe Provenance]] only ever orders.

**Recipe Library Editor**
The standalone surface for the [[Irrigation Recipe]] library itself — browse every saved recipe, rename one, correct the values it stores, delete it. Reached from the header menu's Setup group, beside Nutrients, and takes **no growspace**: the library is global, so this edits recipes as objects, while the [[Recipe Tab]] is where one growspace gets one. That split is why nothing here filters, orders or gates by a tent, and why both kinds are always listed. `recipe-library-dialog.container.ts` owns the `RecipeLibrarySM` and the two mutations; `recipe-library.viewmodel.ts` derives the rows and the form; `irrigation-recipe-library.ts` renders it. An edit is **sparse** — only the fields the grower actually changed are sent, so a rename travels with no values — and it changes no growspace: applying is by value, so a tent that carries the recipe keeps its numbers and simply starts reading as drifted. Deleting is never refused and never cascades; the confirmation names the [[Irrigation Program]]s whose slots point at the recipe, because a slot with no recipe holds rather than watering wrongly. That list is the editor's one reason to read the program library, and naming plans is all it ever does with it.

**Irrigation Program**
A whole-run plan assigning [[Irrigation Recipe]]s to `(stage, week)` slots, held in a **global** library exactly as the recipe library is and read off `irrigation.programs` on the growspace payload the card already has; hydration sets it once into the Irrigation slice's `irrigationPrograms$`. A slot carries a `recipeId` and nothing else — recipes are held **by reference**, so correcting one recipe corrects every plan using it, and a slot naming a deleted recipe becomes a gap rather than cascading. A growspace follows one through an explicit `irrigationProgramId` on its [[Irrigation Strategy]]; assigning writes that one id and **no setpoint**, so picking a plan from a dropdown cannot change what a pump does that same minute. GSM owns the shape and every rule — see growspace_manager's CONTEXT.md and ADR-0045.
_Avoid_: irrigation schedule (means [[Irrigation Schedule]] — the times a pump fires), feed chart.

**Program Hold**
The [[Irrigation Program]] layer's single safe default: when the plan has no unambiguous instruction, nothing changes and the payload says why. The card **renders** the cause and derives none of it — `no_position`, `no_slot`, `program_complete`, `recipe_missing`, `drifted`, `not_applicable` — each with its own heading on the [[Program Tab]], because the behaviour being identical in every case is exactly why a finished run must not read as a broken plan. The cause set has already grown once, so the wire value is read as a plain string and narrowed: an unrecognised cause becomes `null` and the backend's own `detail` sentence carries the meaning, rather than failing the parse of a whole growspace payload.
_Avoid_: skip (implies the week was passed over rather than deliberately left alone), fail.

**Program Progression**
What the program layer will *do* about a growspace's current position, at `irrigation.program.progression` as `{state, hold, detail}` — `up_to_date`, `available`, `due`, or `held` with a [[Program Hold]] cause. It is the **same resolution** the backend's own refresh acts on, which is why the card reads it and never re-derives it: a card that computed its own answer could say a week is held while the tick stamps it. `detail` is the backend's grower-facing sentence and is always rendered, so even an answer the card does not recognise still says something true. The one thing the card does derive is the drift **annotation** — the fields it can compare like-for-like between the applied recipe and the live growspace — and only ever on top of a `drifted` verdict the backend has already given; it stays silent about [[Substrate-Relative Shot Storage]], whose stored unit depends on the growspace.
_Avoid_: auto-apply (names the happy path only, and the holds are the point).

**Program Tab**
The Irrigation Dialog's `program` tab — one growspace's whole relationship with the plan it follows: the assigned program, the week it is in, what that week calls for, what the plan holds next, the auto-advance opt-in, and the [[Program Progression]] answer above all of it. Pump-gated, like the [[Recipe Tab]] beside it. "Next" follows the **run**, not the week number: a plan crosses stages, so veg week 4 is followed by flower week 1 and the tab says whether what it found is literally next week. Turning auto-advance **on** while a program is assigned is confirmed first — it is not a preference that takes effect later, it makes the week the growspace is already in due — and turning it off is not, because it takes something away. `program-tab.viewmodel.ts` derives it; `irrigation-program-tab.ts` renders it.

**Program Library Editor**
The standalone surface for the [[Irrigation Program]] library — browse every plan, build one, edit its slots, delete it. Reached from the header menu beside [[Recipe Library Editor]], and takes **no growspace** for the same reason: a plan exists whether or not any tent follows it. The detail is a **grid** with a column per stage and a row per week, and that is the design rather than a layout: a position the plan says nothing about is rendered as an empty cell, because a grower has to be able to see that week 4 is empty — that emptiness is the [[Program Hold]] that makes the tent hold. Saving replaces the plan's whole slot list, so the draft carries what the grid is showing and a cell the grower emptied is gone precisely because it is not sent. `program-library-dialog.container.ts` owns the `ProgramLibrarySM`; `program-library.viewmodel.ts` builds the grid; `irrigation-program-library.ts` renders it.
...
**Genetics Tree Layout**
The pure layout/graph module `genetics-tree-layout.ts` beneath the [[Genetics Tree View SM]]: `buildIndex`, the three layout algorithms (`layoutTopDown`, `layoutSubgraph`, `layoutBreederGrouped`), the lineage set operations (`ancestorsOf`, `descendantsOf`, `motherLineOf`), and the SVG edge-path helpers (`edgePath`, `edgePathCurve`). Already a deep seam — pure functions over `TreeNode[]`, no Lit, fully unit-tested. The SM's `chooseLayout`/`visibleNodes`/highlight selectors compose over this; the layout module itself is untouched by the SM extraction.

**Genetics Tree View SM**
The state machine for the shared `genetics-tree-view.ts` component, to be extracted into `genetics-tree-view-sm.ts`. A plain **functional** SM (`createInitialSM()` / `transition(sm, event)` + pure selectors) like the [[Strain Editor SM]] — **not** nanostore-backed (it is a shared component, not a store-backed dialog) and **does not satisfy** [[DialogStateMachine]] (no tabs). Replaces the component's scattered sibling `@state()` flags, which partition into three classes with different fates:

- **Navigation / filter / selection → SM state:** `mode` (`tree | lineage | families`), `focalId`, `breederFilter`, `collapsed` (Set), `selectedId`, `search`, `genFilter`. Two filter tiers live here: `breederFilter` + `collapsed` are **relayout-affecting** (they feed the pure `visibleNodes` selector and the `chooseLayout` decision), whereas `search` / `genFilter` / `selectedId` are **decoration-only** (highlight/dim + the `hasActiveFilters` "show Reset" derivation — no relayout). The pure selectors (`visibleNodes`, `chooseLayout`, `ancestorSet`/`descendantSet`/`highlightSets`, `hasActiveFilters`) are the leverage and the test surface — assert transition sequences instead of screenshotting the component.
- **Viewport ephemera → stays component `@state`:** `panX/panY`, `scale`, `viewW/viewH`, plus non-reactive `dragging`/`didPan`. Carve-out per [[Ephemeral expander state stays Shell-`@state`]] — pan/zoom has no draft semantics. `hoverId` is also component-local (transient pointer state) and is **passed into** the highlight selector as a parameter rather than promoted into SM state.
- **The seam-crossing flag → a generation counter.** The old `_userHasInteracted` boolean (re-armed by navigation, disarmed by viewport gestures, read to decide auto-refit) is replaced by a monotonic `reframeGen` on the SM: the SM bumps it on every re-arming transition (mode change; focal change via double-click / isolate / clear-focus / jump-to; and **`breederFilter`**, normalized into the re-arming set — the old code force-refit on breeder without re-arming, an oversight). The component owns `panGen`, pins it to `sm.reframeGen` on any pan/zoom, and auto-refits when `reframeGen > panGen` — so "auto-frame after navigation, hold the camera after manual pan" survives the split with no shared boolean. **Resize stays a component-local always-refit rule**, orthogonal to the counter and never touching the SM (it is a viewport-input change, not a layout-content change). The pure layout/graph functions in [[Genetics Tree Layout]] (`genetics-tree-layout.ts`) are unchanged — already a deep seam beneath this.
...
**Crop Steering Dialog SM** _(retired — [[ADR-0016]])_
...
The Config Dialog's Growlights tab: enable/power/sunrise controls plus the plain-entity and AC Infinity configurator pickers. It also **owns the edit surface for the lights-on time** — the crop-steering photoperiod anchor (`IrrigationStrategy.lightsOnTime`). Although `lightsOnTime` remains a _strategy_ field (not a `GrowLightConfig` field), this tab is where the user sets it, because lights-on is conceptually a property of the light schedule. The input is placed **outside** the controller's enable-gate, so a crop-steering-only user with no grow-light controller configured can still set the anchor. Unlike the rest of the tab — which buffers into the [[Shared Environment Draft]] and persists on the dialog's Save via `configure_environment` — lights-on **persists immediately** on change via `updateIrrigationStrategy(id, { lightsOnTime })` (a field-by-field partial merge, so it never clobbers other strategy fields). The [[Steering Tab]] displays lights-on **read-only** with a hint pointing here. See ADR-0026.

## Light Cycle Tracking

An opt-in sub-feature of Crop Steering (`IrrigationStrategy.autoLightTracking`). When enabled, the backend listens for the light sensor's off→on transition and records the time as `detectedLightsOnTime` on the strategy. The VWC coordinator resolves lights-on time as `detectedLightsOnTime ?? lightsOnTime`. The user's manually configured `lightsOnTime` is never overwritten. The `lightsOnTime` value is _edited_ on the [[Grow Light Controller]] tab (Config Dialog); the auto-tracking **toggle** and the `detectedLightsOnTime` **read-out** remain in the Steering tab of the Irrigation Dialog, shown alongside the now-read-only lights-on display.

## Lifecycle Timestamp

A plant stage-start value (`seedling_start`, `mother_start`, `clone_start`, `veg_start`, `flower_start`, `dry_start`, `cure_start`) — the moment a plant entered a stage. Represented as a timezone-aware ISO 8601 datetime string (date _and_ time), never date-only, across input, validation, and wire. The card owns this contract in one place: `lifecycle-timestamp.ts`, with `fromBackend(value)` (backend ISO → `datetime-local` input value, also tolerant of legacy date-only values without a UTC-midnight off-by-one) and `toWire(inputValue)` (input value → verbatim wire string, or `null` when empty). `md3-date-input` displays via `fromBackend`; `mapDialogToApiPayload` serialises via `toWire`. The seam guarantees the format, so there is no "set both date and time" save-time validation — `datetime-local` cannot emit a partial value. The backend stores and returns the same datetime string (growspace_manager ADR-0013). Distinct from drying-tab `date` inputs (`WeightEntry`/`MoistureEntry`), which stay date-only.
...
A pulsing warning chip rendered to the left of the Optimal Conditions chip in the secondary chip strip. Visible from the day `flower_start` is reached for any plant in the growspace; persists until explicitly dismissed (dismiss keyed to `growspaceId + flowerStart`). Since editing lights-on moved to the [[Grow Light Controller]] tab (ADR-0026), clicking it opens the **Config Dialog** on the Growlights tab via `openConfigDialog(device, ConfigTab.GROWLIGHT, 'lightsOnTime')` — the chip fires _at flower-flip_ to prompt the user to **set** lights-on, so it must land on the editable tab, not the Steering tab's read-only display. The tab is seeded through the payload's `currentTab` → the dialog's `initialTab`; the third arg is a `scrollToField` deep-link threaded through the dialog to the `<config-growlight-tab>`, which scrolls the lights-on input into view and pulses it once with `var(--primary-color)` (a one-shot `field-pulse`, mirroring the irrigation dialog's field-pulse). The scroll+pulse runs inside the Growlights component itself because the target `[data-scroll-target="lightsOnTime"]` lives in that component's own shadow root.
...
Pure module tests (state machines, slices, utilities — anything that does not call `fixture()`) live next to their source file as `src/foo/foo.test.ts`. Tests that mount Lit components via `fixture()` live in `tests/`. The split is enforced by the rule: _if it touches the DOM, it goes in `tests/`_. Applies to new test files only — existing tests are not migrated.
...
**Portal Token Scope**
The `:host` block `growspace-dialog-host` declares, generated from `tokens.ts` as `portalVariables`. The host remains outside the card subtree, even though it is mounted below Home Assistant's root context provider, so it inherits none of the card's custom properties — without this block a bare `var(--font-size-sm)` under `src/dialogs/` resolves to nothing and the text renders at its inherited size. Carries every token except the [[Card-Only Token]] names, with references to those resolved to their card values so nothing in the block depends on a name it does not declare. See ADR 0036.

**Card-Only Token**
A token withheld from the [[Portal Token Scope]] because Home Assistant defines a custom property of the same name — `--divider-color` and `--error-color`, the only two of 94. Inside the card they shadow the user's theme; declaring them on the portal would take the dialogs' dividers and error colour away from that theme too. The shadowing itself is unresolved: renaming them is a migration of its own, tracked with #608.
...
**Contract Fixture**
The GSM repo's golden growspace payload (`tests/fixtures/contract/growspace_payload.json` there), serialized from a maximally populated growspace, fetched from two refs: GSM `main` and the latest GSM release. Card CI compares its key set against the [[Wire Shape]] tree, with a different verdict per ref — against `main` a fixture key the schema does not declare is drift and fails; against the release ref the check runs the other way, failing only when the schema _requires_ a key the released backend does not send. (Not a strict parse: production schemas strip unknown keys, because [[hassCall seam|hassCall]] throws on parse failure and a strict schema would blank the card on the next backend release. ADR 0031 amends ADR 0029.)

**Backward-Safe Card Change**
A card change whose declared [[Wire Shape]]s require no key the **release-ref** [[Contract Fixture]] lacks, proving it works against the GSM version users actually run — in practice, that every field newer than the release is `.optional()`. The only sanctioned exception to the GSM-first landing order; a PR green on `main` but red on the release ref waits for the next GSM release. Pattern named after the env-clear fix (#439).

**Wire Shape**
A named object on the GSM contract, described **exactly once** as a zod schema in its owning [[Slice]], with its TypeScript type derived via `z.infer` — never a hand-written `Serialized*` twin joined by a cast, which is how fields came to be "present" in TypeScript while zod stripped them at runtime. A Wire Shape declares **every field the backend emits**, including ones the card never reads (marked as such), so the [[Contract Fixture]] diff can tell drift from deliberate omission. Inbound shapes **strip** unknown keys — the only mode where reading an undeclared field is a compile error, since on zod 4 `.passthrough()`/`z.looseObject()` infer an index signature that silently permits it. Outbound request payloads are **strict** instead: the card authors them, so an unknown key is a card-side bug, and stripping one silently drops a field the card meant to send. See ADR 0031.

**Opaque Region**
A licensed hole in a [[Wire Shape]] — a spot left deliberately unvalidated instead of declared field-by-field. Two classes qualify. **Opaque-by-arity**: an open-ended collection whose element count is user-driven (`daily_readings`, `irrigation_tanks`, `sensor_groups`, `active_events`), where a stricter element type would let one malformed row fail the whole `get_data` parse — trading a missing field for a blank card. **Non-GSM**: shapes from a backend the project does not own and the [[Contract Fixture]] cannot reach, such as `HistoryPointSchema` over HA's REST history API. The test is blast radius, not ownership — a singular object the card never reads has no amplification, so it gets declared (`max_daily_readings`). Written as an explicit exemption with its reason at the call site, never as residual `.passthrough()`.
