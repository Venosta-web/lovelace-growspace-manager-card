...
Pure expand/collapse/accordion view state — the Climate tab's Temperature-Override and Critical-Temperature expanders (`_fanTempOverrideExpanded` / `_exhaustCriticalTempExpanded`), the Humidity tab's per-stage accordion (`_openHumidityStageId`), and the still-inline VPD-targets accordion (`_openVpdStageId`) — lives as **[[Config Dialog Shell]] `@state`**, *not* in the SM. The dumb [[Tab Component]] emits a toggle [[Tab Intent]] (`toggle-fan-temp-override`, `toggle-exhaust-critical-temp`, `toggle-stage`); the Shell flips its own `@state` and the flag is projected into the [[Tab ViewModel]]. This satisfies the no-`@state` component contract while keeping ephemeral view fluff out of the persistent SM. The ADR-0019 "draft/edit state → SM" rule targets *draft* state (the inline-edit-after-CANCEL bug class); expanders have no draft semantics, so they are deliberately exempt. The Humidity tab (#360) followed this carve-out; the VPD-targets tab (#363) carries the same accordion.
