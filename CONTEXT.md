...
**Chip** (also: Badge)
A small metric indicator rendered below the hero section. Each chip maps to a `HeaderChip` (canonically defined in `header-metrics/index.ts`; `metrics-utils.ts` re-exports the same shape for its legacy device-chip path) and carries a `MetricKey`. Clicking a chip opens the Env Graph for that metric. Chips support drag-and-drop reordering.
...
Pure derived module that, given a `(growspaceId, viewContext)`, returns `{ hero, chips }` for rendering. `viewContext` is `'main' | 'subarea' | 'analytics'` — the main and subarea cards get both hero and chips; the analytics card gets chips only. Inputs are `EnvSnapshot`, plant state, and irrigation state — all atom-sourced; per its own documented constraint it never touches `hass`/`hass.states`. For the *main growspace view* it supersedes `MetricsUtils.computeHeaderMetrics()` for hero and secondary chips — device chips (exhaust, fan, humidifier, dehumidifier, lights) still come from the legacy `MetricsUtils` path until [[DeviceState slice]] lands (issue #144), since deriving them requires reading `hass.states` directly. The *subarea view*'s hero/secondary chips (`MetricsUtils.computeSubareaMetrics`) are also still legacy-only — porting them is blocked on giving the Environment slice a subarea-aware snapshot, since [[EnvSnapshot]] is growspace-scoped and a subarea's sensor set is independently configured.
