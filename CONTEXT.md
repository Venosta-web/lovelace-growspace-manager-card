...
The agreed target shape of the Crop Steering Dialog: the single owner of *all* steering configuration and measured steering diagnostics. Three tabs: **Overview** (read-only measured metrics — Overnight Dryback, EC Trend, steering score vs. declared Steering Mode intent, phase state), **Steering** (Steering Mode selector + stamped targets, per-phase P1/P2 shot parameters, P2 Thresholds, phase override), and **Substrate & EC** (Substrate Profile, Shot Sizing Mode, Pore EC Target Band, EC Modulation, feed-EC target ranges). Supersedes the current two-tab (`diagnostics | settings`) dialog; the Irrigation Dialog's Steering tab retires once migration completes (Irrigation Dialog keeps schedules, tanks, EC ramp, manual mode). See the backend glossary for the steering domain terms.
