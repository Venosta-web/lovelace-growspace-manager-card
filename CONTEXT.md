...
**Hero Card**
The large metric display at the top of the main card and subarea card. Shows an aggregated sensor value (e.g. temperature, VPD). Clicking a hero card opens the Env Graph for that metric. Implemented in `growspace-header-hero-ui.ts`. Each hero item is itself a [[Chip]] (carries a `MetricKey`), so it can be hidden via `hidden_chips` the same way a regular chip can — there is no separate hero-specific visibility setting. When every hero metric is hidden, the hero deck collapses entirely rather than rendering empty. The VPD hero icon colour is stage-and-cycle-aware: `MetricsUtils` selects `day_vpd_*` or `night_vpd_*` thresholds from the overview entity based on the current `is_lights_on` state; those thresholds are pre-computed by the backend per grow stage (veg, flower, etc.), so the frontend gets both dimensions for free via a single day/night switch.

**Chip** (also: Badge)
A small metric indicator rendered below the hero section. Each chip maps to a `HeaderChip` (canonically defined in `header-metrics/index.ts`; the `metrics-utils.ts` re-export is retired with the MetricsUtils deletion — ADR-0018 P3) and carries a `MetricKey`. Clicking a chip opens the Env Graph for that metric. Chips support drag-and-drop reordering.
...
**EnvSnapshot**
A normalized point-in-time view of a growspace's — or a subarea's — sensor values, exposed as atoms by the Environment slice: `envSnapshots$` keyed by growspaceId, `subareaEnvSnapshots$` keyed by subareaId. The slice is the single place that reads `hass.states` for environmental sensors, structured as a shared read-and-aggregate core (explicit sensor lists → EnvSnapshot) behind two entity-resolution adapters: the **growspace adapter** (slug/overview-entity resolution, growspace calculated-VPD fallback chain) and the **subarea adapter** (direct `environment_config` sensor lists, per-temp/hum-pair subarea calculated-VPD resolution — `sensor.growspace_manager_{gsId}_subarea_{subId}_calculated_vpd*`). Both scopes share this one type; fields without configured sensors are null. Both maps are fed by `sync-service` on each hass update (subareas iterated from the Subarea slice's `subareas$`, so a card showing a subarea must ensure it is hydrated there). Covers the full sensor set from the config dialog: air metrics (temperature, humidity, VPD, CO₂, DLI, optimal conditions), substrate/medium metrics (soil moisture, substrate temperature), and irrigation monitoring metrics (pH, feed EC, bulk EC, pore EC, runoff EC, drain volume, irrigation flow, power, energy). Multi-sensor fields carry a `SensorReadings` object (`avg`, `perSensor`, `entityIds`) to support the "Multiple + per-sensor" chip display pattern. `SensorReadings.entityIds` is also the single source for history fetching — the subarea card derives its metric→entityIds mapping from the snapshot rather than re-resolving from `environment_config` (retiring its private `_resolveCalculatedVpdIds`). Cards and derived modules subscribe to the atoms instead of reaching into `hass`. See ADR-0018.

**DeviceSnapshot**
The device-controlled-entity counterpart of [[EnvSnapshot]], owned by the DeviceState slice (`src/slices/device-state/`): a normalized view of light sensors, exhaust fans, circulation fans, humidifiers, and dehumidifiers for one growspace or subarea. Each category is a `DeviceEntry` (`entityIds`, aggregated display `value` — `"On"`, `"70%"`, `"Multiple"`, … — `multiValues` for multi-entity categories, MDI icon) or null when unconfigured. The slice is the single place that reads `hass.states` for device entities; [[Fan Entity Mode]] runtime detection (ADR-0008) lives in its normalizers. Mirrors the EnvSnapshot adapter split: `deviceSnapshots$` (keyed by growspaceId) and `subareaDeviceSnapshots$` (keyed by subareaId), both fed by `sync-service`. The [[HeaderMetrics module]] consumes it to build device chips. Closes issue #144. See ADR-0018.

**HeaderMetrics module**
Pure derived module that, given a `(growspaceId, viewContext)`, returns `{ hero, chips, deviceChips }` for rendering. `viewContext` is `'main' | 'subarea' | 'analytics'` — the main and subarea cards get hero, secondary chips, and device chips; the analytics card gets chips only. Inputs are [[EnvSnapshot]], [[DeviceSnapshot]], plant state, and irrigation state — all atom-sourced; per its own documented constraint it never touches `hass`/`hass.states`. Supersedes `MetricsUtils` (`computeHeaderMetrics()` and `computeSubareaMetrics()`) entirely: device chips (light, exhaust, circulation fan, humidifier, dehumidifier) are derived from the DeviceSnapshot input and keep the legacy `MetricKey`s (`LIGHT`, `EXHAUST`, `CIRCULATION_FAN`, `HUMIDIFIER`, `DEHUMIDIFIER`) so existing `hidden_chips` configs and graph toggling are unaffected. The subarea view is fed by the subarea-scoped snapshot adapters and passes empty plant/irrigation inputs — dominant-stage and irrigation chips simply don't render there. Migration is phased (ADR-0018): P1 device chips into the result for the main view; P2 subarea adapters + subarea card migration; P3 delete `metrics-utils.ts` and repoint `HeaderChip` type imports to this module's canonical definition.
