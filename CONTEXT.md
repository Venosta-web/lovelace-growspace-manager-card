...
The top group of three tabs in the Irrigation Dialog — **Overview**, **Steering**, **Substrate & EC** — that consolidates all crop-steering interaction into one home. Overview is the read-only diagnostics readout (score, mode badge, dry-back, peak/trough VWC, EC trend); Steering is the [[Phase Windows]] override (immediate-write, confirm-on-change); Substrate & EC is the per-stage feed-EC target ranges (draft + explicit Save). The three share one visibility gate (`(hasSoilMoisture || hasStrategy) && hasPump`) so they appear and disappear together. Replaces the former standalone `crop-steering-dialog`, which is retired — the crop-steering metric chip now opens the Irrigation Dialog on the Overview tab via `initialTab`. The [[Crop Steering Day Chart]] is unaffected: it stays in the Schedules/Steering content, not the Overview tab.
