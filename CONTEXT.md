...
`envDraftFromDevice` (reached via `createInitialSM(device)`) is the **single seeding seam** — the one `GrowspaceDevice → EnvironmentDraft` transform, owning the legacy-singular fallbacks and the AC Infinity bundles. The Config Dialog takes a `growspaceId` string, resolves the device from its injected `devices` list, and seeds **once per open** via `_seedFromDevice` (gated by `_initialStateApplied`, so a later background `devices` refresh never clobbers in-progress edits; with no `growspaceId` the constructor-default draft is left untouched). This replaced three drifted duplicate seeders — `openConfigDialog`'s flat `EnvironmentConfigData` payload, the dialog's `setInitialState`, and the Subarea card's hand-copied env snapshot — all now deleted along with the `EnvironmentConfigData` type. Openers (header container, Subarea card) pass only a growspace id.
...
The pure function (`composeEnvironmentConfig(draft) → EnvironmentConfigEventDetail`, plus `needsExhaustCall(payload) → boolean`) that turns the [[Shared Environment Draft]] into the outbound `configure-environment-submit` **event detail** — *not* the `configure_environment` service payload. It exists because `configure_environment` is a **full replace**: it rebuilds `EnvironmentConfig` and silently resets any field not present (see also the integration's own note that `configure_environment` is a full replace). Composing the *whole* seeded draft on every env-tab save re-sends a complete config, so a sensor-only edit no longer clobbers fan/irrigation fields (the full-replace bug class becomes a pure unit assertion). The two humidity control toggles (`humidifierControlEnabled` / `dehumidifierControlEnabled`) are ordinary [[Shared Environment Draft]] fields — seeded by `envDraftFromDevice` and mutated via `UPDATE_ENV_DRAFT` like any other, so the composer reads them straight off the draft (no separate `controlFlags` argument). Their toggle handlers additionally fire the live `set_humidifier_control` / `set_dehumidifier_control` service call immediately, so the flag persists on toggle as well as riding the next full-replace save. **Architecture:** the Config Dialog persists by *dispatching* `configure-environment-submit`; the [[Growspace Dialog Host]] fulfils it as `configure_environment` + a conditional, last-dispatched `configure_exhaust_fan`. So the host — not the dialog — owns the detail→service mapping and the two-call orchestration (config-dialog has no [[MutationRunController]]). `needsExhaustCall` is the host's gate for that second call (`configure_environment` cannot carry `exhaust_fan_config`; see [[Exhaust Fan Controller Panel]]); the Climate tab (#359) is the primary producer of a non-default exhaust config.
