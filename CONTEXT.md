...
The app-global module (`services/hydrate.ts`) that turns one fetched growspace collection into populated slice atoms. Signature: `hydrate(collection: Record<string, GrowspaceAPIResponse>, hassStates: HassEntities): Set<string>`. Transforms the raw WS collection **once** (via `GrowspaceAdapter.transformGrowspace`) and calls every slice's bootstrap setter (`setDevices`, `setPlants`, `setEnvSnapshot`, `setIrrigationConfig`, `setTankLevels`, `setSubareaEnvSnapshot`, …). Returns the watched-entity ID set as a byproduct of the fan-out — the [[Bootstrap controller]] stores and uses this for the entity-change optimization. Reads `subareasGrowspaceId$` / `subareas$` atoms internally to resolve the subarea list (payload-sourced subareas are replaced by the fresher hydrated list when `subareasGrowspaceId$` matches). It is the single fan-out to ~7 slices and the **single bootstrap writer** of `devices$` (Grid slice) — `growspaceDevices$` (Growspace slice) is the retired atom; the orchestrator does not write it. Distinct from [[mutate]] (which syncs *one* optimistic write); this is the *read/bootstrap* counterpart. Pure of Lit and of `hass` injection — inputs are the raw collection plus a `hass.states` snapshot — so it is tested with a fake collection and no host (`src/services/hydrate.test.ts`).
_Avoid_: putting the fan-out in the Growspace slice; letting it read `hass` directly; writing `growspaceDevices$` from the orchestrator.

**Bootstrap controller**
A per-card Lit `ReactiveController` (`src/controllers/bootstrap.controller.ts`) that owns the *per-card* half of the old `SyncService`. Constructor: `(host, gridSliceRef, cardConfig)`. Owns: `_lastHassRef` (reference-equality guard), `_isFetching` (in-flight guard), `_lastCollection` (cached raw payload for entity-change re-runs), `_watchedEntities: Set<string>` (entity-change optimization, seeded from `hydrate()`'s return value), `_defaultApplied` (per-card auto-select flag), `loading: boolean` (plain property; calls `host.requestUpdate()`). Two paths on `updateHass(hass)`: (1) no collection cached → call `fetchRawCollection()` → `hydrate(collection, hass.states)` → cache collection + store watched-entity set; (2) collection cached, a watched entity's state reference changed → `hydrate(_lastCollection, hass.states)` without re-fetching. Fetches via the Growspace slice's `fetchRawCollection()` export (a bare `hassCall` wrapper that returns the raw payload without transforming or writing atoms). Peer to `hass-subscription-controller` / `polling.controller`. The split is deliberate: app-global domain hydration lives in the orchestrator (testable without a host); per-card UI state lives here (tested by driving `hass` against a host, `tests/controllers/bootstrap.controller.test.ts`). Replaces `SyncService`'s per-card responsibilities; `SyncService` and its [[DataService]] dependency are then deleted once all cards migrate.
