...
Despite the legacy name, this is **not** a secondary-header [[Chip]] — no `MetricKey.WATER` chip is built in the header-metrics slice. The live `waterUsage.litersToday` value is rendered as the **"Liters today" KPI card** in the Water Analytics tab (`irrigation-water-analytics-tab.ts`, `_renderTodaysUsage`), gated on `vm.hasPump`. Its value is the [[Aggregate Water Use]] figure, so as of `growspace_manager`'s ADR-0017 it reflects tank-derived **plus** manual watering (in tank mode) rather than tank-derived alone — the displayed number can include non-tank water, which is why the "tank-derived" name is now a misnomer. The KPI section (`_renderTodaysUsage`) is gated on `vm.hasPump || vm.hasTank`, so it renders for a [[Irrigation Method|tank-based]] grower too. **Note:** `hasPump` (an irrigation/drain *pump entity* is configured) is independent of [[Tank-Derived Water Mode]] (tanks present, no flow/drain sensors). A gravity-fed tank grower with no pump has `hasPump` false but `hasTank` true, so the figure now surfaces via the tank gate (previously this grower had no display surface for the additive figure — see [[Irrigation Method]]). Separately, `MetricKey.WATER` routes to a [[Tank Water Chart]] (`growspace-analytics-ui.ts`), which fetches its own bucketed history and does not read `litersToday`.
