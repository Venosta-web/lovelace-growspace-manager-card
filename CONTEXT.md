...
**Contract Fixture**
The GSM repo's golden growspace payload (`tests/fixtures/contract/growspace_payload.json` there), serialized from a maximally populated growspace, fetched from two refs: GSM `prerelease` and the latest GSM release. Card CI compares its key set against the [[Wire Shape]] tree, with a different verdict per ref — against `prerelease` a fixture key the schema does not declare is drift and fails; against the release ref the check runs the other way, failing only when the schema _requires_ a key the released backend does not send. (Not a strict parse: production schemas strip unknown keys, because [[hassCall seam|hassCall]] throws on parse failure and a strict schema would blank the card on the next backend release. ADR 0031 amends ADR 0029.) **Not yet built** — until it lands, completeness is verified per shape by round-trip tests plus manual confirmation at both refs.

**Backward-Safe Card Change**
A card change whose declared [[Wire Shape]]s require no key the **release-ref** [[Contract Fixture]] lacks, proving it works against the GSM version users actually run — in practice, that every field newer than the release is `.optional()`. The only sanctioned exception to the GSM-first landing order; a PR green on `prerelease` but red on the release ref waits for the next GSM release. Pattern named after the env-clear fix (#439).
