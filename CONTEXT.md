...
*Resolves an ambiguity:* VPD status was derived twice — the Environment slice's `_resolveVpdStatus` (current value, `vpd_target_*`/`vpd_danger_*`) and the chart's own threshold lookup (historical points, with a `day_`/`night_` split and a night-falls-back-to-day rule the slice lacks). These are not redundant — the slice answers "status *now*", the chart needs a threshold *function* over past points with `ChartUtils.getIsDay` deciding day vs night per point — but the threshold *table* was duplicated, and the Metric Descriptor now owns it. See ADR-0030.
