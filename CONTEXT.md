...
**Active growspace** *(per-card selection — two atoms, one live)*
Which growspace a card is currently showing/acting on. **Two `$selectedDevice` atoms coexist** and this distinction is a live footgun: (1) the page-global `selectedDeviceId$` (`slices/grid/index.ts`, exposed via the `gridSlice` facade) — the pre-slice shared atom, now **never written by any live code**; and (2) a per-card isolated atom minted by `makePerCardGridSlice()`, which every [[GrowspaceDataStore]] owns (`store.grid.$selectedDevice`) and which is the **only** one written — by `handleDeviceChange` → `setSelectedDevice` and by every [[Bootstrap controller]]. Per-card isolation is deliberate so carousel and standalone cards don't clobber each other's selection (same motivation as the page-global-UI-state fixes, #406/#407/#408). **Any code that resolves the active growspace must read the per-card atom (or an explicit `growspaceId` threaded through the dialog payload) — never the global `selectedDeviceId$`, which always reads `null`.** The [[ActionDispatcher]] retirement (#342–344) violated this: the inlined add-plant handlers (`_confirmAddPlant`/`_confirmAddPlants`) and the no-arg `openAddPlantDialog()` were pointed at the global atom, so every add-plant save failed with "No growspace selected" even though the dialog *rendered* the right growspace (its `render()` correctly reads the per-card atom). The pre-retirement code read `ctx.grid.$selectedDevice` (per-card) and worked.
