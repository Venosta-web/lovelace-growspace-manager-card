...
**Env Graph Wall**
A desktop-only fullscreen overlay presenting every [[Open Env Graph]] at once, so a grower can dedicate a browser tab to their environment graphs. Toggled from a view-level button in the [[Time Range Selector]] row, which travels into the overlay as its sticky toolbar. Implemented by `growspace-analytics-ui` wrapping **its own output** in `<ha-dialog width="full">` inside the card's shadow root — the charts move into the dialog rather than being duplicated, so no chart remounts and neither the crop-steering `PollingController` nor the tank fetch runs twice. It deliberately bypasses `growspace-dialog-host`: the standalone analytics card never mounts one, and the state lives in the container all three analytics hosts share. Ships on the main card, the subarea card, and the analytics card. Live-bound to `activeEnvGraphs` — it closes itself when the set empties — and ephemeral, dying on reload. The toggle is absent from the DOM (not hidden) on mobile and while any card task state is non-idle. See ADR-0046.
