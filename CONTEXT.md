...
**Env Graph**
An inline historical chart that appears when a Hero Card or Chip is clicked, showing sensor data over time for that metric. Toggled via `_toggleEnvGraph` / `_toggleMetricGraph`. Its render input is [[Env Series]] (value space); `<growspace-env-chart>` owns the geometry.
...
**Metric Descriptor** *(planned)*
The per-`MetricKey` record of everything both a [[Chip]] and an [[Env Graph]] must know about a metric: its backing sensor entities as structured `{ entityId, name }` pairs (not `'metric:entityId'` strings), display unit, colour, title, chart type (line vs step), axis scale (fixed bounds or auto), and — for VPD — the day/night threshold table. Derived by `computeMetricDescriptors()` in `slices/metric-descriptors/`. Like the [[HeaderMetrics module]] it takes **explicit data parameters and reads no atoms** — parameters are added as each metric family lands and its facts stop being static (at temperature-only it needs none). Unlike HeaderMetrics it is not atom-*only*: it takes its inputs the way `computeEnvSnapshot(device, hassStates)` and the [[Hydration orchestrator]] do — **pure of `hass` injection, with a `hass.states` snapshot passed as an argument**. That difference is forced, not stylistic: HeaderMetrics is atom-only because everything it needs already sits on the snapshots, whereas three of the descriptor's inputs sit on no atom today — the day/night VPD threshold table (`EnvSnapshot` carries only the resolved `vpdStatus`; `day_vpd_target_*` / `night_*` are read off `overviewEntityId`), the light sensor's `unit_of_measurement` (`DeviceSnapshot` holds a *display* string like `"70%"`, not the unit), and the per-sensor `friendly_name` used for multi-sensor series titles. What *is* already atom-covered: `DeviceEntry.entityIds` supplies the fan/light/exhaust/humidifier entities, and [[Fan Entity Mode]]'s type facet is id-derived, so fan axis scale and the `%` unit need no state lookup at all. Note also that the available inputs differ by `viewContext` — the growspace adapter leaves the four hero metrics' `SensorReadings` null, while the subarea adapter populates them. It becomes the **single owner of metric→entity resolution**, subsuming `history-store`'s `METRIC_ENTITY_KEYS` / `getEntityIdsForMetric`, so the multi-sensor grouping is carried structurally rather than flattened into a `':'`-joined key that every consumer re-parses. (No conflict with ADR-0005: that ADR keeps *transport* — `getHistory` / `getBatchHistory` / `getHistoryStats` — in `history-store`; metric→entity mapping is not transport.) Three consumers make this a real seam rather than a hypothetical one: `history-store` (which entities to fetch), the [[HeaderMetrics module]] (chip and hero display), and [[Env Series]] (graph shaping) — one table decides unit, colour and axis, so a chip and its own graph cannot disagree.
*Resolves an existing ambiguity:* VPD status is derived twice today — the Environment slice's `_resolveVpdStatus` (current value, `vpd_target_*`/`vpd_danger_*`) and the chart's `_getVpdThresholds` (historical points, with a `day_`/`night_` split and a night-falls-back-to-day rule the slice lacks). These are not redundant — the slice answers "status *now*", the chart needs a threshold *function* over past points with `ChartUtils.getIsDay` deciding day vs night per point — but the threshold *table* is duplicated, and the Metric Descriptor owns it. See ADR-0030.
_Avoid_: metric config (that is `METRIC_CONFIG`, one of its inputs), metric spec.

**Env Series** *(planned)*
The value-space render input of an [[Env Graph]]: per series `{ id, title, color, unit, points: { time, value, meta }[], min, max, avg, chartType, vpdBands? }` — domain units and timestamps only, **no pixels and no SVG paths**. Built by the pure `computeEnvSeries(descriptors, histories, range, now)` in `features/environment/env-series.ts`, alongside the existing pure `crop-steering-model.ts`; `<growspace-env-chart>` owns width and height and turns Env Series into path strings at render time. Keeping geometry out of the derivation is what makes composite-key expansion, step-vs-line detection, per-metric axis scaling and the VPD day/night status bands assertable without mounting the element — replacing the private-field reaches (`(element as any)._renderSeries`, `_computeGraphSeries`) that the pre-split shape forced on its tests. See ADR-0030 for the landing order and test-migration plan.
...
A fan/humidifier actuator that exposes **no single `fan`/`switch` entity** and so cannot ride the plain entity multi-selects. It is driven as a *bundle*: a mode `select` entity + a speed `number` entity + an on-speed value (1–10). Configured per role — the [[Fan Controller Panel]] and [[Exhaust Fan Controller Panel]] each hold an *array* of them (`circulation_fan_ac_infinity_devices` / `exhaust_fan_ac_infinity_devices`), and the Humidity tab holds `humidifier_`/`dehumidifier_ac_infinity_devices`. Rendered by the shared `renderAcInfinityDevices` helper: one bordered "Port N" card per bundle (mode picker, speed picker, on-speed) plus an add button. **Identity rule:** an entity counts as AC Infinity only when its entity-registry `platform` is `ac_infinity` — the mode picker offers `select.*` and the speed picker `number.*` entities *of that platform only* (read from `hass.entities[eid].platform`, an untyped-but-present runtime field on the frontend hass object). An already-saved entity is always kept in its picker's option list even if it no longer matches the filter (e.g. the integration is unavailable), so existing configs never render blank. Bundles can be filled in one step via [[Port Pre-fill]]. See ADR-0022 *in the integration repo* for the backend bundle model.

**Automated Mode Conflict**
When a bound [[AC Infinity Device]] port's mode `select` sits in one of the controller's self-running modes — any state other than `Off`/`On` (e.g. `Auto`, `VPD`, `Timer to On/Off`, `Cycle`, `Schedule`) — the AC Infinity controller/cloud keeps re-asserting the port and Growspace Manager's `On`/`Off` writes do not reliably stick. This is a limitation of the `ac_infinity` integration, not GSM, and GSM cannot fix it (the setting lives on the device/app). The config dialog surfaces a **passive, non-blocking warning** beside the port's mode picker whenever the selected `select` reads a non-`Off`/`On` state, telling the user to take that device out of its automated mode in the AC Infinity app/entity. Detection is **card-side and reactive** — a value derived live from `hass.states[modeEntity].state`, so it fires on fresh selection *and* on dialog reopen for a port left in an automated mode, and clears live once the mode changes. Applies to all four roles (`circulation`/`exhaust`/`humidifier`/`dehumidifier`). `unavailable`/`unknown` are treated as "not automated" (no warning), matching the backend `ACInfinityDriver.is_on()` rule. The device is named from the device-registry name (friendly-name fallback), and the message includes the current mode string. See the identical *Automated Mode Conflict* term in the integration `CONTEXT.md`.

**Port Pre-fill**
The device-picker affordance on every [[AC Infinity Device]] editor (grow light tab + the four actuator roles): the user picks an `ac_infinity` *device* (a port) and the card resolves the bundle's member entities from the frontend entity registry, matching by `device_id` + `platform === 'ac_infinity'` + `translation_key` + domain (`select`/`active_mode` → mode; `number`/`on_power` → speed/power; `time`/`schedule_mode_on_time`/`schedule_mode_off_time` → on/off time; `switch`/`sunrise_timer_enabled` + `number`/`sunrise_timer_minutes` → sunrise). Picking a device **overwrites all role fields**: resolved roles are filled, unresolved roles are *cleared* (never left stale from another port) with an inline warning naming what wasn't found (disabled entities don't appear in `hass.entities`, so they resolve as missing). Pure UI sugar — the saved config remains the explicit entity bundle; resolution happens only at the moment of the pick, never at save time or runtime, and a mis-resolution degrades to the manual pickers. The picker is ephemeral (not part of the environment draft — no seeder involvement); on dialog reopen it displays the device derived live from the saved mode entity's `device_id`. Only devices exposing an `ac_infinity` `active_mode` select are listed (what makes a device a controllable port, vs. the controller parent device). See card ADR-0028; amends the "one picker rejected" verdict of integration ADR-0024.

**Duplicate Port Warning**
A passive, non-blocking warning (same conventions as [[Automated Mode Conflict]]) rendered when a port's mode entity appears in more than one role bundle within the growspace's environment draft — e.g. the same port configured as both exhaust fan and dehumidifier, where two GSM controllers would fight over one device. Draft-scoped: it does not detect the same port claimed by a *different growspace*.
...
The single `environmentDraft: EnvironmentDraft` field on `ConfigDialogSM`, seeded up-front from the device by `envDraftFromDevice` with **every** environment field (sensors, climate, humidity, irrigation, vision, VPD targets, circulation + exhaust fan configs). The six env-group tabs each project their own slice of it and emit `UPDATE_ENV_DRAFT` intents; none owns an independent draft. Seeding it complete is what makes the full-replace save safe — see [[Environment Save Composer]].

**Ephemeral expander state stays Shell-`@state`** *(carve-out, ADR-0019)*
Pure expand/collapse/accordion view state — the Climate tab's Temperature-Override and Critical-Temperature expanders (`_fanTempOverrideExpanded` / `_exhaustCriticalTempExpanded`), the Humidity tab's per-stage accordion (`_openHumidityStageId`), and the VPD-targets tab's per-stage accordion (`_openVpdStageId`) — lives as **[[Config Dialog Shell]] `@state`**, *not* in the SM. The dumb [[Tab Component]] emits a toggle [[Tab Intent]] (`toggle-fan-temp-override`, `toggle-exhaust-critical-temp`, `toggle-stage`); the Shell flips its own `@state` and the flag is projected into the [[Tab ViewModel]]. This satisfies the no-`@state` component contract while keeping ephemeral view fluff out of the persistent SM. The ADR-0019 "draft/edit state → SM" rule targets *draft* state (the inline-edit-after-CANCEL bug class); expanders have no draft semantics, so they are deliberately exempt. Both accordion tabs (Humidity #360, VPD Targets #363) followed this carve-out, each moving its read logic + stage list to the VM (`readThreshold`, `getVpdOptimal`) and forwarding `{ key, …, value }` edit intents the Shell merges against the live draft.

The Humidity tab adds two related patterns: (1) the two control-enable toggles (`_humidifierControlEnabled` / `_dehumidifierControlEnabled`, which double as the [[Environment Save Composer]]'s `controlFlags`) are Shell `@state` *and* fire an immediate `setHumidifierControl` / `setDehumidifierControl` backend service — so the component emits a `set-humidifier-control` / `set-dehumidifier-control` intent (a side-effect, not a draft change) and the Shell does both. (2) Per-stage threshold edits forward `{ stage, cycle, point, value }` (the `stage` being the `DehumidifierStage`/`HumidifierStage` enum *value* Record key the VM supplies, distinct from the display `id`); the Shell merges against the live draft via the retained `_updateThreshold` / `_updateHumidifierThreshold`. The threshold-read logic + stage list + defaults moved to `humidity-tab.viewmodel.ts` (exported `readThreshold`), so the read is pure and unit-tested at its new home.

**Climate fan/exhaust config edits — Shell merges against the live draft**
The Climate [[Tab Component]] (`<config-climate-tab>`) forwards a *partial* fan/exhaust edit via `fan-config-changed` / `exhaust-config-changed` intents; the [[Config Dialog Shell]] merges it against `this._sm.environmentDraft.circulationFanConfig` (resp. `exhaustFanConfig`) and dispatches `UPDATE_ENV_DRAFT`. This (rather than the component merging against its own `.vm` prop and emitting the whole config) is what makes synchronous multi-field edits accumulate: the Shell reads the live, just-updated draft on each merge, whereas the component's `.vm` prop only refreshes on the next async render. The component still never reads the SM — it forwards a field partial; the merge-against-live-draft is the Shell's.

**Config Tanks tab — a hybrid (env-draft list + own sub-state)** *(`<config-tanks-tab>`, #364)*
The Config Dialog's Tanks tab is the first **independent-draft** tab decomposed, but it is really a *hybrid*: the tank **list** lives in `environmentDraft.irrigationTanks` (so the tab rides the [[Environment Save Composer]] like the env-cluster tabs — add/delete dispatch `UPDATE_ENV_DRAFT`), while the inline add/edit **draft** is its own `tabs.tanks.sub` SM sub-state (`idle | adding | editing`, like the Notifications tab). The [[Tab ViewModel]] projects both — formatted list rows from the env draft, the open editor from the sub-state. The dumb component emits `add-/edit-/delete-tank-requested`, `tank-draft-changed`, `cancel-tank`, `save-tank-requested`; the Shell keeps `_openAddTank` / `_editTank` / `_deleteTank` / `_saveTank` / `_cancelTank` (which dispatch the `BEGIN_ADD_TANK` / `BEGIN_EDIT_TANK` / `COMMIT_TANK` SM events) as the intent handlers. **Not** the Irrigation *dialog*'s `<irrigation-tanks-tab>`, which edits live [[Tank Levels]] through the Irrigation slice — this one edits [[Tank Config]] in the config dialog's env draft (see [[Tank Config vs Tank Levels]]).

**Environment Save Composer** *(implemented #358, `features/config/environment-save.ts`)*
The pure function (`composeEnvironmentConfig(draft, controlFlags) → EnvironmentConfigEventDetail`, plus `needsExhaustCall(payload) → boolean`) that turns the [[Shared Environment Draft]] into the outbound `configure-environment-submit` **event detail** — *not* the `configure_environment` service payload. It exists because `configure_environment` is a **full replace**: it rebuilds `EnvironmentConfig` and silently resets any field not present (see also the integration's own note that `configure_environment` is a full replace). Composing the *whole* seeded draft on every env-tab save re-sends a complete config, so a sensor-only edit no longer clobbers fan/irrigation fields (the full-replace bug class becomes a pure unit assertion). The two control toggles that live on the dialog outside the draft (`humidifierControlEnabled` / `dehumidifierControlEnabled`) are passed in as `controlFlags`. **Architecture:** the Config Dialog persists by *dispatching* `configure-environment-submit`; the [[Growspace Dialog Host]] fulfils it as `configure_environment` + a conditional, last-dispatched `configure_exhaust_fan`. So the host — not the dialog — owns the detail→service mapping and the two-call orchestration (config-dialog has no [[MutationRunController]]). `needsExhaustCall` is the host's gate for that second call (`configure_environment` cannot carry `exhaust_fan_config`; see [[Exhaust Fan Controller Panel]]); the Climate tab (#359) is the primary producer of a non-default exhaust config.
