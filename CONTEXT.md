...
**Curated Combo**
A fixed pairing the card asserts belongs in one chart — a primary metric plus the secondaries that give it context (temperature with exhaust duty, soil moisture with irrigation shots). Its own `AnalyticsItem` type, **orthogonal to [[Metric Comparison]]**: a combo makes an editorial claim about which metric is primary and which is context, where a Comparison is a grouping the grower owns and can dismantle. The recipe set is a hard-coded table beside `METRIC_CONFIG`, not card YAML. Its geometry follows from its secondary's data shape — see [[Interval Metric]] and ADR-0049, ADR-0051. Rendered by `<metric-combo-chart>`, which does not draw the primary itself: it mounts the ordinary `<growspace-env-chart>` — [[Guide Mark]]s, [[Dark Period]] shading and scrub included — and projects its own bar panes into that chart's card through a `secondary-pane` slot, so they read as one chart. All are drawn against one window, computed by the combo and handed to the Env Graph as `chartWindow`, because two now-anchored windows resolved a moment apart are a silently misaligned axis. Routing is on the `AnalyticsItem` type rather than on a `MetricKey` (unlike [[Custom Graph Routing]]): the pairing is what the type carries, and the item's `metrics` is the combo's flat *identity* — what keys the render and says what history is needed — while its structure stays in the recipe.

A recipe holds a **list** of [[Combo Secondary]] panes, stacked in the order given, and a secondary the growspace has no sensor for drops out on its own, so a combo degrades one pane at a time rather than all-or-nothing. Six recipes ship: Temperature and CO2 with exhaust duty, Humidity with humidifier *and* dehumidifier duty, Soil Moisture with irrigation duty, Pore EC with the runoff-minus-feed EC delta, and Energy with power. Every pane is derived by `computeComboIntervalPane` in `features/environment/combo-series.ts`, in value space only.
_Avoid_: combo chart, default comparison, seeded comparison.

**Combo Secondary**
One subordinate pane of a [[Curated Combo]]: an [[Interval Metric]], optionally read `relativeTo` another metric so the pane reports the *difference* — feed EC against runoff EC says whether the substrate is accumulating salt, which neither absolute trace says alone. What a bar is depends on the metric's axis: one with a fixed axis reports **duty**, the time-weighted mean over the bucket as a percentage of its own full scale, because that is what a grower reads an actuator by; one that scales to its data has no full scale to be a percentage of and reports the mean in its own unit, as does every delta (the difference of two duties is percentage points, not duty). A recipe may also name a `limitOf` — *where* a configured ceiling comes from on the growspace, never the number — and where one resolves it, not the peak, is the pane's scale, drawn as a dashed rule the bars are read against. The bars still fit when they breach it. This is the two-pane primitive repeated and parameterised, not new geometry.
_Avoid_: sub-chart, mini chart, sparkline.
