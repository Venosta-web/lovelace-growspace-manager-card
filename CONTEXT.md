...
The state in which a growspace's water consumption is *inferred* from tank-level deltas rather than *measured* by flow/drain sensors. Backend-authoritative predicate (`view_model_builder`): the growspace has an environment config with **no** `irrigation_flow_sensors` and **no** `drain_volume_sensors`. In this mode the backend populates `waterUsage.litersToday` (= Σ per-tank `get_total_liters_today()`); outside it, `litersToday` is `null`. Independent of whether a pump is configured — a pump can draw from a monitored tank, so pump presence neither enables nor disables this mode.

**Tank-Derived Water Chip**
A [[Chip]] in the secondary header strip showing a growspace's calendar-day water consumption (liters since local midnight) when it is in [[Tank-Derived Water Mode]]. Value is `waterUsage.litersToday` from the growspace payload; because the backend supplies that field *only* in Tank-Derived Water Mode, the chip keys off its presence (`litersToday != null`) plus a `tanks.length > 0` guard (which suppresses the 0-liter chip on an otherwise-unconfigured growspace) rather than re-deriving the flow/drain predicate client-side. Clicking it opens a [[Tank Water Chart]] via [[Custom Graph Routing]] on `MetricKey.WATER`. _Flagged duality:_ the [[Water Analytics Tab]]'s "Consumed today" KPI is the **same** calendar-day figure derived independently client-side from tank `waterHistory`; the chip reads the server figure while that KPI re-derives it — the two are not yet consolidated.
