...
**Env Graph**
An inline historical chart that appears when a Hero Card or Chip is clicked, showing sensor data over time for that metric. Toggled via `_toggleEnvGraph` / `_toggleMetricGraph`. Its render input is [[Env Series]] (value space); `<growspace-env-chart>` owns the geometry and nothing else — it derives no series of its own and reads no `hass.states`, taking the [[Metric Descriptor]] table as a property from `growspace-analytics.container` (#472).
