...
The config dialog panel (inside the Climate tab, between the Climate Control panel and the Humidity Control panel) that exposes all `CirculationFanConfig` fields: enabled toggle, regulation mode selector, active-mode target+tolerance pair, VPD-mode-only temperature override sub-section (collapsed by default), min/max speed, dynamic wind settings (period + amplitude revealed when wind*enabled is toggled on), and — when Stage-Aware VPD is on — the [[Stage VPD Overrides Table]]. Disabled fields are greyed out when `enabled` is false, not hidden. Submitted as part of the existing `configure-environment-submit` event; the config **rides inside the `configure_environment` payload** as `circulation_fan_config` — there is **no** separate service call in the save path. (The standalone `configure_circulation_fan` service, the `configureFanController` store action, and the `configure_circulation_fan` backend handler all exist but are **dead in the dialog save path** — circulation persists purely via `configure_environment`. The glossary previously claimed a separate dispatch; that does not match the current code. Contrast the [[Exhaust Fan Controller Panel]], which \_must* use its own service because `configure_environment` does not accept `exhaust_fan_config`.)
