...
import { mdiClose, mdiPlus, mdiDragHorizontalVariant, mdiSprout, mdiFlower, mdiCalendarClock, mdiDna, mdiCannabis } from '@mdi/js';
...
export class GrowspaceManagerCard extends LitElement implements LovelaceCard {
  ...
  private _getPlantStageIcon(state: string): string {
    switch (state.toLowerCase()) {
      ...
      case 'dry': return mdiFlower;
      case 'cure': return mdiCannabis;
      default: return mdiSprout;
    }
  }
  
  private async _harvestPlant() {
    ...
    try {
      await this.hass.callService('growspace_manager', 'harvest_plant', {
        ...
        target_growspace_name: 'dry', // your dry growspace ID
      });
              ...
              >
                <div class="plant-header">
                  ...
                  <svg style="width:12px;height:12px;fill:var(--secondary-text-color);opacity:0.5;" viewBox="0 0 24 24">
                    <path d="${mdiDragHorizontalVariant}"></path>
                  </svg>
                </div>
              ...
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--spacing-md);">
                <div class="form-group">
                  ...
                  <input 
                    ...
                    @input=${(e: Event) => {
                      const target = e.target as HTMLInputElement;
                      if (this._plantOverviewDialog) this._plantOverviewDialog.editedAttributes.veg_start = target.value;
                    }}
                  />
                </div>
