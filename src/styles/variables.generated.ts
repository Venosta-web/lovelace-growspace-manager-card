  ...
  :host {
    ...
    */
    ...
    /* The \`?? fallback\` arm of a stage lookup, for a stage the card does not recognise. ADR
       ...
       header-metrics used to fall back to the veg green, so an unrecognised stage rendered as
       a recognised one.
    */
    --stage-unknown: #757575;
    /* Red 900, the tier above --error-color on the vision checkup severity ramp
       (none/low/medium/high/critical). Deliberately not --error-dark, whose note already binds
       it to the danger gradient stop and the destructive-button pressed state — a severity
       tier and a pressed state would have to move together for no reason.
    */
    ...
    /* Orange 300, the label on the current-stage chip in the plant overview, over its own
       ...
       5.88:1 there and already passes AA. The brightening buys 7.33:1 at 0.7rem, which is why
       the value is kept rather than folded.
    */
