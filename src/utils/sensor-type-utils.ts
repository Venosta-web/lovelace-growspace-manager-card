...
export class SensorTypeUtils {
  static isLight(device: GrowspaceDevice | undefined, hass: any, entityId: string): boolean {
    ...
    const isIlluminance =
      ...
      (unit && (unit.includes('lx') || unit.includes('fc') || unit.toLowerCase().includes('lux')));
    const isLightId =
      lowerId.includes('light_sensor') ||
      lowerId.includes('illuminance') ||
      (lowerId.includes('light') &&
        !lowerId.includes('humidifier_light') &&
        !lowerId.includes('vpd'));
  ...
  static isTemperature(device: GrowspaceDevice | undefined, hass: any, entityId: string): boolean {
    ...
    const lowerId = entityId.toLowerCase();
    return (
      deviceClass === 'temperature' || (unit && unit.includes('°')) || lowerId.includes('temp')
    );
  }
  ...
  static isHumidity(device: GrowspaceDevice | undefined, hass: any, entityId: string): boolean {
    ...
    return (
      ...
      (unit && unit.includes('%')) ||
      lowerId.includes('humi') ||
      lowerId.includes('humid')
    );
  }
  ...
  static isVPD(device: GrowspaceDevice | undefined, hass: any, entityId: string): boolean {
    ...
    return (
      ...
      (unit && (unit.includes('Pa') || unit.includes('vpd'))) ||
      lowerId.includes('vpd') ||
      lowerId.includes('calculated_vpd') ||
      lowerId.includes('deficit')
    );
  }
  ...
  static isSoilMoisture(device: GrowspaceDevice | undefined, entityId: string): boolean {
    ...
    return !!(
      env?.soilMoistureSensors?.includes(entityId) ||
      env?.soilMoistureSensor === entityId ||
      types[entityId] === 'soil_moisture'
    );
  }
  ...
  static isIrrigationPump(device: GrowspaceDevice | undefined, entityId: string): boolean {
    ...
    return (
      device.irrigationConfig?.irrigationPumpEntity === entityId ||
      types[entityId] === 'irrigation_pump'
    );
  }
  ...
  static isDrainPump(device: GrowspaceDevice | undefined, entityId: string): boolean {
    ...
    const types = device.environmentAttributes?.sensorTypes || {};
    return (
      device.irrigationConfig?.drainPumpEntity === entityId || types[entityId] === 'drain_pump'
    );
  }
  ...
  static isCO2(device: GrowspaceDevice | undefined, entityId: string): boolean {
    ...
    const types = env?.sensorTypes || {};
    return !!(
      env?.co2Sensors?.includes(entityId) ||
      env?.co2Sensor === entityId ||
      types[entityId] === 'co2' ||
      entityId.toLowerCase().includes('co2')
    );
  }
  ...
  static isHumidifier(device: GrowspaceDevice | undefined, entityId: string): boolean {
    ...
    const types = env?.sensorTypes || {};
    return !!(
      env?.humidifierEntities?.includes(entityId) ||
      env?.humidifierEntity === entityId ||
      types[entityId] === 'humidifier'
    );
  }
  ...
  static isDehumidifier(device: GrowspaceDevice | undefined, entityId: string): boolean {
    ...
    const types = env?.sensorTypes || {};
    return !!(
      env?.dehumidifierEntities?.includes(entityId) ||
      env?.dehumidifierEntity === entityId ||
      types[entityId] === 'dehumidifier'
    );
  }
  ...
  static isIrrigationTank(device: GrowspaceDevice | undefined, entityId: string): boolean {
    ...
    const types = env?.sensorTypes || {};
    return !!(
      env?.irrigationTanks?.some((t) => t.sensorEntity === entityId) ||
      types[entityId] === 'irrigation_tank' ||
      entityId.toLowerCase().includes('tank')
    );
  }
