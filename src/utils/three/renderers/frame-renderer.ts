...
export class FrameRenderer extends BaseRenderer {
    ...
    public render() {
        ...
        const gridMat = this.getSharedMaterial('gridMat', () => new THREE.LineBasicMaterial({ color: 0x222222 }));
    ...
    private createFrameModel(width: number, height: number, depth: number): THREE.Group {
        ...
        const aluminumMat = this.getSharedMaterial('aluminum', () => new THREE.MeshStandardMaterial({
            color: 0xf0f0f0,
            metalness: 0.6,
            roughness: 0.4,
        }));
        const connectorMat = this.getSharedMaterial('connector', () => new THREE.MeshStandardMaterial({ color: 0x333333, roughness: 0.8, metalness: 0.2 }));

        const poleRadius = 1.0;
        const poleGeo = this.getSharedGeometry('poleGeo', () => new THREE.CylinderGeometry(1, 1, 1, 12));
        const connectorGeo = this.getSharedGeometry('connectorGeo', () => new THREE.BoxGeometry(2.4, 2.4, 2.4));
