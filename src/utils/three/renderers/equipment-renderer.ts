...
export class EquipmentRenderer extends BaseRenderer {
  ...
  public render() {
    ...
    const pumps = new Set(
      [irrigationConfig?.irrigationPumpEntity, irrigationConfig?.drainPumpEntity].filter(Boolean)
    );
    Object.keys(env?.sensorTypes || {}).forEach((k) => {
      if (
        env?.sensorTypes?.[k] === this.SENSOR_TYPES.IRRIGATION_PUMP ||
        env?.sensorTypes?.[k] === this.SENSOR_TYPES.DRAIN_PUMP
      ) {
        pumps.add(k);
      }
    });
    ...
    this._humidifiers = Array.from(this.cache.values()).filter(
      (g) => g.userData.types?.includes('humidifier') || g.userData.types?.includes('dehumidifier')
    ) as THREE.Group[];
    this._pumps = Array.from(this.cache.values()).filter(
      (g) =>
        g.userData.types?.includes('irrigation_pump') || g.userData.types?.includes('drain_pump')
    ) as THREE.Group[];
    this._exhaustFans = Array.from(this.cache.values()).filter(
      (g) => g.userData.types?.includes('exhaust') || g.userData.entityId?.includes('exhaust')
    ) as THREE.Group[];
  }
  ...
  private updateHumidifierModel(
    ...
    if (
      isOutside &&
      (group.userData.isOutside !== isOutside || group.userData.targetH !== targetH)
    ) {
      ...
      const hose = new THREE.Mesh(
        new THREE.TubeGeometry(path, 20, 1.5, 8, false),
  ...
  private createHumidifierModel(
    ...
    const baseGeo = this.getSharedGeometry(
      'humBase',
      () => new THREE.CylinderGeometry(12, 12, 15, 32)
    );
    const darkMat = this.getSharedMaterial(
      'darkPlastic',
    ...
    const tankGeo = this.getSharedGeometry(
      'humTank',
      () => new THREE.CylinderGeometry(12, 12, 35, 32)
    );
    const tankMat = this.getSharedMaterial(
      'humTankGlass',
      () =>
    ...
    const panelMat = this.getSharedMaterial(
      'panelMat',
    ...
    if (intensity > 0) {
      ...
      const digitsMat = this.getSharedMaterial(
        'digitsMat',
  ...
  private createDehumidifierModel(
    ...
    if (isOutside) {
      ...
      const hose = new THREE.Mesh(
        new THREE.TubeGeometry(path, 20, 4.5, 8, false),
        this.getSharedMaterial(
          'dehumHoseMat',
          () => new THREE.MeshStandardMaterial({ color: 0x222222, roughness: 0.8 })
        )
  ...
  private createPumpModel(
    ...
    const bracketMat = this.getSharedMaterial(
      'bracketMat',
      () =>
        new THREE.MeshPhysicalMaterial({
          color: 0xeeeeee,
    ...
    const outputPoint = new THREE.Vector3(
      -bodyLength / 2 - headLength - portLength,
      bodyRadius + baseHeight,
      0
    );
    ...
    group.userData = {
      ...
      isActive,
      isOutside,
      targetH: hoseTargetHeight,
      logicalZ: hoseTargetHeight,
    };
    return group;
  }
  ...
  private createExhaustModel(speed: number, rotation: number, entityId: string) {
    ...
    const bMat = this.getSharedMaterial(
      'exhaustBladeMat',
  ...
  private initHumidifierParticles() {
    ...
    geom.setAttribute(
      'position',
      new THREE.BufferAttribute(new Float32Array(count * 3).fill(0), 3)
    );
  ...
  private initWindParticles() {
    ...
    geom.setAttribute(
      'position',
      new THREE.BufferAttribute(new Float32Array(count * 3).fill(-1000), 3)
    );
    geom.setAttribute('velocity', new THREE.BufferAttribute(new Float32Array(count * 3), 3));
    geom.setAttribute(
      'lifetime',
      new THREE.BufferAttribute(new Float32Array(count).fill(Math.random()), 1)
    );
  ...
  private animateParticles(deltaTime: number) {
    ...
    if (this._humidifierParticles) {
      ...
      const active = this._humidifiers.filter(
        (h) => h.userData.intensity > 0 && !h.userData.isDehumidifier
      );
    ...
    if (this._pumpWaterParticles) {
      ...
      const active = this._pumps.filter(
        (p) => p.userData.isActive && p.userData.isOutside && p.userData.hosePath
      );
      ...
      for (let i = 0; i < life.length; i++) {
        ...
        if (life[i] <= 0 && activeExhaust.length > 0) {
          ...
          if (isSuction) {
            ...
            pos[i * 3 + 2] =
              worldPos.z + Math.cos(angle) * startOffset + (Math.random() - 0.5) * 10;
