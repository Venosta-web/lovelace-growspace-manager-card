...
export class PlantRenderer extends BaseRenderer {
    ...
    private createPlantContainer(row: number, col: number, cellWidth: number, cellDepth: number, plant: any, stage: string): THREE.Group {
        ...
        const hitBoxGeo = this.getSharedGeometry('hitBoxGeo', () => new THREE.CylinderGeometry(18, 18, 100, 8)); // Oversized but static geo
        const hitBoxMat = this.getSharedMaterial('hitBoxMat', () => new THREE.MeshBasicMaterial({ visible: false }));
    ...
    private createPotModel(radius: number, height: number): THREE.Group {
        ...
        const potMat = this.getSharedMaterial('potMat', () => new THREE.MeshStandardMaterial({
            color: 0x212121,
            roughness: 0.6,
            metalness: 0.2
        }));
        ...
        // Soil
        const soilGeo = this.getSharedGeometry(`soil_${radius.toFixed(1)}`, () => new THREE.CircleGeometry(radius * 0.95, 32));
        const soilMat = this.getSharedMaterial('soilMat', () => new THREE.MeshStandardMaterial({
            color: 0x3d2b1f,
            roughness: 0.9
        }));
        ...
        const perliteGeo = this.getSharedGeometry('perliteGeo', () => new THREE.SphereGeometry(radius * 0.02, 4, 4));
        const perliteMat = this.getSharedMaterial('perliteMat', () => new THREE.MeshBasicMaterial({ color: 0xffffff }));
    ...
    private createPlantModel(stage: string, potHeight: number, plant: any, requestUpdate?: () => void): THREE.Group {
        const group = new THREE.Group();
        let scale = 1;
        let density = 1;
        ...
        switch (stage.toLowerCase()) {
            case 'seedling': scale = 0.2; density = 0.3; break;
            case 'clone': scale = 0.3; density = 0.4; break;
            case 'veg': scale = 0.7; density = 0.8; break;
            case 'flower': scale = 1.0; density = 1.0; colorValue = 0x2e7d32; break;
            case 'mother': scale = 1.3; density = 1.2; break;
            default: scale = 0.8; colorValue = 0x8d6e63;
        }
        ...
        const stemGeo = this.getSharedGeometry('stemGeo', () => new THREE.CylinderGeometry(0.5, 1, 1, 8));
        const stemMat = this.getSharedMaterial('stemMat', () => new THREE.MeshStandardMaterial({ color: 0x558b2f, roughness: 0.9 }));
        ...
        const leafMat = this.getSharedMaterial(leafMatKey, () => new THREE.MeshStandardMaterial({
            color: leafColor,
            roughness: 0.8,
    ...
    private async addFlowerDetails(group: THREE.Group, scale: number, potHeight: number, stemHeight: number, plant: any, requestUpdate?: () => void) {
        ...
        const pistilTargetColor = strainColors.length > 1 ? new THREE.Color(strainColors[1]) : new THREE.Color(0xffa726);

        const budMatKey = `budMat_${budColor.getHexString()}`;
        const budMat = this.getSharedMaterial(budMatKey, () => new THREE.MeshStandardMaterial({
            color: budColor,
            roughness: 0.7,
        ...
        const whitePistilMat = this.getSharedMaterial('whitePistilMat', () => new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 0.9 }));
        const orangePistilMatKey = `orangePistilMat_${pistilTargetColor.getHexString()}`;
        const orangePistilMat = this.getSharedMaterial(orangePistilMatKey, () => new THREE.MeshStandardMaterial({ color: pistilTargetColor, roughness: 0.9 }));
        ...
        const sugarLeafMat = this.getSharedMaterial(sugarLeafMatKey, () => new THREE.MeshStandardMaterial({ color: leafColor, roughness: 0.8, side: THREE.DoubleSide }));

        const colaGeo = this.getSharedGeometry('colaGeo', () => {
            const g = new THREE.DodecahedronGeometry(1, 1);
            return g;
        });
        const sugarLeafGeo = this.getSharedGeometry('sugarLeafGeo', () => new THREE.SphereGeometry(1, 4, 4));
        const pistilGeo = this.getSharedGeometry('pistilGeo', () => new THREE.CylinderGeometry(0.05, 0.05, 1, 4));
