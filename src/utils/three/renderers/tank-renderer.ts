...
export class TankRenderer extends BaseRenderer {
  ...
  public render() {
    ...
    const fMat = this.getSharedMaterial(
      'tankFrameMat',
      () => new THREE.MeshStandardMaterial({ color: 0x1a1a1a })
    );
    const eMat = this.getSharedMaterial(
      'tankEdgeMat',
      () => new THREE.LineBasicMaterial({ color: 0x444444, transparent: true, opacity: 0.3 })
    );

    const cGeo = this.getSharedGeometry(
      'tankContainerGeo',
