...
export class SensorRenderer extends BaseRenderer {
    public render() {
        ...
        allSensorEntities.forEach(entityId => {
            ...
            if (!sensorModel) {
                if (isLight) {
                    const geo = this.getSharedGeometry('lightSensorGeo', () => new THREE.SphereGeometry(width * 0.02, 16, 16));
                    const mat = this.getSharedMaterial('lightSensorMat', () => new THREE.MeshBasicMaterial({ color: 0xffeb3b, transparent: true, opacity: 0.9 }));
    ...
    private createSensorProbeModel(color: string): THREE.Group {
        ...
        const cableMat = this.getSharedMaterial('cableMat', () => new THREE.MeshStandardMaterial({ color: 0x050505, roughness: 0.8 }));
        const glandMat = this.getSharedMaterial('glandMat', () => new THREE.MeshStandardMaterial({ color: 0xdddddd, metalness: 0.8, roughness: 0.2 }));
        const bodyMat = this.getSharedMaterial('probeBodyMat', () => new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 0.5 }));
        const filterMat = this.getSharedMaterial('probeFilterMat', () => new THREE.MeshStandardMaterial({ color: 0xbbbbbb, metalness: 0.6, roughness: 0.3 }));
        ...
        // 2. Gland
        const glandGeo = this.getSharedGeometry('glandGeo', () => new THREE.CylinderGeometry(1.2, 1.2, 3, 12));
        const gland = new THREE.Mesh(glandGeo, glandMat);
        gland.position.y = 3.5;
        group.add(gland);

        // 3. Body
        const bodyGeo = this.getSharedGeometry('probeBodyGeo', () => new THREE.CylinderGeometry(2.5, 2.5, 12, 16));
        ...
        const filterGeo = this.getSharedGeometry('probeFilterGeo', () => new THREE.CylinderGeometry(1, 1, 1, 16));
        const filter = new THREE.Mesh(filterGeo, filterMat);
        filter.scale.set(2.4, 6, 2.4);
        cageGroup.add(filter);

        const ringGeo = this.getSharedGeometry('probeRingGeo', () => new THREE.TorusGeometry(2.5, 0.1, 8, 24));
        for (let i = -1; i <= 1; i++) {
        ...
        const diodeGeo = this.getSharedGeometry('probeDiodeGeo', () => new THREE.TorusGeometry(2.51, 0.2, 8, 24));
        const diodeMatKey = `diodeMat_${color.replace('#', '')}`;
        const diodeMat = this.getSharedMaterial(diodeMatKey, () => new THREE.MeshStandardMaterial({
