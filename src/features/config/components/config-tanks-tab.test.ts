...
function row(over: Partial<TankRowVM> = {}): TankRowVM {
  return { index: 0, displayName: 'Main', sensorEntity: 'sensor.a', volumeLiters: 100, warningLevel: 20, ...over };
}

const draft: TankEditVM = { sensorEntity: 'sensor.a', name: 'Main', volumeLiters: 100, warningLevel: 20 };
...
describe('ConfigTanksTab — render', () => {
  ...
  it('renders a row per tank with its formatted summary', async () => {
    const el = await mount(makeVm({ showEmpty: false, tanks: [row(), row({ index: 1, displayName: 'Tank 2' })] }));
  ...
  it('renders the inline form (with sensor datalist) when editing', async () => {
    const el = await mount(makeVm({ showEmpty: false, editing: draft, sensorOptions: ['sensor.a', 'sensor.b'] }));
...
describe('ConfigTanksTab — intents out', () => {
  it('emits add-tank-requested on the Add Tank button', async () => {
    ...
    [...el.shadowRoot!.querySelectorAll('button')].find((b) => b.textContent?.includes('Add Tank'))!.click();
    expect(fired).toBe(1);
  });

  it('emits edit/delete intents with the row index', async () => {
    const el = await mount(makeVm({ showEmpty: false, tanks: [row({ index: 0 }), row({ index: 1, displayName: 'Tank 2' })] }));
  ...
  it('emits tank-draft-changed / cancel-tank / save-tank-requested from the form', async () => {
    ...
    [...el.shadowRoot!.querySelectorAll('button')].find((b) => b.textContent?.trim() === 'Cancel')!.click();
    [...el.shadowRoot!.querySelectorAll('button')].find((b) => b.textContent?.includes('Save Tank'))!.click();
