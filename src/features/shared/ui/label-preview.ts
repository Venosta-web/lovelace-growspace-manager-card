...
function buildQrSvg(data: string): string {
  ...
  for (let r = 0; r < count; r++) {
    for (let c = 0; c < count; c++) {
      if (qr.isDark(r, c)) {
        cells.push(
          `<rect x="${c * cellSize}" y="${r * cellSize}" width="${cellSize}" height="${cellSize}" fill="currentColor"/>`
        );
      }
    }
  }
...
export class LabelPreview extends LitElement {
  ...
  static styles = css`
    ...
    :host([data-density='low']) {
      opacity: 0.6;
      filter: contrast(0.7);
    }
    :host([data-density='normal']) {
      opacity: 1;
      filter: contrast(1);
    }
    :host([data-density='high']) {
      opacity: 1;
      filter: contrast(1.3);
    }
    ...
    :host([data-layout='std']) .label-inner {
      flex-direction: column;
    }
    :host([data-layout='std']) .field-name {
      font-size: 1em;
      font-weight: bold;
      width: 100%;
    ...
    :host([data-layout='std']) .lower-row {
      display: flex;
      flex: 1;
      gap: 8px;
    }
    :host([data-layout='std']) .text-cols {
      display: flex;
      flex-direction: column;
      flex: 1;
      gap: 2px;
    ...
    :host([data-layout='std']) .text-cols .field-lineage {
      font-weight: bold;
    }
    :host([data-layout='std']) .field-qr {
      width: auto;
      display: flex;
      align-items: center;
      justify-content: center;
    }
    :host([data-layout='std']) .field-qr svg {
      max-width: 48px;
      max-height: 48px;
      width: 100%;
      height: auto;
    ...
    :host([data-layout='tall']) .label-inner {
      flex-direction: column;
      align-items: center;
    }
    :host([data-layout='tall']) .field-name {
      font-size: 1em;
      font-weight: bold;
      width: 100%;
    }
    :host([data-layout='tall']) .lower-row {
      display: flex;
      flex-direction: column;
      width: 100%;
      gap: 2px;
    }
    :host([data-layout='tall']) .text-cols {
      display: flex;
      flex-direction: column;
      gap: 2px;
      font-size: 0.7em;
    }
    :host([data-layout='tall']) .field-qr {
      display: flex;
      justify-content: center;
      margin-top: 4px;
    }
    :host([data-layout='tall']) .field-qr svg {
      width: 60%;
      height: auto;
    }

    /* --- wide layout (50x15) --- */
    :host([data-layout='wide']) .label-inner {
      flex-direction: row;
      align-items: center;
      gap: 4px;
    }
    :host([data-layout='wide']) .field-name {
      font-size: 0.8em;
      font-weight: bold;
      white-space: nowrap;
    }
    :host([data-layout='wide']) .lower-row {
      display: flex;
      flex: 1;
      gap: 4px;
      align-items: center;
    }
    :host([data-layout='wide']) .text-cols {
      display: flex;
      flex-wrap: wrap;
      gap: 2px;
      font-size: 0.6em;
    }
    :host([data-layout='wide']) .field-qr {
      display: flex;
      align-items: center;
    }
    :host([data-layout='wide']) .field-qr svg {
      width: 32px;
      height: 32px;
    }
  ...
  private _renderTextCols() {
    return html`
      <div class="text-cols">
        ${this.fields.phenotype
          ? html`<span class="field-phenotype">${this.values.phenotype}</span>`
          : nothing}
        ${this.fields.breeder
          ? html`<span class="field-breeder">${this.values.breeder}</span>`
          : nothing}
        ${this.fields.lineage
          ? html`<span class="field-lineage">${this.values.lineage}</span>`
          : nothing}
        ${this.fields.startDate
          ? html`<span class="field-startDate">${this.values.startDate}</span>`
          : nothing}
        ${this.fields.stageAge
          ? html`<span class="field-stageAge">${this.values.stageAge}</span>`
          : nothing}
        ${this.fields.plantId
          ? html`<span class="field-plantId">${this.values.plantId}</span>`
          : nothing}
        ${this.fields.logo
          ? html`<div class="field-logo"><img src="${this.values.logo}" alt="logo" /></div>`
          : nothing}
      </div>
    `;
  }
  ...
  override render() {
    return html`
      ...
      <div class="label-inner">
        ...
        <div class="lower-row">${this._renderTextCols()} ${this._renderQr()}</div>
      </div>
    `;
  }
}
