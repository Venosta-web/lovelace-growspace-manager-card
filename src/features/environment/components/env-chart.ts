...
const MIGRATED_ENV_SERIES_METRICS = new Set<string>([MetricKey.TEMPERATURE, MetricKey.VPD]);
...
export class GrowspaceEnvChart extends LitElement {
  ...
  private _metricDescriptors() {
    const overviewEntity = this.device?.overviewEntityId
      ...
      : undefined;
    return computeMetricDescriptors(overviewEntity);
  }
  ...
  private _isEnvSeriesMigrated(): boolean {
    ...
    if (!MIGRATED_ENV_SERIES_METRICS.has(this.metricKey)) return false;
