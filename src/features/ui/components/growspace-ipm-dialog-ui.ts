...
export class GrowspaceIPMDialogUI extends LitElement {
  @property({ type: Boolean }) open = false;
  @property({ attribute: false }) growspaceId: string | undefined = undefined;
  @property({ attribute: false }) plantIds: string[] | undefined = [];
  @property({ attribute: false }) presets: Record<string, IPMPreset> = {};
    ...
    if (this._view === 'APPLY') {
      return html`
        ...
        <button
          ...
          ?disabled=${!this._selectedPresetId || this.isSubmitting}
          @click=${this._handleApply}
        >
          <ha-svg-icon .path=${mdiCheck} style="margin-right: 8px;"></ha-svg-icon>
          ${this.isSubmitting ? 'Applying...' : 'Apply Treatment'}
        </button>
      `;
